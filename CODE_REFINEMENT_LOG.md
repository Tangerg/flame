...
- `reference` is contractgen's final production cognitive finding at 22. It renders one header plus eight document sections: methods, HTTP endpoints, streaming methods, run-event reliability, discriminated unions, value constraints, capability gating, and system invariants.
- Each section consumes a distinct manifest projection and owns its own heading, explanatory prose, table/list shape, and row formatting. The only shared state is the append-only Markdown builder; no section computes facts used by another.
- Section and row order are externally visible generated bytes. The manifest already owns deterministic source ordering, while HTTP status formatting and union pattern rows are local presentation policies that must stay in their respective sections.
- Baseline focused contractgen and generated-drift tests remain green at 2.32s; production complexity scanning reports only `reference` at 22 cognitively.

### Root cause

The top-level document renderer accumulated every section body. Its nested loops and optional rows are unrelated presentation concerns joined only by writing to one builder.

### Impact and acceptance criteria

- Give the reference header and each manifest-backed document section one concrete private renderer.
- Keep `reference` as the explicit document-order composition path; do not add a template engine, generic table abstraction, section registry, or intermediate document model.
- Preserve API-reference Markdown byte-for-byte, including blank lines, table headers, em dashes, status formatting, and row order.
- Reduce every new function below the selected complexity threshold.
- Pass focused, drift, race, static, full Runtime/CLI, and bounded live verification.

### Plan

- **Completed:** extracted the header and eight section renderers, formatted them, compared generated output, and remeasured complexity.
- **Completed:** ran focused/full/live verification, cleaned resources, inspected compatibility, and recorded results.

### Validation

- The first post-change focused contractgen and generated-drift run passed in 5.10s. `go generate ./...` then produced no diff in `API_REFERENCE.md` or `manifest.json`.
- The final focused generated-reference, manifest, and drift tests passed in 5.16s against the 2.32s green baseline.
- Production-only complexity scanning now reports zero cognitive findings in contractgen. The only remaining findings are the pre-existing cyclomatic scores for the flat reflection-type switch in `schemaSet.walk` and the closed constraint-vocabulary switch in `applyValueConstraints`.
- Contractgen passed with `-race`; focused `go vet` and `staticcheck` also passed in the combined 16.36s gate.
- Runtime `GOWORK=off go vet ./...` plus `GOWORK=off go build ./...` passed in 8.50s, and uncached `GOWORK=off go test -count=1 ./...` passed in 65.22s.
- Current-workspace CLI `go vet ./...` plus `go build ./...` passed in 8.74s, and uncached `go test -count=1 ./...` passed in 49.32s.
- The current-source CLI completed a bounded production-bootstrap Run using the authorized DeepSeek configuration. It returned exactly `FLAME_LIVE_ROUND21_OK`, status `completed`, one step, 9,183 input tokens, 8 output tokens, 9,088 cache-read tokens, and 781ms total model duration. No credential value was printed or copied.
- `git diff --check` passed.

### Changes and compatibility

- The API-reference top level now states document order directly and delegates the header plus each manifest-backed section to a concrete renderer with an exact input type.
- No template engine, generic table helper, intermediate document model, or section registry was added. Blank lines, tables, lists, status rendering, section order, row order, and generated bytes are unchanged.
- Public APIs, wire shapes, generated machine-readable artifacts, persistence, diagnostics, and Runtime/CLI behavior are unchanged.

### Resource cleanup

- The temporary strict complexity configuration was deleted after the final scan.
- The bounded live Run used a validated `/tmp/flame-live-round21.*` directory containing its isolated Runtime home and current-source CLI binary. The exit trap moved it to the system Trash; no matching temporary path remained.
- No shared cache, global dependency, user Runtime data, or configuration was removed.
