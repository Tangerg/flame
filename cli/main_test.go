...
func requireTerminalModeLifecycle(t *testing.T, binary string, test terminalModeCase) {
	...
	waitForVisibleTerminalText(t, session, ptytest.Size{Cols: 80, Rows: 24}, "Ask flame")
...
func TestInteractiveBinaryConsumesTheStartupBrandOnce(t *testing.T) {
	...
	waitForVisibleTerminalText(t, session, size, "Flame CLI", "Ask flame")
	narrowSize := ptytest.Size{Cols: 36, Rows: 18}
	if err := session.Resize(narrowSize); err != nil {
		t.Fatal(err)
	}
	waitForVisibleTerminalText(t, session, narrowSize, "FLAME", "Ask flame")
	...
	cleared := waitForVisibleTerminalText(t, session, size, "cleared", "Ask flame")
	if visible := strings.Join(cleared.Rows(), "\n"); strings.Contains(visible, "Flame CLI") || strings.Contains(visible, "FLAME") {
...
func TestInteractiveBinarySurvivesResizeAndApprovalRoundTrip(t *testing.T) {
	...
	waitForVisibleTerminalText(t, session, ptytest.Size{Cols: 96, Rows: 24}, "Ask flame")
...
func TestInteractiveTerminalCanStartANewRunAfterCancelingInteractionReview(t *testing.T) {
	...
	session, err := ptytest.Start(t.Context(), ptytest.Config{
		...
		Env: terminalTestEnvironment(t, map[string]string{
			"TERM": "xterm-256color", "COLORTERM": "truecolor", "LANG": "en_US.UTF-8",
			"FLAME_PTY_TEST_SCENARIO": cancelReentryPTYScenario,
		}),
...
func waitForInteractiveExit(t *testing.T, session *ptytest.Session) {
	...
	if err := session.Wait(ctx); err != nil {
		t.Fatalf("flame did not exit cleanly: %v", err)
	}
}
