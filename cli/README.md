...
```sh
# From the Flame worktree, runtime/config/config.yaml is discovered as the
...
Runtime durability and the user runtime `config.yaml` live under `$FLAME_HOME/runtime` (default `~/.flame/runtime`). That user file has priority; when it is absent and Flame is launched from this worktree, `runtime/config/config.yaml` is used as the development fallback. `FLAME_RUNTIME_CONFIG_DIR` selects another absolute config directory explicitly. The embedded binding exclusively leases the data directory until the CLI exits, so two CLI/desktop processes cannot open the same store concurrently; use a different absolute `FLAME_HOME` for an isolated instance. `FLAME_RUNTIME=mock` selects the scripted adapter explicitly for development and test automation.
...
The interactive client uses a stable agent shell: session and workspace identity at the top, a responsive one-shot Flame brand banner on initial entrance, selectable transcript content in the center, a compact live plan and run status near the bottom, and a framed multiline composer whose footer shows the provider-qualified model and run limits. The banner is presentation-only, is consumed by the first conversation block or clear, and never returns during session switching or recovery. Optional regions yield their space on constrained terminals; at extreme sizes the shell reduces to transcript, status, and a one-row composer, then restores the full chrome without losing the draft or keyboard focus. It supports session switching, runtime-backed workspace inspection, search, protocol-representable text/image attachments, structured approval and question workflows, provider/model and global approval-mode pickers, tool details, plugin inspection, and plugin reload/unload. `flame completion <bash|zsh|fish|powershell>` generates shell completion.

Core terminal interactions are available from both the keyboard and mouse:

- `Enter` sends a prompt while idle and queues it while a run, its cancellation, or a run-affecting runtime mutation is still settling. Queued prompts are session-scoped and drain in FIFO order only after every prerequisite has settled; with an empty composer, `Enter` promotes the next queued prompt and cancels the active turn first. `Shift+Enter` or `Alt+Enter` inserts a newline.
...
- Choice questions retain option descriptions and previews. When the runtime permits custom answers, `Other` adds a validated text field without replacing single- or multi-selection; restored answers and terminal resizes preserve both parts.
...
- `/schedules`, `/schedule-create`, `/schedule-edit`, `/schedule-enable`, `/schedule-disable`, `/schedule-run`, and `/schedule-delete` expose scheduled headless runs. Catalog reads consume every cursor page, updates carry the exact revision read, immediate firing reports its new session/run identities, and `schedules.changed` refreshes only the open schedule projection.
- `/memory [project|user]`, `/memory-add [project|user]`, and the remaining `memory-*` commands expose governed agent-maintained memory without conflating it with user-authored instructions; omitted scopes select the current project. Pending automatic facts retain their session/day provenance, approve/reject and delete require confirmation, and successful mutations re-read the selected scope.
- `/knowledge`, `/knowledge-read <cwd|projectRoot|home>`, and `/knowledge-edit <scope>` expose the human-authored FLAME.md cascade. The resize-safe editor accepts multiline text, preserves it verbatim, allows intentional clearing, and reads the saved scope back from the runtime. External edits refresh an open knowledge Reader through `knowledge.changed`.
...
| --- | --- | --- |
| Domain and ports | `internal/agent`, `internal/agentmemory`, `internal/authoringcontext`, `internal/changefeed`, `internal/diagnostictool`, `internal/feedback`, `internal/goal`, `internal/hookpolicy`, `internal/knowledge`, `internal/mcp`, `internal/modelconfig`, `internal/schedule`, `internal/sessiontransfer`, `internal/settings`, `internal/skills`, `internal/usage`, `internal/workspace` | Validated agent, context, diagnostics, hook-governance, feedback, runtime-management, invalidation, portable-session, and configuration contracts with consumer-owned narrow ports. |
...
- preserve schedule revisions, optional firing times, model/workspace selection, complete pagination, and headless run identities while leaving cron parsing and firing in the runtime;
- keep governed agent memory and human-authored FLAME.md knowledge as separate bounded contexts, preserve memory provenance/review state, and omit workspace context from runtime-global scopes;
- keep diagnostic JSON opaque and owned until the protocol edge, reject non-read-only direct tools, preserve semantic-index lifecycle and scored spans, and confine every direct invocation to the admitted workspace;
- project agent documents, recipe templates, lifecycle hooks, project trust, and feedback into separate consumer models; recipe expansion and hook decisions reuse existing prompt and confirmation lifecycles rather than creating parallel execution paths;
...
1. command flags;
2. `FLAME_` environment variables;
3. one selected YAML file: `--config`, otherwise `./.flame.yaml` when present, otherwise the user configuration file;
4. product defaults.

The user file is `flame/config.yaml` under the operating system's user configuration directory. Nested environment keys use underscores, for example `FLAME_UI_TRANSCRIPT_RETAIN=80` and `FLAME_APPROVAL_REMEMBER=project`.

This CLI-settings file is separate from the embedded runtime's `$FLAME_HOME/runtime/config.yaml`. The built-in CLI selection is `deepseek` plus `deepseek-v4-flash`, matching the repository runtime config. The paired `FLAME_PROVIDER` and `FLAME_MODEL` variables override it; DeepSeek credentials may still be supplied through `DEEPSEEK_API_KEY`. `FLAME_HOME` and `FLAME_RUNTIME_CONFIG_DIR`, when set, must be absolute.
...
Run `flame config show` to inspect the merged, validated value and `flame config path` to identify the selected file.
...
Each configured plugin directory, or one of its immediate child directories, may contain `flame-plugin.json` and an executable entry. The manifest schema version is `2`; the executable host API version is `1`.
...
The host caps each output stream at 1 MiB, request JSON at 128 KiB, arguments at 64 KiB, and response messages at 4 KiB. It passes only a small platform-safe environment allowlist plus `FLAME_PLUGIN_PROTOCOL`, `FLAME_PLUGIN_ID`, and `FLAME_PLUGIN_COMMAND`; unrelated secrets are not inherited. Cancellation terminates the owned process tree on supported platforms.
