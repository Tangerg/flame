	...
	for _, contributed := range a.registry.OwnedValues(SlashCommands) {
		...
		if command.Available != nil {
			evaluate = func(host *app) CommandAvailability {
				request := CommandRequest{Workspace: host.session.current.Workspace.Ref.Path, SessionID: host.session.current.ID}
...
func (a *app) executeCommand(pluginID string, command SlashCommand, argument string) {
	...
	request := CommandRequest{Argument: argument, Workspace: a.session.current.Workspace.Ref.Path, SessionID: a.session.current.ID}
