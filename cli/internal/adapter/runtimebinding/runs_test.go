...
func TestSubscribeRunRejectsInvalidReplayCursorBeforeBinding(t *testing.T) {
	t.Parallel()
	called := false
	runtime := &Connection{runs: runBindingStub{subscribe: func(context.Context, protocol.SubscribeRunRequest, flameruntime.RunSubscriptionOptions) (*protocol.SubscribeRunResponse, iter.Seq2[protocol.RunEvent, error], error) {
		called = true
		return nil, nil, nil
...
func TestRunAdaptersRejectMismatchedAcknowledgements(t *testing.T) {
	...
	runtime := &Connection{runs: runBindingStub{
		resume: func(context.Context, protocol.ResumeRunRequest, flameruntime.RunCommandOptions) (*protocol.ResumeRunResponse, iter.Seq2[protocol.RunEvent, error], error) {
