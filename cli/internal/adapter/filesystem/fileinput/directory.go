...
// OpenDirectoryAt opens name beneath root only when it remains the same
// directory observed before the non-blocking open.
func OpenDirectoryAt(root *os.Root, name string) (_ *os.File, _ os.FileInfo, err error) {
	if root == nil {
	...
	directory, err := root.OpenFile(name, readOnlyFlags, 0)
	if err != nil {
		return nil, nil, err
	}
	defer func() {
