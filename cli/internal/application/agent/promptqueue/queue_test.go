...
func TestEntryIdentityRejectsZeroAndAllocationOverflow(t *testing.T) {
	...
	if _, err := enqueueTestMessage(queue, "session", agent.Message{Text: "must not wrap"}); err == nil {
...
func TestQueueKeepsSessionQueuesIsolatedAndSnapshotsDetached(t *testing.T) {
	queue := New()
	first, err := enqueueTestMessage(queue, "one", agent.Message{Text: "first"})
	if err != nil {
		t.Fatal(err)
	}
	if _, enqueueErr := enqueueTestMessage(queue, "two", agent.Message{Text: "other session"}); enqueueErr != nil {
		t.Fatal(enqueueErr)
	}
	second, err := enqueueTestMessage(queue, "one", agent.Message{Text: "second"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueueUpdatesMovesRemovesAndClearsByStableIdentity(t *testing.T) {
	queue := New()
	first, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "first"})
	second, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "second"})
	third, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "third"})
...
func TestQueueRejectsInvalidMessagesWithoutMutation(t *testing.T) {
	queue := New()
	if _, err := enqueueTestMessage(queue, "", agent.Message{Text: "valid"}); !errors.Is(err, ErrSessionIDRequired) {
		t.Fatalf("empty session returned %v", err)
	}
	if _, err := enqueueTestMessage(queue, "session", agent.Message{}); err == nil {
...
func TestQueueRejectsInvalidRunOptionsWithoutMutation(t *testing.T) {
	queue := New()
	existing, err := enqueueTestMessage(queue, "session", agent.Message{Text: "existing"})
	if err != nil {
		t.Fatal(err)
	}
	if _, ok := queue.BeginDispatch("session"); !ok {
...
func TestQueueRestoresAnExactSnapshotAfterARejectedTransaction(t *testing.T) {
	...
	next, err := enqueueTestMessage(queue, "session", agent.Message{Text: "third"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueueRestoresDurableCommandsWithFreshLocalIdentities(t *testing.T) {
	queue := New()
	if _, err := enqueueTestMessage(queue, "other", agent.Message{Text: "advance local identity"}); err != nil {
		t.Fatal(err)
	}
...
func TestQueueRejectsAnInvalidDurableDispatchWithoutMutation(t *testing.T) {
	queue := New()
	existing, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "existing"})
	before := queue.State("session")
	commands := []agent.StartRun{
		{CommandID: agent.CommandID("cli_11111111111111111111111111111111"), SessionID: "session", Message: agent.Message{Text: "first"}, Options: agent.RunOptions{Limits: agent.UnlimitedRunLimits()}},
		{CommandID: agent.CommandID("cli_22222222222222222222222222222222"), SessionID: "session", Message: agent.Message{Text: "second"}, Options: agent.RunOptions{Limits: agent.UnlimitedRunLimits()}},
...
func TestDispatchReservationProtectsRuntimeIdentityFromPriorityEdits(t *testing.T) {
	queue := New()
	first, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "opening"})
	second, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "send next"})
	third, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "leave last"})
...
func TestRestoreStatePreservesDispatchReservation(t *testing.T) {
	queue := New()
	first, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "opening"})
	second, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "queued"})
	if _, ok := queue.BeginDispatch("session"); !ok {
...
func TestRejectedDispatchIsReidentifiedAndReleasedAtomically(t *testing.T) {
	queue := New()
	first, _ := enqueueTestMessage(queue, "session", agent.Message{Text: "retry me"})
	if _, ok := queue.BeginDispatch("session"); !ok {
...
func enqueueTestMessage(queue *Queue, sessionID string, message agent.Message) (Entry, error) {
