...
func TestAgentMemoryAndKnowledgeReadersShowScopeAndProvenance(t *testing.T) {
	...
	host, stop := runUIWithRuntimeServices(t, Config{Services: backendcontract.Services{Agent: runtimefixture.New(), AgentMemory: memory, Knowledge: knowledgeStore}, Workspace: "/workspace"})
...
func TestAgentMemoryMultilineAddSurvivesResize(t *testing.T) {
	memory := newAgentMemoryServiceStub()
	host, stop := runUIWithRuntimeServices(t, Config{Services: backendcontract.Services{Agent: runtimefixture.New(), AgentMemory: memory}, Workspace: "/workspace"})
	host.Shows(t, "Ask flame")
...
func TestAgentMemoryUpdateDoesNotInstallAReaderAfterSessionSwitch(t *testing.T) {
	...
	release := sync.OnceFunc(func() { close(memory.release) })
	t.Cleanup(release)
	host, stop := runUIWithRuntimeServices(t, Config{Services: backendcontract.Services{Agent: runtimefixture.New(), AgentMemory: memory}, SessionID: "ses_demo_1"})
	host.Shows(t, "Ask flame")
...
func TestKnowledgeReadUsesTheRequestedScope(t *testing.T) {
	knowledgeStore := newKnowledgeServiceStub()
	host, stop := runUIWithRuntimeServices(t, Config{Services: backendcontract.Services{Agent: runtimefixture.New(), Knowledge: knowledgeStore}, Workspace: "/workspace"})
	host.Shows(t, "Ask flame")
...
func TestKnowledgeChangeConvergesTheExactOpenScope(t *testing.T) {
	...
	host, stop := runUIWithRuntimeServices(t, Config{Services: backendcontract.Services{Agent: runtimefixture.New(), Knowledge: knowledgeStore, Changes: source}, Workspace: "/workspace"})
	host.Shows(t, "Ask flame")
