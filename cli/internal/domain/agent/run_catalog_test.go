...
func TestRunPageValidatesItemsIndependentlyOfTreePageBoundaries(t *testing.T) {
	...
	page := RunPage{Items: []Run{{
		...
		Lineage: lineage,
		Status:  protocol.RunStatusWaiting,
	}}}
...
func TestRunCancellationClosesRootAndChildResults(t *testing.T) {
	...
	rootCanceled := Run{
		...
		Outcome: Outcome{Status: protocol.OutcomeCanceled},
	}
	...
	childCanceled := Run{
		...
		Status:  protocol.RunStatusFinished, Outcome: Outcome{Status: protocol.OutcomeCanceled},
	}
