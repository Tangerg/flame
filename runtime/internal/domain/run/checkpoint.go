...
func NewCheckpoint(state CheckpointState) (Checkpoint, error) {
	...
	state.ToolResultIDs = slices.Clone(state.ToolResultIDs)
...
func (e Checkpoint) ToolResultIDs() []toolresult.ID { return slices.Clone(e.state.ToolResultIDs) }
...
func (e Checkpoint) State() CheckpointState {
	...
	state := e.state
	state.ToolResultIDs = e.ToolResultIDs()
...
func (e Checkpoint) ValidateSuccessor(next Checkpoint) error {
	...
	if left.RootMemberID != right.RootMemberID || left.BuildID != right.BuildID || left.Scope != right.Scope ||
		!left.ModelSelection.Equal(right.ModelSelection) || !left.Capabilities.Equal(right.Capabilities) {
...
func (e Checkpoint) Equal(other Checkpoint) bool {
	...
	return left.RootMemberID == right.RootMemberID && left.BuildID == right.BuildID && left.Scope == right.Scope &&
		left.ModelSelection.Equal(right.ModelSelection) && left.Capabilities.Equal(right.Capabilities) &&
		slices.Equal(left.ToolResultIDs, right.ToolResultIDs) && bytes.Equal(left.Payload, right.Payload) && slices.Equal(left.Usage.Models, right.Usage.Models)
}

func (e Checkpoint) validateInitialState() error {
	if err := toolresult.ValidateReferences(e.state.ToolResultIDs); err != nil {
