...
// actually summarized durable context. Skill workers are absent only when the
// user Skill library is unconfigured.
...
func NewPipeline(consolidator *MemoryConsolidator, skillMiner *SkillProposalMiner, skillArchiver *IdleSkillArchiver) (*Pipeline, error) {
	if consolidator == nil {
		return nil, errors.New("maintenance: memory consolidator is required")
	}
	return &Pipeline{
