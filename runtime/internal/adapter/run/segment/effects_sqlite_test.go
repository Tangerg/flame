...
func TestCommitEventRecordsGoalRunWithTerminalRun(t *testing.T) {
	...
	unwritten, err := goal.Unwritten(g.SessionID())
	if err != nil {
		t.Fatal(err)
	}
	goalReplacement, err := goal.NewReplacement(unwritten.Version(), g)
	if err != nil {
		t.Fatalf("prepare Goal replacement: %v", err)
	}
	if saved, saveErr := goals.Save(ctx, goalReplacement); saveErr != nil || !saved {
...
func TestCommitWaitingSubtreeCancellationReconcilesAmbiguousCommit(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			otherDraft.commitID = testCommitID("run_commit_other_waiting_cancellation")
			other := mustWaitingCancellationCommit(t, otherDraft)
...
func waitingCancellationDraft(commit runs.WaitingSubtreeCancellationCommit) waitingCancellationCommitDraft {
	...
	if resume, ok := commit.Resume(); ok {
		draft.resume = &resume
	}
	return draft
}

func (d waitingCancellationCommitDraft) build() (runs.WaitingSubtreeCancellationCommit, error) {
	switch {
	...
	default:
		return runs.WaitingSubtreeCancellationCommit{}, errors.New(
			"waiting cancellation fixture must choose exactly one disposition",
		)
	}
}

func mustWaitingCancellationCommit(
	t testing.TB,
	draft waitingCancellationCommitDraft,
) runs.WaitingSubtreeCancellationCommit {
	t.Helper()
	commit, err := draft.build()
	if err != nil {
		t.Fatalf("waiting cancellation fixture: %v", err)
	}
	return commit
}
...
func TestCommitEventReconcilesAmbiguousAuthoritativeCommit(t *testing.T) {
	...
	matched, err = state.RunCommitCommitted(
		ctx, draft.SessionID, draft.RunID, draft.SegmentID, testCommitID("run_commit_event_authoritative_later"),
	)
	if err != nil || !matched {
