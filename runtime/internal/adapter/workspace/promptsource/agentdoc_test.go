...
func TestDiscoverAgentDocsCanonicalizesAndDedupesSymlinkedUserSource(t *testing.T) {
	...
	root := t.TempDir()
	mkGitDir(t, root)
	writeFile(t, filepath.Join(home, ".flame", "AGENTS.md"), "shared-user-source")
...
func TestDiscoverAgentDocsPicksFlameSubdirBeforePlainFile(t *testing.T) {
	root := t.TempDir()
	mkGitDir(t, root)
	writeFile(t, filepath.Join(root, ".flame", "AGENTS.md"), "flame-subdir")
	...
	if files[0].Content != "flame-subdir" {
		t.Fatalf("files[0] = %+v (want flame-subdir first)", files[0])
	}
...
func TestDiscoverAgentDocsUserScope(t *testing.T) {
	...
	root := t.TempDir()
	mkGitDir(t, root)
	writeFile(t, filepath.Join(home, ".flame", "AGENTS.md"), "user-prefs")
...
func TestDiscoverAgentDocsRejectsBrokenHigherPrecedenceSource(t *testing.T) {
	root := t.TempDir()
	mkGitDir(t, root)
	if err := os.MkdirAll(filepath.Join(root, ".flame"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("missing-agent-document", filepath.Join(root, ".flame", "AGENTS.md")); err != nil {
		t.Skipf("symlinks unavailable: %v", err)
	}
