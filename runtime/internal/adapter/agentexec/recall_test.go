...
func TestRecalledMemoriesSkipsPinnedAndInjectsRest(t *testing.T) {
	...
	composer := mustWorkingContextComposer(t, WorkingContextConfig{AgentMemorySearch: search})
...
func TestRecalledMemoriesEmptyCases(t *testing.T) {
	composer := mustWorkingContextComposer(t, WorkingContextConfig{AgentMemorySearch: &fakeAgentMemorySearcher{}})
	...
	composer = mustWorkingContextComposer(t, WorkingContextConfig{AgentMemorySearch: allPinned})
	if _, ok, _ := composer.recallMessage(context.Background(), "/repo", "q"); ok {
