...
// parseMCPServerValue dispatches by prefix. `stdio:` is a Flame convention —
...
// FLAME_MCP_<NAME>_TOKEN (name upper-cased, non-alphanumerics → '_') and returns
// it as an "Authorization: Bearer <token>" header value, or "" when unset. It
// authenticates Flame to an access-controlled MCP server. The token lives in its
// own env var (not the FLAME_MCP_SERVERS list) so the secret stays separate from
