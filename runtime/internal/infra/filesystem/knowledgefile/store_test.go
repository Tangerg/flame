...
func TestStoreRejectsOversizedKnowledgeDocuments(t *testing.T) {
	...
	t.Run("external file", func(t *testing.T) {
		home := t.TempDir()
		path := filepath.Join(home, "FLAME.md")
...
func TestStoreRejectsKnowledgeSymlinkOutsideScope(t *testing.T) {
	...
	if err := os.Symlink(outside, filepath.Join(home, "FLAME.md")); err != nil {
...
func TestStoreUsesInScopeSymlinkPhysicalIdentityAndPreservesMode(t *testing.T) {
	...
	alias := filepath.Join(home, "FLAME.md")
...
func TestStoreUsesPrivateModeForNewHomeAndReadableModeForNewWorkspace(t *testing.T) {
	...
	for _, scope := range []struct {
		...
		{value: knowledge.ScopeHome, mode: 0o600, path: filepath.Join(home, "FLAME.md")},
		{value: knowledge.ScopeCWD, dir: workspace, mode: 0o644, path: filepath.Join(workspace, "FLAME.md")},
	} {
...
func TestStoreConcurrentUpdatesRejectStaleRevisionsWithoutTornWrites(t *testing.T) {
	...
	legacyTemporary := filepath.Join(home, "FLAME.md.tmp")
...
func TestStoreCrossProcessUpdatesHaveOneCASWinner(t *testing.T) {
	if os.Getenv("FLAME_TEST_KNOWLEDGE_CAS_CHILD") == "1" {
		store := newKnowledgeStore(t, os.Getenv("FLAME_TEST_KNOWLEDGE_HOME"), t.TempDir())
		ready := os.Getenv("FLAME_TEST_KNOWLEDGE_READY")
		start := os.Getenv("FLAME_TEST_KNOWLEDGE_START")
	...
	for index := range children {
		...
		children[index].command.Env = append(os.Environ(),
			"FLAME_TEST_KNOWLEDGE_CAS_CHILD=1",
			"FLAME_TEST_KNOWLEDGE_HOME="+home,
			"FLAME_TEST_KNOWLEDGE_READY="+ready,
			"FLAME_TEST_KNOWLEDGE_START="+start,
			"FLAME_TEST_KNOWLEDGE_REVISION="+fresh.Revision,
			fmt.Sprintf("FLAME_TEST_KNOWLEDGE_BODY=writer-%02d", index),
		)
...
func TestStoreRecoversAfterWriterProcessDiesDuringStaging(t *testing.T) {
	if os.Getenv("FLAME_TEST_KNOWLEDGE_CRASH_CHILD") == "1" {
		store := newKnowledgeStore(t, os.Getenv("FLAME_TEST_KNOWLEDGE_HOME"), t.TempDir())
	...
	command.Env = append(os.Environ(),
		"FLAME_TEST_KNOWLEDGE_CRASH_CHILD=1",
		"FLAME_TEST_KNOWLEDGE_HOME="+home,
		"FLAME_TEST_KNOWLEDGE_READY="+ready,
		"FLAME_TEST_KNOWLEDGE_REVISION="+written.Revision,
	)
	...
	for staged == "" && time.Now().Before(deadline) {
		...
		for _, entry := range entries {
			if strings.HasPrefix(entry.Name(), ".FLAME.md.flame-stage-") {
...
func TestStoreRecoveryScansLargeDirectoriesWithoutLeavingOrphans(t *testing.T) {
	...
	for index := range 130 {
		...
		path := filepath.Join(home, ".FLAME.md.flame-stage-"+suffix)
...
func TestStoreListIncludesEmptyAddressableScopes(t *testing.T) {
	...
	if entries[0].UpdatedAt.IsZero() {
		t.Error("UpdatedAt is zero; want the FLAME.md file mtime")
	}
}
...
func TestStoreListPreservesDistinctCascadeScopes(t *testing.T) {
	...
	for index, want := range []struct {
		...
		{knowledge.ScopeHome, "home knowledge", filepath.Join(home, "FLAME.md")},
		{knowledge.ScopeProjectRoot, "project knowledge", filepath.Join(projectRoot, "FLAME.md")},
		{knowledge.ScopeCWD, "workspace knowledge", filepath.Join(cwd, "FLAME.md")},
	} {
		got := entries[index]
