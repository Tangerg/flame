	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err := sqlite.NewModelInvocationStore(database).CompleteModelInvocation(
				ctx, draft.SessionID, draft.RunID, draft.SegmentID, "model_call_pending", startedAt, startedAt.Add(time.Second), nil, nil,
			); err != nil {
...
func TestEventCommitMarkerDoesNotCrossSuspendResumeGeneration(t *testing.T) {
	...
	if err := store.RecordRunCommit(
		ctx, "ses_A", "run_1", "seg_open", testCommitID("run_commit_event_old"),
	); err != nil {
	...
	if recordRunCommitErr := store.RecordRunCommit(
		ctx, "ses_A", "run_1", "seg_next", testCommitID("run_commit_event_next"),
