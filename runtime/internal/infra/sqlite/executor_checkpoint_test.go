...
func storedExecutorCheckpoint(rootMemberID, sessionID, payload string) run.Checkpoint {
...
func TestExecutorCheckpointStoreReplacesOneRootOwnedAggregate(t *testing.T) {
	...
	replacementState.Usage.Models[0].Calls = 2
	replacement = testsupport.MustCheckpoint(replacementState)
...
func TestExecutorCheckpointStoreRejectsImmutablePolicyReplacement(t *testing.T) {
	...
	mutations := []struct {
		...
		{name: "limits", mutate: func(checkpoint *run.CheckpointState) {
	...
	for _, mutation := range mutations {
		t.Run(mutation.name, func(t *testing.T) {
			replacement := first
			replacementState := replacement.State()
			replacementState.Payload = []byte(`{"tree":"replacement"}`)
			replacement = testsupport.MustCheckpoint(replacementState)
			checkpointState := replacement.State()
			mutation.mutate(&checkpointState)
			replacement = testsupport.MustCheckpoint(checkpointState)
			if err := store.SaveCheckpoint(t.Context(), replacement); !errors.Is(err, run.ErrInvalidCheckpoint) {
...
func TestExecutorCheckpointStoreRejectsCumulativeUsageRegression(t *testing.T) {
	...
	firstState.Usage.Models[0].Calls = 2
	first = testsupport.MustCheckpoint(firstState)
	if err := store.SaveCheckpoint(t.Context(), first); err != nil {
		t.Fatalf("SaveCheckpoint(first): %v", err)
	}
	replacement := first
	replacementState := replacement.State()
	replacementState.Payload = []byte(`{"tree":"stale"}`)
	replacementState.Usage.Models[0].Calls = 1
	replacement = testsupport.MustCheckpoint(replacementState)
	if err := store.SaveCheckpoint(t.Context(), replacement); !errors.Is(err, run.ErrInvalidCheckpoint) {
...
func TestExecutorCheckpointStoreRoundTripsDomainEnvelope(t *testing.T) {
	_, store := newExecutorCheckpointStorage(t)
...
func TestExecutorCheckpointStorePersistsUnlimitedPolicyWithoutCaps(t *testing.T) {
	...
	wantState.Limits = run.UnlimitedLimits()
	want = testsupport.MustCheckpoint(wantState)
	if err := store.SaveCheckpoint(t.Context(), want); err != nil {
		t.Fatalf("SaveCheckpoint: %v", err)
	}
	got, err := store.LoadCheckpoint(t.Context(), want.RootMemberID())
...
func TestExecutorCheckpointStoreRejectsZeroBeforeMutation(t *testing.T) {
	db, store := newExecutorCheckpointStorage(t)
	if err := store.SaveCheckpoint(t.Context(), run.Checkpoint{}); !errors.Is(err, run.ErrInvalidCheckpoint) {
		t.Fatalf("SaveCheckpoint: %v", err)
	}
	var count int
	if err := db.QueryRowContext(t.Context(), `SELECT count(*) FROM executor_checkpoints`).Scan(&count); err != nil || count != 0 {
...
func TestExecutorCheckpointStoreDeletesExactAggregates(t *testing.T) {
	_, store := newExecutorCheckpointStorage(t)
	ctx := t.Context()
	for _, checkpoint := range []run.Checkpoint{
...
func TestExecutorCheckpointStoreDeletesBySessionOwnership(t *testing.T) {
	_, store := newExecutorCheckpointStorage(t)
	...
	for _, rootID := range []string{"drop-session", "drop-unowned"} {
		if _, err := store.LoadCheckpoint(ctx, rootID); !errors.Is(err, run.ErrCheckpointNotFound) {
