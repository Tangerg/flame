...
func TestCounterOwnsExactRangeAndNeverWraps(t *testing.T) {
	t.Parallel()
	...
	if err != nil {
		t.Fatalf("Restore(Maximum): %v", err)
	}
	if _, err := last.Next(); !errors.Is(err, ErrExhausted) {
		t.Fatalf("last.Next() error = %v, want ErrExhausted", err)
	}
	if _, err := Restore(Maximum + 1); !errors.Is(err, ErrOutOfRange) {
		t.Fatalf("Restore(Maximum+1) error = %v, want ErrOutOfRange", err)
	}
	...
	if err := Follows(Maximum, 0); !errors.Is(err, ErrExhausted) {
		t.Fatalf("Follows(Maximum, 0) error = %v, want ErrExhausted", err)
	}
	if err := Follows(7, 9); !errors.Is(err, ErrNotSuccessor) {
		t.Fatalf("Follows(7, 9) error = %v, want ErrNotSuccessor", err)
	}
}
