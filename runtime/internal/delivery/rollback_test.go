...
import (
	...
	appRuns "github.com/Tangerg/flame/runtime/internal/application/agent/runs"
...
func putTestSession(t *testing.T, rt *stubRuntime) {
	t.Helper()
...
func TestRollbackSession_Busy(t *testing.T) {
	...
	ctx := context.Background()
	sess, _ := insertSessionFixture(ctx, rt.sess, "s", "/w")
	_, _, _ = rt.admissions.AcquireSession(t.Context(), sess.ID()) // simulate a run in flight (admission slot held)
...
func TestPersistRunCarriesCreatedAt(t *testing.T) {
	...
	if err := rt.runs.Admit(ctx, testsupport.RunDraft(run.Draft{SegmentID: "seg_open", RunID: "run_1", SessionID: sess.ID(), CreatedAt: started})); err != nil {
...
func TestAcquireSession(t *testing.T) {
	_, rt := rollbackHarness(t)
	releaseS1, ok, _ := rt.admissions.AcquireSession(t.Context(), "s1")
	if !ok {
		t.Fatal("first claim must succeed")
	}
	if _, ok, _ := rt.admissions.AcquireSession(t.Context(), "s1"); ok {
		t.Fatal("second claim on the same session must fail while the first is outstanding")
	}
	if _, ok, _ := rt.admissions.AcquireSession(t.Context(), "s2"); !ok {
