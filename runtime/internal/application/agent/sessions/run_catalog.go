...
func validateRunCatalog(values []run.Run, sessionID string) error {
	...
	for index, value := range values {
		if value.IsZero() {
			return fmt.Errorf("sessions: Run store row %d is unconstructed", index+1)
		}
		if sessionID != "" && value.SessionID() != sessionID {
			return fmt.Errorf("sessions: Run store row %d belongs to Session %q, want %q", index+1, value.SessionID(), sessionID)
		}
