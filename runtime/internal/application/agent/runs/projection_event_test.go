...
func TestProjectRejectsMalformedItemChangeBeforePublication(t *testing.T) {
	...
	_, err := newReducer(testReducerConfig()).project([]ProjectionEvent{ItemChanged{ItemID: "item_1"}})
	if !errors.Is(err, errReducerInvariant) {
		t.Fatalf("project error = %v, want reducer invariant violation", err)
	}
}

func TestProgressRejectsImpossiblePreviews(t *testing.T) {
	t.Parallel()
	...
	for _, test := range []struct {
		...
		{name: "empty"},
		{name: "negative step", progress: Progress{Step: &negativeStep}},
		{name: "invalid usage", progress: Progress{Usage: &accounting.Usage{
			Total: accounting.Totals{CostUSD: &invalidCost},
		}}},
		{name: "negative context", progress: Progress{ContextTokens: &negativeContext}},
		{name: "noncanonical activity", progress: Progress{Activity: " calling model "}},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
	...
	valid := Progress{Step: &zeroStep, ContextTokens: &zeroContext, Activity: "Calling model"}
...
func TestProjectRejectsEveryMalformedEventVariantBeforePublication(t *testing.T) {
	...
	for _, event := range []ProjectionEvent{
		SegmentStarted{},
		SegmentProgressed{},
		SegmentFinished{},
		ItemStarted{},
