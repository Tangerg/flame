...
func TestCoordinatorHoldsSessionAdmissionThroughTerminalMaintenance(t *testing.T) {
	...
	if _, ok, _ := coordinator.admission.AcquireSession(t.Context(), "ses_1"); ok {
...
func (f *fakeEffects) CommitExecutionTree(ctx context.Context, update ExecutionTreeUpdate, commits []EventCommit) error {
	for _, commit := range commits {
		if err := f.CommitEvent(ctx, commit); err != nil {
			return err
		}
	}
	return nil
