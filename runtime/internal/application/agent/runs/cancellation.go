...
func (c *Coordinator) commitWaitingChildCancellation(
	...
	durable, err := transformation.durableCommit(plan.pending, newRunCommitID())
	if err != nil {
		return CancelResult{}, err
	}
...
func (c *Coordinator) resumeAfterWaitingChildCancellation(
	...
	events, err := c.openSegment(ctx, segmentSpec{
		...
		CommitOpening: func(commitCtx context.Context, opening OpeningCommit) error {
			...
			durable, durableErr := transformation.resumedDurableCommit(
				plan.pending, opening.CommitID(), resume, opening.Events(),
			)
			if durableErr != nil {
				return durableErr
			}
