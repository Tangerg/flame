...
> [`runtime/contract/schema.json`](../contract/schema.json)（所有 shape 与跨字段约束）、
...
Flame Runtime 是一个**本地、领域中立的 agent runtime**。客户端可以是 web UI、桌面外壳、TUI 或另一个本地进程。协议是
...
- OpenRPC 的 by-name `params[]` 从完整 request frame 的属性直接投影；不得重新按字段 Go 类型生成一个更宽的副本。
  因而 `minLength` / `minimum` / `maximum` 等约束在逐参数视图、`x-flame-requestFrame`、Go 与 TS validator 中完全一致。
...
- **分页类别不是方法名约定**：Registry 从 result 的完整 `data/nextCursor` shape 与 params 是否同时含有
  ...
  `pagination:"cursor"`；只出现半套字段时启动即失败。manifest、OpenRPC 的 `x-flame-pagination` 与 SDK 都消费这一个
  事实，不维护第二张方法表。
...
- 终段位置保证会把该段最后一次 Plan 以新的 `eventId` 再发布一次。因此正常订阅者可以收到两份 **revision 与内容都
	相同**的 `plan.updated`；客户端必须按这对业务事实幂等折叠。相同 revision 却内容不同是协议冲突，更小 revision
	只有在已被 cold read 水位覆盖时才是可忽略的恢复重叠，不能普遍吞掉倒序事件。
...
`knowledge.*` 专指人工维护的 `FLAME.md` 级联：`knowledge.list` 列出 workspace 可见且可寻址的条目，包括尚未创建的空文档；
