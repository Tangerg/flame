# Flame Runtime Domain 模型精修设计

> 状态：已接受专项设计；实施授权与完成事实由 [`EXECUTION_PLAN.md`](EXECUTION_PLAN.md) 和 [`CAPABILITY_LEDGER.md`](CAPABILITY_LEDGER.md) 唯一记录
>
> 适用范围：`runtime/internal/domain` 及为归还领域行为所有权所必需的 Application、Adapter、Persistence、Delivery 服务端爆炸半径
...
| `knowledge` | 人类维护的 FLAME.md scope/document value；与 AgentMemory 的所有权和写入来源不同 |
