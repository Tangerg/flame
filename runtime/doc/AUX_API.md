# Flame Runtime Protocol · 旁路 API（定稿 `2026-08-30`）

> **状态：正式契约（canonical）。** 本文是 [`API.md`](./API.md) 的配套契约，定义 Flame Runtime 的**旁路面**——不经 LLM 的
...
> 本文写语义与不变量，不重述字段表。
>
> 文内裸 `§x` 指**本文**小节；引 `API.md` 一律写全 `API.md §x.y`。`protocolVersion`：**`2026-08-30`**（与 `API.md` 同）。
...
- **`watches` 是 workspace 文件身份的附加维度**（`features.fileWatch`）：每个 watch 必带客户端命名的 `watchId` 与显式
  ...
  Runtime 精确观察对应 workspace/projectRoot 的 `FLAME.md` 与 hooks cascade。无 watch 的对应 topic 仍观察 Runtime 全局
...
- **配置与后台任务有专用信号**：成功的 Skill library/proposal、`knowledge.update` / `hooks.setTrust`、provider/role、approval policy、agent-memory
  ...
  `FLAME.md` / `.flame/hooks.json` 也发对应信号。客户端只重读所属资源；事件不携带业务值，也不把
...
同一份 `SessionArtifact`（**version 27**）的两端：Session exact bounded provider/model/reasoning selection + 每条终态 Run 的 frozen exact selection + 完整 Item 历史 + chat 消息 + offload 的工具正文 +
...
- ToolCall 的 `startedAt` / `finishedAt` 保存可见 lifecycle；可选 `durationMillis` 保存精确 Tool execution，排除审批等
  执行前等待。无法证明 execution interval 的终态保持 duration 缺省，导入不按 lifecycle 差值猜测。
...
- **规则的 KEY = 工具名 + 该次调用的 per-tool `subject`**：`shell` 取 `command`，`read` 取 `path`；`apply_patch`
  可能同时修改多个路径，故与其余工具一样使用空 subject（= 整个工具，任意参数）。subject 由**后端从被批准的调用参数中提取**，
