...
- Instance 整体 shutdown 同样由唯一 active generation 连续拥有：必须先退役 Delivery Endpoint，再 join scheduler/database/recovery workers，最后 join Host owner attempt。Handler 只由 Endpoint 退役，Bootstrap 不保存第二套 service closer。CLI/public Close caller timeout 不能把推进义务退回外层；Instance 不得复制 Runtime context，只能从 Host 已注入 lifetime 派生 values-preserving owner context；
...
- 同时进入管理面与 prompt 的 human-authored complete document 必须由所属 Domain 定义单文档包络，并在 Application command admission 与 filesystem adapter read/write 共同执行。Knowledge `FLAME.md` 最多 1 MiB；读取必须以 stat + cancellation-aware `limit+1` 同时覆盖已超限文件和读取期间增长，cascade 不得静默跳过或截断越界文档，transport body limit 不能替代领域所有权。原子写入的 orphan-stage recovery 必须以正数固定批次流式扫描并逐批检查 context；禁止 `os.ReadDir` 或 non-positive `File.ReadDir` 在资源边界前完整物化用户目录。
- 没有独立 Domain 的 authored prompt source 由消费 Application 直接拥有合同，不为常量新建 facade。AGENTS.md/Recipe 每份最多 1 MiB 且为有效 UTF-8；Agent document cascade 最多 64 份/4 MiB，Recipe 每 scope 最多 128 份、完整级联最多 256 份/8 MiB。Filesystem 必须在 parse/string 前 stat + cancellation-aware `limit+1`，目录枚举也必须有 sentinel；missing/empty 可以缺席，现存 invalid source 必须让管理面与执行面共同 fail closed。模型预算只能删除装不下的较低优先级完整文档，任一单文档不能被截断或静默丢弃。
- 外部 VCS 读取必须同时限制 process bytes、complete-list cardinality 与 consumer projection，不能只在完成解析后切 slice。Git stdout 最多 64 MiB，stderr 以 64 KiB bounded-drain 保持 pipe 流动；pager、external diff 与 textconv 必须关闭，后台 observation 还必须有独立 timeout。Workspace changes 最多 10,000 项，Git-backed file catalog 最多 20,000 paths；untracked regular file 只允许 streaming stat，symlink 必须按 link 自身的 Git 语义处理，不得跟随 workspace 外 referent。Run-boundary Checkpoint 不得再构造 `exec.Cmd`/`bytes.Buffer`；它复用同一 Git process owner，并在 index mutation 前拒绝超过 20,000 paths、512 MiB current material、64 KiB source alternates 或 64 MiB source index 的完整 selection。`ls-files -z` 必须按原始 NUL records 解析，不能 trim 或按行猜测路径。
- Structured diff 同时服从 5,000 complete files、默认/最高 5,000 rows 与 64 MiB material。预算必须在进入/保留下一个文件前检查，第一文件、binary/zero-row 文件和 direct Application port 都不能绕过；无法完整容纳一个文件时只返回此前完整文件并诚实标记 truncation。Raw diff 超界与 complete changes 超界必须明确失败，不得返回 partial patch、截断 catalog 或转入 filesystem fallback；Git C-quoted/binary path 在严格化后仍须无损恢复。
- Read-before-mutation fingerprint 必须以 caller-cancellable streaming digest 观察完整 regular file，不得为 stamp 额外 materialize 全文件，也不得在 inspect/read failure 时跳过 guard。资源删除必须同步撤销 stamp；只有本 session 已知的成功创建/修改才能刷新 stamp，外部同内容重建不能继承旧资源授权。
- Auto-format 是有副作用的完整值变换，必须在读入前固定 input 上限，并在 formatter 成功、完整 output 通过同等上限后才原子替换目标。Go/JSON 优先用进程内标准库；第三方 formatter 只能通过 bounded stdin/stdout 工作，不得用 in-place flag 抢先提交。诊断 writer 必须继续 drain 且有限保留；禁止嵌入提供 `ReadFrom` 的 buffer 使 `io.Copy` 绕过自定义 `Write`，caller cancellation 与 `WaitDelay` 仍是必需进程所有权。
- Model/direct file read 的 input bytes、单行 bytes 与一次 result bytes 必须由 Runtime consumer 固定拥有；通用 executor 的 zero/unlimited 语义和最终 Tool-result offload 都不能代替准入。分页只返回完整行并让 `EndLine` 指向实际返回边界；`Truncated` 必须覆盖省略 prefix、suffix 与 byte budget。完整 UTF-8/NUL/line-count 扫描最多发生在已准入文件内，并在 64 KiB 级 buffer 上响应 caller cancellation。
- Read-before-mutation stamp 必须覆盖 Tool call 的完整线性区间，而不是在结果返回后单点观察。调用前后 full-file digest 只有在 generation 存在且相等时才能提交；读取期间替换、删除、超限或取消一律撤销 stamp。Read consumer 的 8 MiB admission 与 mutation guard 的 arbitrary-size bounded-memory digest 是两个明确语义，不能用后者让模型授权自己无法读取的文件。
- Public Workspace file read 必须由 Application 拥有 zero/default、caller maximum、complete-source 与 single-line 四层预算；不得把共享 filesystem executor 的 post-materialization `MaxBytes` 当成产品准入。编辑器结果可在有效 UTF-8 边界保留最后一行 prefix，但必须继续扫描已准入完整 source 以验证 NUL/UTF-8、计算诚实 `TotalLines` 并响应 caller cancellation；Application 必须复验 port output budget、text、window/content correspondence 与 omission marker，替代 adapter 不能绕过。超出文件的 range 明确失败。没有 truncation 字段的 head 只能返回完整请求窗口或错误。Desktop 若以 `path:line` 打开文件，必须请求目标附近窗口并消费服务端 `StartLine`，不能把默认前缀伪装成目标文件视图。
- Public Workspace search 必须在 Application acceptance boundary 编译并限制 regex 与 caller result count，再把 compiled plan 交给 port；不得把 raw query、unlimited positive limit 或共享 executor 的默认值下放。搜索目录、单文件、单行、aggregate scanned source 与 retained whole-row material 必须各有唯一上限；`Total` 必须在完整 admitted text corpus 上精确计数，不能从截断的 count output、`limit+1` 或客户端 overflow 猜测。Application 必须复验 direct port 的 material、root-relative path、行号、稳定排序、文本合法性与 query correspondence。Binary/invalid/oversized individual source 可以由合同排除，catalog/aggregate 超界则必须明确失败，不能发布伪完整 total。
- Model/direct `glob` 与 `grep` 必须复用 Runtime 的有限、ignore-aware Workspace catalog/text corpus，不得把通用 filesystem executor 的 `find/rg/grep` stdout、宿主 PATH 或最终 Tool-result offload 当作资源准入。两种工具都必须在 filesystem I/O 前限制 pattern/path/result count，把 path 收敛为 canonical root 内 identity，并在 bounded complete corpus 上发布 exact `total` 与诚实 `truncated`。模型 schema 只保留可组合的稳定语义：文件集合属于 glob，case flag 属于 regex，命中上下文属于 read；不得重新暴露 file type、multiline、context、output mode 或兼容字段来镜像某个搜索二进制。
- 同一 Skill source 进入非分页管理面和模型 progressive disclosure 时，必须由 Runtime consumer 共同限制 raw directory entries、valid candidates、完整 `SKILL.md` 与 bundled resource bytes；共享 SDK 的 complete `ReadDir`/`ReadFile`/`io.ReadAll` 不构成产品包络。用户托管库的 active+archived 总量、批准写入、idle sweep、完整 list 与 usage sidecar 必须共享容量真相；写事务不得制造下一次 strict reader 必然拒绝的状态。越界 source 整体失败，不截断、不分页伪装、不让 project/user/model/Desktop 各自选择不同前缀。
- 跨 Runtime/Desktop 的本地凭据文件必须由公共 deployment handoff package 唯一拥有 grammar、创建与读取；HTTP transport 和 Desktop 不得各自 `ReadFile`、trim 或重验。Token 必须是精确 43-byte canonical RawURL encoding of 32 random bytes，文件必须是 0600 regular file；reader 必须先做固定大小准入，再通过 opened-file identity 与常量大小 sentinel 拒绝 symlink、替换和增长。缺失与现存 invalid 必须区分，invalid 不得降级为空 token。
...
- 任何交给 `os/exec` pipe 的 bounded writer 都不得匿名嵌入 `bytes.Buffer` 或其他实现 `io.ReaderFrom` 的 storage；`io.Copy` 可以绕过自定义 `Write` 并直接走 promoted fast path。Sandbox command stdout/stderr 各最多保留 256 KiB、继续完整 drain 并发布 truncation marker；容量必须由 writer 方法集本身保证，不能靠调用方约定或事后 `Len` 检查。
- MCP stdio 的 `ClientSession`、session context 与 subprocess 必须由同一个 ledger 结算。Unix command 必须在 Start 前独占 process group，context cancellation 必须终止整组；正常 close、probe、dial failure、replacement、detach 与 Host shutdown 都必须在关闭协议连接后执行同一个 fallible cleanup，并把错误并入 one-shot teardown diagnostic。不得把 SDK 只等待/终止直接子进程的行为误当成完整后代所有权，也不得用无错误 `CancelFunc` 吞掉最终 cleanup failure。
- 同进程、一次性的 directory materialization 不得先构造 archive/blob/DTO 再立即解析；没有 durability、transport 或独立消费者时应在两个 capability-confined roots 之间直接流式复制。Sandbox working tree 固定使用 64 KiB chunk，并在写入前拥有 100,000 entries、128 MiB/file、512 MiB aggregate、opened identity/size、growth、symlink confinement 与 cancellation；destination 物理位于 source 内必须拒绝。资源上限必须约束读取和驻留过程，不能只在完整 `[]byte` 形成后检查长度。
- foreground/background 只能改变模型 Shell 的返回与寻址方式，不能改变进程资源 owner。Unix command 必须从 Start 前独占 process group；timeout、stop、foreground cancel、natural leader exit 与 Host shutdown 都必须 group-stop、join Wait 并保存 cleanup diagnostic。`WaitDelay` 只封顶 descendant-held pipe，不得把成功 leader 改写为失败，也不得替代 final group cleanup。任何 foreground entry 只有在 Done/cleanup 后才能从 ledger 删除；“本地单用户”不是允许 orphan descendant 的理由。
- 外部策略进程的 stdin 必须先经过 Domain-owned consumer projection，再经过 adapter-owned exact encoding envelope：prompt/result 等自由文本可用有效 UTF-8 bounded prefix，但必须携带 truncation marker；Tool arguments 属于 effect policy fact，禁止截断。Application 只为实际匹配的 command action 懒投影一次，Shell 必须在 marshal 前拒绝超过 512 KiB 的 raw material、marshal 后拒绝超过 512 KiB 的 JSON，并在任何 timer/process spawn 前失败；不得把上限散落到各 event producer，也不得让 declarative inject 支付 command projection 成本。
