...
from the private local `@flame/runtime-contract` package rooted at
`runtime/contract/typescript`; it does not vendor a second generated tree.
...
- `runtime/contract/typescript/` generated bindings, validators, canonical
  samples, and the handwritten JSON Schema check vocabulary;
- `desktop/frontend/src/rpc/` SDK, preflight, and schema tests;
- `desktop/frontend/src/plugins/builtin/runtime/` discovery and capability
  store tests;
- `desktop/frontend/visual/installVisualWorkspaceFixture.ts`;
- `desktop/frontend/CONTENT_RENDERING.md`.
