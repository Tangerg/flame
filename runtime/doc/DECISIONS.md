# Flame Runtime 架构决策记录

> 状态：持续维护
>
> 范围：`runtime` 重构的已接受决策；后续改变结论必须追加明确取代关系的 ADR，不能静默改写历史理由
...
- 决策：在现有 `runtime` 中保留成熟产品能力，对执行纵切面进行绿色重写。允许使用独立分支或 worktree 隔离开发，但最终只有一个 Runtime module/path。
...
- 背景：title、compaction、Agent Memory consolidation 与 Skill mining 共用 `auxiliary.Complete`，但旧 helper 没有设置 `chat.Options.MaxTokens`；Skill/Memory 还选择 uncapped tool results，compaction 虽限制单条结果却没有总 transcript 上限。失败反例用 128 条 8KiB tool result 得到约 1.05MiB 的普通维护输入与约 518KiB 的 compaction 输入，并证明 provider request 的 output token limit 为 nil。把更多 cap 分散到每个 worker 会继续保留可绕过的公共 helper 和第二套资源规则。
- 决策：`adapter/model/auxiliary.Prompt` 是唯一辅助请求包络，强制正数 `MaxInputBytes` / `MaxOutputTokens`、在 provider I/O 前校验 aggregate bytes，并唯一投影 `chat.Options.MaxTokens`。Session title 使用 20KiB/64-token envelope；Run maintenance 使用 512KiB hard input envelope，conversation transcript 在其中最多 384KiB、每条 message 最多 24KiB，按 message 公平保留 rune-safe head/tail；compaction 与 Skill 默认最多 4096 output tokens，Memory 默认最多 2048。删除 uncapped renderer 与 per-tool-result-only policy；本阶段先让compaction trigger不消费被裁剪的auxiliary transcript，P191再把其原始文本近似替换为完整主请求预算。
...
## ADR-RT-089：Workspace file API 必须拥有编辑器窗口与完整扫描包络

- 状态：已接受并实施，P170 完成；改变 Runtime internal Workspace Application/adapter/Delivery 语义与 Desktop file-view consumer，公共 Protocol shape、Artifact、SQLite、Agent Framework、`tools/fs` 与 CLI 不变。
- 背景：失败优先反例 `c2ef7dfe5` 证明 `workspace.files.read` 的 `maxBytes=0` 会完整返回 1.2 MiB 且声称未截断，caller 提交 16 MiB 后可返回 9 MiB，预取消 context 被忽略，invalid UTF-8 甚至作为成功 string 穿过 Delivery。根因是产品 API 直接复用 `tools/fs.LocalExecutor.Read`：先 `os.ReadFile` 全文件，之后才做可选 byte truncation；transport 与 Desktop 的 `truncated` 标签无法追回已经发生的分配，也没有稳定 MIME 错误。
- 决策：`application/workspace` 唯一规定默认 1 MiB、最高 8 MiB output，64 MiB complete-source 与 8 MiB single-line admission；zero 选择默认，过大 budget clamp，越界 source/line/range 明确失败。共享 scanner（P197 后由 `tools/textread` 单点拥有）让模型 read 与 Workspace read 共用 64 KiB cancellation-aware input validation、UTF-8/NUL、BOM/CRLF、trailing line、growth 和 total count；模型 consumer 保持 complete-line pagination，编辑器 consumer 可在有效 UTF-8 边界保留最后一行 prefix。Workspace adapter 对 direct input 复验，Application 再验证 port result 的 output bytes、text、window/content correspondence 与 truncation marker；Delivery 把 invalid text 映射为 `unsupported_mime`、资源/range 映射为 `invalid_params`。
- 决策：无 `truncated` 字段的 file head 默认 200、最高 400 行，output 被裁剪时整体失败。Desktop file view 不再把“打开 `path:line`”实现为默认读取文件前缀；目标存在时请求前后各 200 行，data provider 保留响应 `startLine`，gutter/highlight/scroll 使用真实源文件行号。
- 后果：公共文件浏览不再因零值、caller-supplied 大数或深行跳转进入无界整文件 materialization；窗口仍提供完整 `TotalLines` 和有用的行 prefix，且与模型完整行 continuation 保持各自消费语义。没有复制 app2 `workspaceflow.Service`、第二 filesystem facade、配置旋钮、legacy unlimited path 或 byte-offset API；共享 `tools/fs` 与 `app/cli` 零修改。

## ADR-RT-090：Workspace search 必须拥有编译准入、有限语料与精确总数

- 状态：已接受并实施，P171 完成；只改变 Runtime internal Workspace Application/adapter/Delivery 语义与测试文档，公共 Protocol shape、Artifact、SQLite、Desktop source、Agent Framework、`tools/fs` 与 CLI 不变。
- 背景：失败优先反例 `d5fe5eef1` 证明旧 `workspace.files.search` 把 raw regex 与 caller positive limit 直接交给 `tools/fs.LocalExecutor`，后者先用 `exec.Cmd.Output` 完整保留 `rg/grep` stdout、再在内存中截断；9 条 1 MiB 匹配会先保留约 9 MiB。超过 retained limit 后的第二次 count 又继承 executor 的 250-entry default：300 个分别命中的文件最终只报告 `total=250`。无效 regex 泄漏为底层进程错误，Application 对 `limit=100000`、越界路径或 count/material 不诚实的 direct-port 结果也不复验。
- 决策：Application 在 workspace/path I/O 前先准入最多 64 KiB 的有效 regex 并编译成唯一 `GrepPlan`，零/最高 retained count 固定为 100/1000；raw query 与 caller limit 不再进入 port。Adapter 删除该公共纵切对 `tools/fs` 与 `rg/grep` 的依赖，改为复用 ignore-aware、最多 20,000 项的 Workspace file catalog，并通过共享 `textread.VisitLines` 以 64 KiB buffer 扫描。Searchable corpus 只包含最多 8 MiB、单行最多 1 MiB 的完整 UTF-8 regular file；binary/invalid/oversized individual source 整体排除，一次请求最多实际扫描 512 MiB，目录或 aggregate 越界明确失败。
- 决策：Adapter 在完整 admitted corpus 上单程精确累计 `Total`，同时只保留 count 与 8 MiB material 都允许的稳定 whole-row prefix；不再启动二次 count 或从 `limit+1` 推断。Application 对 direct port 再验证 count/material、root-relative clean path、positive line、stable order、UTF-8/NUL、单行上限及每行仍匹配 compiled regex。Delivery 只把 invalid regex/资源越界稳定映射为 `invalid_params`。
- 后果：Desktop 既有 `total > matches.length` overflow presentation 得到精确事实，搜索取消在文件扫描中可见，超大输出不会在截断前先 materialize。该批采用 app2 `workspaceflow` 的 in-process、file-catalog-driven 思路，但保留正则语义并补上 app2 的无 aggregate source cap、fixed-Scanner long-line request failure、silent per-file omission、caller limit 与 direct-port validation 缺口；不复制其 facade、第二 filesystem owner、兼容 API 或配置旋钮。

## ADR-RT-091：模型文件搜索复用有限语料而不是通用子进程 executor

- 状态：已接受并实施，P172 完成；允许 model-facing Tool schema/response breaking change，只修改 `runtime`，公共 Protocol、Artifact、SQLite、Desktop source、Agent Framework、共享 `tools/fs` 与 CLI 不变。
- 背景：失败优先反例 `85536eb29` 证明 Runtime model/direct `glob` 与 `grep` 仍把 `max_results` 交给 `tools/fs.LocalExecutor`，后者先以 `exec.Cmd.Output` 完整保留 `find` 或 `rg/grep` stdout，再解析并切 slice。宿主 PATH 缺少二进制时能力整体消失；300 条命中只保留 2 条却没有 exact total；一条超过 1 MiB 的病态文本先形成超过 1 MiB Tool result；glob 还会遍历 `.git/node_modules`。Grep schema 同时暴露 rg-specific file type/multiline、独立 ignore flag、上下文与三种 output mode，使组合能力和外部实现细节进入模型决策面。
- 决策：Runtime 以 consumer-owned typed functions 直接实现 model/direct `glob` 与 `grep`，两者共享 Workspace 的 ignore-aware 20,000-entry catalog；grep 直接调用 P171 的 bounded text scanner，glob 使用同一 catalog 上的完整 segment-doublestar matcher。Path 在 catalog/read 前解析为 canonical root 内 identity；glob/regex/path 分别最多 4/64/4 KiB，默认/最高 retained count 统一 100/1000。两种最终 encoded Tool result 都最多 1 MiB，逐行计算 JSON representation 并只发布稳定完整前缀；grep 的前置 scan 仍服从 8 MiB row material、8 MiB file、1 MiB line、512 MiB aggregate source。响应都发布 exact `total` 和诚实 `truncated`。
- 决策：grep request 收敛为 `pattern/path/max_results`，regex 明确为 Go/RE2 line search；删除 `file_glob/file_type/ignore_case/multiline/before_context_lines/after_context_lines/output_mode`，文件选择、case flag 与上下文分别由 `glob`、RE2 inline flag 与 `read` 组合。glob 删除独立 `ignore_case`，并把 pattern 明确解释为相对所选 path 的 doublestar。不存在旧字段 alias、rg fallback、第二 executor、环境开关或 response dual shape。
- 后果：模型搜索不再依赖 `rg/grep/find` 是否存在，也不会在 retained cap 前完整 materialize 外部 stdout；binary、invalid UTF-8、超大文件与病态长行按 P171 从 searchable corpus 整体排除，catalog/aggregate failure 保持显式。永久 architecture gate 禁止 Runtime toolset 重新构造共享 Glob/Grep tool 或在 bounded search owner 启动进程。Tool 名称与 Presenter 的 `matches/path/line/text` 消费保持，新增 total 不要求 Desktop 改动；共享 `tools/fs` 留给其他真实消费者。

## ADR-RT-092：Skill progressive disclosure 与托管生命周期共用有限来源合同

- 状态：已接受并实施，P173 完成；只修改 `runtime`，公共 Protocol shape、Artifact、SQLite、generated binding、Desktop source、Agent Framework、共享 `skills`/`tools/skills` 与 CLI 不变。
- 背景：app2 证明 `skills.discovered.list`、Desktop Available 与 Run 内 progressive tools 应共用同一 `ResourceSource`，但两代实现都直接调用共享 SDK 的 complete `ReadDir`/`ReadFile`，没有产品容量。失败优先反例 `1ca213c51` 证明超过 1 MiB 的 `SKILL.md` 会完整进入模型、单 source 257 个 Skill 会进入非分页 discovery、托管 List/idle sweep 会分别扫描 257 个 active entries，`.usage.json` 也会整体接纳超过 64 KiB 或 256 records；复核同时发现 `read_skill_resource` 的共享 `io.ReadAll` 可从资源文件旁路正文上限。
- 决策：Skill Domain 唯一发布每 source 256 candidates、每 top-level directory 272 raw entries、1 MiB authored document 与 1 MiB model-facing resource 上限及稳定 capacity/size errors。`adapter/promptsource` 用 Runtime-owned finite `ResourceSource` 同时服务 project-first/user discovery 与模型工具：目录先用 capacity+1 sentinel，正文 stat 后经 cancellation-aware `limit+1` 读取，资源再以 stat 与 read-time sentinel 防增长。共享 Agent Skills SDK 继续拥有格式、merge 与 confined resource path，不复制其公共包或修改其他消费者。
- 决策：用户托管 Store 在同一 directory lease 下以一个 active+archived 256-entry snapshot 驱动 list、idle sweep 与 approval capacity；首次 revision archive slot 也必须在任何 rename 前预留容量。usage sidecar 固定 64 KiB/256 valid-name records，以 bounded/cancellable reader 在 JSON materialization 前准入，并在写入时复验 encoded bytes。Malformed metadata 仍按非关键 sidecar 重建，资源或容量越界则明确失败；不截断 list、不跳过 over-capacity source、不保留旧 unbounded reader。
- 后果：Desktop 的 discovered/library view 与模型 `list_skills/load_skill/read_skill_resource` 不再先支付无界目录、正文或资源成本；写事务也不能制造下一次完整读取必然失败的库。合法 project-over-user precedence、archive/restore、proposal CAS、usage best-effort recording 与外部 invalid Skill skip 保持。该批采用 app2 的单一 source/公共验收思路，补上其 source/document/resource/usage capacity 缺口；没有引入 facade、分页假象、配置旋钮、兼容路径或 CLI 改动。

## ADR-RT-093：本地 Runtime 凭据是部署交接合同，不属于 HTTP 或 Desktop

- 状态：已接受并实施，P174 完成；修改 `runtime` 与 `desktop`，公共 Go API breaking 增加 `runtime/localruntime` 并删除 internal HTTP 的 `LocalToken/OpenLocalToken` owner；Protocol、Artifact、SQLite、Wails binding、Agent Framework 与 `app/cli` 不变。
- 背景：失败优先反例 `58ddbeeaf` 证明 Runtime 会 trim 并接受空白包裹的 32-byte token，而 Desktop 用无界 `os.ReadFile` 接受任意文本、1 MiB 文件、0644 文件和 symlink。两个解析器使“Runtime 接受的 bearer”与“Desktop 注入 renderer 的 bearer”不是同一事实；Desktop 又不监管 Runtime process，没有 descriptor/pipe 可替代 durable file handoff。在各消费点补长度判断仍会保留第二套 grammar 和凭据 lifecycle。
- 决策：采用 app2 的 `localruntime` 部署边界，但把其 128-byte/trim/read-all 语义收紧为生产者可唯一生成的 canonical value：精确 43 bytes、Raw URL Base64、解码后精确 32 bytes 且 re-encode 相同。`ReadToken` 先以 `Lstat` 拒绝非 regular/0600/非精确大小，再 open/fstat/`SameFile` 证明未替换，并用固定 44-byte `ReadFull` probe 捕获 inspect 后增长；不分配文件大小相关 buffer。`OpenToken` 以 0600 synced candidate + hard-link no-overwrite 发布并 sync parent；并发 Runtime generation 读取唯一 winner。`Token` fields 私有，只有成功验证才能取得 Value/Path。
- 后果：Runtime executable 与 Wails Desktop 通过同一个生成并冻结的公共 Go package 交接凭据；该 package 是 Go 1.27 独立子模块，使 Desktop 的 module graph 不吸收 Runtime 后端依赖。HTTP transport 只剩 constant-time bearer gate，不再发现路径或拥有 durable lifecycle。Desktop 仍允许文件缺失以表达 Runtime 未启动，但任何现存 broken credential 都 fail closed，不能把任意磁盘 material 注入 renderer。旧 HTTP token API、trim compatibility、unbounded reader、alias/fallback 均删除；没有把 Runtime server graph 嵌入 Desktop。

## ADR-RT-094：Knowledge crash recovery 流式拥有任意项目目录

- 状态：已接受并实施，P175 完成；只修改 `runtime` internal Knowledge filesystem owner、架构门禁、测试与文档，公共 Protocol、Artifact、SQLite、公共 Go API、Desktop source、Wails binding、Agent Framework 与 CLI 不变。
- 背景：P162 已把每份 `FLAME.md` 收敛为 1 MiB complete document，但原子替换的 cold recovery 仍在 home/project/cwd 根目录调用 `ReadDir(-1)`，先完整物化所有 sibling 再筛选 `.FLAME.md.flame-stage-*`。该目录可以是用户选择的任意大型项目根；文档上限、transport cap 和最终只删除少量 stage 文件都不能追回这次无界分配。失败优先架构反例 `9d701070f` 唯一命中该调用，证明 Runtime 其余生产目录读取已使用显式正数上限。
...
## ADR-RT-095：Workspace Checkpoint 复用唯一 Git process 与有限 selection

- 状态：已接受并实施，P176 完成；只修改 `runtime` internal Checkpoint/Git Infra、架构门禁、测试与文档，公共 Protocol、Artifact、SQLite、公共 Go API、Desktop source、Wails binding、Agent Framework 与 CLI 不变。
- 背景：P167 已让 Workspace VCS read model 统一经过 `gitprocess.Run`，但从 app2 原样保留的 Run-boundary Checkpoint 仍用两套 `gitprocess.CommandContext + bytes.Buffer` 完整保留 stdout/stderr；`ls-files` 后又用无上限 map/slices 接纳任意 path count。单文件 2 MiB 只限制个体，任意数量的小文件仍可让一次 snapshot 与 shadow repository 无界增长；source alternates 用 `os.ReadFile`，source index 也可无上限复制。通用 output cap、最终 Git commit 与 Desktop rollback UI 都不能追回这些 admission。失败优先架构反例 `43df0c7fa` 精确命中 Checkpoint 的 `bytes` import 与两个 direct `CommandContext` owner。
- 决策：Checkpoint 保留 shadow-repository/CAS/restore 唯一语义，但所有 real/shadow Git command 统一调用 `gitprocess.Run`，继承 64 MiB stdout、64 KiB bounded-drain stderr、caller cancellation、clean Git environment 与 WaitDelay；nonzero exit 通过私有 typed status 保留 predicate 语义。`ls-files -z` 使用未 trim 的 raw bytes 并要求完整 NUL record，selection 在任何 index mutation 前限制为 20,000 unique local paths 与 512 MiB current regular/symlink material；caller cancellation 在逐项 inspect 中可见。source alternates 限 64 KiB/256 stores，resolved output 复验 64 KiB；source index stat 与 bounded copy 均限 64 MiB。源仓库先由严格 `git.IsRepo` 分类，corrupt/unavailable 不再伪装成 non-repository empty seed。
- 后果：Checkpoint 不再是第二 Git process/lifetime owner，stderr 噪声、stdout、path set、seed metadata 与磁盘复制都有明确上限；20,001 paths、512 MiB+ admitted material、超大 alternates/index 在 commit/tag 前整体失败。首个以空白开头的合法文件名不再被 `TrimSpace` 改写，history/files/both rollback、2 MiB per-file exclusion、linked worktree seed、atomic initial publish、reversible restore 与 tag identity 保持。没有 app2 facade、第二 runner、partial snapshot、truncate-to-success、配置旋钮或兼容路径。

## ADR-RT-096：Sandbox bounded writer 的方法集不能绕过容量 owner

- 状态：已接受并实施，P177 完成；只修改 `runtime` internal Sandbox process writer、测试与文档，公共 Protocol、Artifact、SQLite、公共 Go API、Desktop source、Wails binding、Agent Framework 与 CLI 不变。
...
- 状态：已接受并实施，P178 完成；只修改 `runtime` internal MCP process/session owner、测试与文档，公共 Protocol、Artifact、SQLite、公共 Go API、Desktop source、Wails binding、Agent Framework 与 CLI 不变。
...
- 状态：已接受并实施，P179 完成；只修改 `runtime` internal Sandbox workspace materialization、测试与文档，公共 Protocol、Artifact、SQLite、公共 Go API、Desktop source、Wails binding、Agent Framework 与 CLI 不变。
...
## ADR-RT-099：模型 Shell 的 background 状态不能放弃进程树所有权

- 状态：已接受并实施，P180 完成；只修改 `runtime` internal Shell Infra/Tool consumer、测试与文档，公共 Shell Tool name/request/response shape、Protocol、Artifact、SQLite、公共 Go API、Desktop source、Wails binding、Agent Framework 与 CLI 不变。
...
- 决策：Agent 只新增中性 `ModelContextReducer`，在每次主模型调用前提供一个预算检查边界；成功 settlement必须携带 actual messages 并由 Execution 安装回 WorkingContext。预算检查低于 message/token 任一触发阈值时必须原样早退，不运行 PreCompact、不调用摘要模型、不写 SQLite；达到阈值才执行压缩，重写后的上下文回落到阈值以下，因此下一次调用不得立即重复压缩。Runtime 定义充血 `ModelContextCompaction` / result 值对象，冻结 durable/transient、Session、exact model selection、instructions、candidate、Tool manifest、options、protected tail 与 PreCompact veto；具体 Compactor 从 catalog解析本次模型窗口、扣除固定/未提交后缀开销，并复用现有 trim/summary ladder。
...
- 决策：Conversation 的 SQLite row count 与 message sequence 是同一个 durable 坐标。完整读取遇到任何无法恢复的 row 必须携带 sequence identity fail closed，禁止跳过后返回缩短历史；否则 Run message mark、rollback/fork boundary 与 compaction expected count 会在同一数据上产生两套位置解释。
...
## ADR-RT-104：可变 Session state 不能冻结为 Deployment instruction

- 状态：已接受并实施，P189 完成；允许 Runtime internal adapter/provenance breaking change，公共 Runtime Protocol、Artifact、SQLite、Desktop binding、Agent Framework 与 CLI不变。
- 背景：fresh Run把 Plan 合并进单个 System prompt，`interactionInstructionContext` 又把所有带 provenance的前导 System message冻结为 Deployment instruction。一个长 Run内 `set_plan` 后，旧 `## Current Plan` 因而永久留在高优先级前缀；达到压缩阈值时 summary后的 live-state reminder再注入新 Plan，provider同时看到旧、新两值。active Goal同样只依赖开场 User control与模型摘要保留 objective；摘要遗漏就没有确定性的当前 Goal owner。
- 决策：`WorkingContextComposer` 单独生成 canonical Goal→Plan Session-state messages，每条只携带一个 data provenance；Deployment instruction只冻结稳定来源。`ModelContextReducer`在每次主模型预算检查前剥离上一份 state snapshot，从同一 Goal/Plan durable reader重新读取、验证 Session ownership/Domain invariants并重接，再把纯 Conversation candidate交给 compactor。配置必须同时提供 compactor与 state source；读失败、混合 provenance、重复或 Plan→Goal乱序均 fail closed。Plan不再进入 summary live-state reminder；该 reminder只拥有不可由 SQLite重建的 running shell进程态。
- 后果：Plan replacement后的下一主调用、压缩前后及 Goal outcome reporting后的调用都只看到同一个当前 durable value；Goal objective/status不依赖摘要质量，Goal/Plan也不参与 Deployment identity或 Conversation rewrite。真实 Bootstrap/SQLite E2E用 active Goal、Plan revision 1→2、12轮 Tool loop、阈值处一次 summary及完成结算证明 objective/status/Plan同值；测试摘要刻意完全遗漏 Goal/Plan仍能完成。没有第二 store、缓存、兼容消息、summary parser或魔法 map。

## ADR-RT-105：运行中 Strategy 内存不是 compaction 的第二 durable participant

- 状态：已接受并实施，P190 完成；只增加真实 Runtime↔HTTP↔TypeScript failure/SIGKILL回归与文档，不修改生产代码、公共合同或持久化 shape。
- 背景：durable root compaction在 reducer内先提交 SQLite conversation与Run watermarks，Agent只有在随后的主 provider调用成功结算时才把effective messages安装进WorkingContext。表面上存在“SQLite已提交、Strategy仍为旧值”的窗口；若误把运行中内存当作第三个durable participant，就会诱导出checkpoint双写、journal或跨框架事务。必须先证明provider failure与进程死亡后的真实恢复事实。
- 决策：SQLite transaction是compaction唯一durable commit：conversation replacement与全部Run watermark CAS共同成功或共同回滚。运行中的Interaction state只属于当前Process generation；普通model effect使用`ReplayPolicyNever`，不保存可恢复的in-flight checkpoint。provider retry耗尽时当前Run确定失败并丢弃该Strategy generation；SIGKILL后startup recovery把in-flight Run结算为lost。两种路径的下一fresh Run都只从已提交SQLite history重建，不读取或恢复旧WorkingContext。只有HITL waiting boundary产生durable executor checkpoint，而该边界不存在未结算主model effect。
- 后果：不新增compaction journal、executor-checkpoint旁写、两阶段提交、补偿重放、兼容状态或SQLite epoch。真实HTTP E2E在精确阈值处提交summary后分别持续返回503与SIGKILL Runtime；前者Run为failed、后者为lost，重启/续跑的首个provider请求都只含summary、不含旧开场前缀，并携带主模型Tool manifest，证明每次预算检查没有立即再次压缩。若未来产品允许恢复in-flight model effect，必须以新的Strategy revision/digest与恢复协议重新准入，不能复用当前waiting checkpoint假装原子。

## ADR-RT-106：模型上下文预算必须覆盖完整请求并由 provider 实际用量校准

- 状态：已接受并实施，P191 完成；允许 Runtime internal compaction/accounting 与 executor-owned checkpoint envelope breaking change，公共 Runtime Protocol、Artifact、SQLite、Desktop binding、Agent Framework 与 CLI不变。
- 背景：P188 已把预算检查移到每次主模型调用前，但旧 token 估算仍只把文本 transcript bytes 除以 4；assistant reasoning/signature、ToolCall identity/arguments、media identity/metadata、完整 Tool manifest 与 Options 会漏算，固定上下文和 history 又分别舍入。反例证明加入 16KiB Tool arguments、CJK reasoning 和 signature 后估算仍与纯文本基线同为 8 token；而“直接JSON序列化完整请求”的朴素修复又会把 2MiB inline image 的传输 bytes解释成约50万文本token。漏算和这种反向误算都会破坏“接近阈值才压缩”的边界。
...
- 决策：Domain `modelref.TokenLimits`不可变地拥有context window、max input与max output三项provider事实；每项以独立presence表达，数值零不再承担“未知”。max input不得越过已知context；max output是独立事实，streaming/multimodal catalog可以合法发布`maxOutput > context`，此时context是input envelope而非shared window。`OutputReservation`显式区分未请求与正值 ceiling；`InputCeiling`始终服从max input，并且只在published facts没有证明output独立时从shared context扣reservation。catalog adapter只在upstream边界把旧零缺席翻译一次，私有compatible model的catalog miss仍合法。`runs.start`在任何durable Run/Item写入前拒绝超过已知max output或耗尽可证明shared window的显式请求；每次主调用与post-Run maintenance都消费相同selection、limits与options。
...
## ADR-RT-109：Reasoning effort 必须属于 exact model selection，而不是 UI 偏好

- 状态：已接受并实施，P196 完成；允许 Runtime Protocol、Artifact、SQLite、executor-owned checkpoint 与 Desktop SDK breaking change，CLI/TUI 不变。
- 背景：Runtime catalog 已发布模型是否支持 reasoning、支持的 effort 等级与默认值，Core/provider 也能传递该参数，但产品只持久化 provider/model pair，Desktop 既没有执行控件，也不能恢复一次 Run 实际使用的 effort。若把 effort 仅放在 Composer local state，重启、Goal、Schedule、HITL continuation、artifact import 与 active Run Context gauge 都会失真；若用 provider/model 到 effort 的手写 map，又会制造第二 catalog 和跨模型泄漏。
- 决策：`modelref.Selection` 充血为 immutable provider/model identity + optional model-owned reasoning effort，并由自身构造、校验和 patch 行为关闭非法组合。Session 是下一 Run 默认选择的唯一 durable owner；reasoning-only patch 保留 identity，显式切换 pair 而省略 effort 必须清空旧 effort。Run、Goal、Schedule/occurrence、Interrupt/executor checkpoint 与 Artifact 各自在 admission/opening 时保存完整选择。known catalog model 的 effort 必须属于其 exact published levels；catalog miss 的私有模型继续允许。executor 只从 frozen Run selection 构造 Core `chat.Options`，provider adapter 不猜默认或维护别名表。
- 决策：Desktop 从生成合同与 `models.list` 投影一个 immutable `SelectableModel`，完整保留 context/max-input/max-output、reasoning levels/default、input/output modalities、tool use、structured output 与 knowledge cutoff。Composer 的 model/effort controls、图片准入和 Goal/Run submission 都消费该对象；active root Run 的 Context gauge 优先使用 frozen Run selection，只有 Run material 未提供 selection 时才读取 editable Session default。恢复时保留 durable effort，即使新版 catalog 已移除该值也不静默改写，下一次 admission 会诚实失败。
- 后果：Artifact 从 v23 一次性升至 v24，SQLite epoch 从 83 升至 84，Session/Run/Goal/Schedule/occurrence 增加 strict `reasoning_effort`；旧归档和数据库不迁移、不双读。没有 UI-only preference、provider-specific magic map、model-id inference、兼容字段或 TUI 旁路。

## ADR-RT-110：Resume 补充输入必须与 HITL claim 共用 continuation 原子边界

- 状态：已接受并实施，P196 完成；允许 Runtime internal continuation port、Protocol/SDK Resume request/result 与 executor-owned checkpoint breaking change，CLI/TUI 不变。
- 背景：公共 `runs.resume` 已允许 `input?`，Delivery 也能创建用户 Item，但 Agent continuation 过去只收到 approval/question responses。于是产品 Transcript/Conversation 声称补充输入已提交，恢复后的 Strategy 却看不到它；若再走普通 steer 路径，projection writer 又可能把同一内容追加第二遍。HITL answer、opening input、checkpoint deletion 和 next Segment 必须只有一个 durable winner。
- 决策：Application 在 answer claim 后把可选 input 规范成带 exact committed Item identity 的 `CommittedUserInput`；同一 continuation opening write-set 原子提交 answer claim、User Item、checkpoint removal 与 next Segment，不提前把 input 写入 provider Conversation。`WaitingExecutionContinuer.BeginContinuation` 同时接收 immutable pending responses 与该 input；Agent 在同一个 safe boundary 按 answer、deterministic steer 的顺序投递信号。answer 先把对应 Tool result 提交到 Conversation，随后 `SteerMessagesApplied` 才追加 input Conversation，从而保持 provider 合法的 `tool → user` 次序。steer 携带 exact projected Item ID，reducer 因此只追加 Conversation 而不创建第二个 Transcript Item；private checkpoint 保存该精确身份，恢复时不能退化成布尔猜测。
- 后果：成功 Resume 可返回 `userItemId`，客户端 driver 使用 `{responses,input?}` 单一参数对象；不提供 input 时行为不变。Interaction checkpoint private schema 从 v4 升至 v5，旧 payload fail closed。不存在 Conversation 提前写入、双写补偿、临时内存旁路、布尔 projection 标记、字符串拼接 signal id、第二 Resume method 或兼容 overload。

## ADR-RT-111：文件系统根目录必须是不可扩张的能力，而不是调用约定

- 状态：已接受并实施，P197 完成；允许 `tools/fs` public Go API 与 Runtime model-tool policy breaking change，Runtime Protocol、Artifact、SQLite、Desktop binding、Agent Framework、CLI/TUI 不变；本批不发布新 tag。
- 背景：`LocalExecutor.Root` 只是公开字符串提示：绝对路径、`..`、`~` 以及 Glob/Grep 请求自己的 `Root` 都能替换或逃出构造时根目录。六操作 `Executor` 又强迫只需 Read 的 Runtime decorator 假装拥有 write/edit/patch/search，并促使它用 `os.Open` 复制一套路径与扫描逻辑；默认 Read 先 `os.ReadFile` 完整物化，长期 executor 的 per-path lock map 也永久保留所有历史路径。Runtime 只在 isolated mutation guard 拦写，non-isolated filesystem tool 因而把宿主机任意绝对路径当作正常能力。
- 决策：删除总集 `Executor`，建立 Reader/Writer/Editor/PatchApplier/Globber/Grepper 六个单操作端口；`LocalExecutor` 的根变为构造时冻结的私有 authority，所有请求路径只能相对缩小，底层 read/write/edit/patch/glob 全部通过 Go `os.Root` 执行，lexical escape、absolute-outside 与 escaping symlink 均 fail closed。Glob/Grep 的请求字段从可替换 `Root` 改为子树 `Path`，结果统一保持 workspace-relative；Glob 只保留有界 lexicographic result set，Grep 禁用 config/symlink following并只接收已验证 relative operand。默认构造器绑定构造时 CWD，不再存在“空根=无限 authority”。
...
- 状态：已接受并实施，当前质量 Goal Q3 本批完成；只修改 CLI settings/command/TUI projection、测试与文档，Runtime Protocol、Artifact、SQLite、Desktop 与 Agent Framework 不变。
- 背景：Runtime 在 fresh Session admission 安装部署配置的完整 provider/model，后续 Run 省略 pair 时读取该 durable selection；但 CLI `settings.Default` 又硬编码 `deepseek/deepseek-v4-flash`，one-shot 与 TUI 每次 Run 都显式发送它。反例证明：Runtime 即使部署为 Anthropic、Ollama 或另一个 DeepSeek model，未配置 CLI 的用户也会被客户端静默改回硬编码 pair；已有 Session 的 durable selection 同样会在下一次发送时被覆盖。CLI 与 Runtime 还共用 `FLAME_PROVIDER/FLAME_MODEL` 环境名，而 Cobra 配置绑定从当前子命令 flag set 取值，使 `sessions update --model` 这类局部 patch 能冒充根级 Run preference。
- 决策：CLI model preference 使用既有 zero-or-complete pair 语义；默认 pair 为空，one-shot、queue、workbench replay 与 Goal start 原样省略，让 Runtime 读取 active Session selection。TUI 仅在展示时把空 override 与当前 Session projection 合成 label，不回写 command state；显式配置、root `--provider` + `--model` 或模型选择仍可建立 exact override。CLI settings environment prefix 改为 `FLAME_CLI_*`，与 Runtime `FLAME_*` 分离；配置 binding 只读取 root persistent flag owner，不观察同名 local flag。
- 后果：CLI 不再特权化某个 vendor，也不再未经用户动作改写 Session model；Runtime env、CLI env、session patch flag 与 run override 各有唯一 owner。回归覆盖默认 Run 保持空 pair但 UI 显示 `mock/balanced`、Runtime env 不进入 CLI config，以及 `sessions update --model` 不污染 settings。没有向 Runtime discovery 添加第二 default 字段、在 CLI 猜 provider、兼容旧 env fallback、per-session CLI cache、新 package 或协议变化。

## ADR-RT-119：Provider test 必须在 Runtime 内有界结算

- 状态：已接受并实施，当前质量 Goal Q4 本批完成；只修改 Runtime models Application policy、测试与文档，公共 Protocol、Artifact、SQLite、Desktop、Agent Framework 与 CLI shape 不变。
- 背景：endpoint model discovery 已在 Infra 用四秒 timeout 有界读取，但 catalog provider 的 `providers.test` 最小 chat call 只继承 transport/TUI application lifetime。真实 endpoint 接受连接后若不返回，probe 会永久占住 CLI 的 `model-config` operation slot；用户既不能再次测试，也不能配置 provider，只能退出整个 CLI。把 timeout 只放在 TUI、HTTP handler 或每个 SDK adapter 会让其他 binding 与未来 provider 拥有不同结算语义。
- 决策：Models Coordinator 为 provider test 建立默认十秒 child context，并把同一 context 交给 ProviderProber；这是 user-facing use case 的 settlement policy。caller context 取消仍优先返回 command error；Runtime-owned deadline 或其他 probe error继续记录到 trace 并投影为既有稳定 `failed` outcome。`ProbeTimeout` 只允许内部 composition/test 缩短策略，非正值回到产品默认；网络 port 与现有 chat clients 必须遵守 context cancellation，Application 不用泄漏 goroutine 的 watchdog 假装能强杀不合作实现。
- 后果：慢/卡住的真实 provider probe 最迟在产品 deadline 后释放 operation owner，CLI 不需计时器、重试按钮旁路或错误字符串匹配；正常 DeepSeek/endpoint probe 与 `not_configured` 快路径不变。决定性测试用 cooperative blocking prober 和短 policy 证明 timeout 返回 `failed` 且不消耗 caller 的安全 deadline。没有协议 timeout 字段、用户配置旋钮、后台 goroutine、provider 特判或多客户端测试。

## ADR-RT-120：Endpoint model discovery 必须服从 provider 的真实 wire protocol
...
- 状态：已接受并实施，当前质量 Goal Q4 本批完成；只修改 Runtime internal Anthropic-compatible model discovery、测试与 API 文档，公共 Protocol、Artifact、SQLite、Desktop、Agent Framework 与 CLI shape 不变。
...
## ADR-RT-126：跨环身份保持强类型，但不按身份种类拆 package

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime shared-kernel 批次完成；允许 Runtime internal Go API breaking change，公共 Go surface、Protocol、Artifact、SQLite 与 HTTP wire 不变。
- 背景：Executable build、Application write-set commit、executor member/effect、durable idempotency namespace、Runtime process incarnation、产品名以及 model/resource/cursor envelope 必须同时被多个架构环消费，放回某个单一环会造成反向依赖或复制规则。但旧结构把这些事实拆成 9 个 package，其中多个 package 只有一个常量或一个值类型；强类型边界被错误等同于物理 package 边界，import、别名、文档和发现成本随 identity 种类线性增长。Go MCP SDK 的 package-layout 证据明确建议把共同演进的 API 留在一个可发现 package，只为真正独立机制拆包。
- 决策：`internal/identity` 作为受限 shared kernel，按职责文件共同拥有 cross-ring identity grammar、resource envelope 和 technical value。`BuildID`、`CommitID`、`ExecutorID`/`MemberID`/`RequestID`/`EffectID`、`IdempotencyNamespace` 与 `RuntimeInstanceID` 仍是不同的不可互换类型；构造器、错误、prefix 和 ceiling 都使用语义全名，不提供 generic `ID`、generic `Parse`、字符串修复或可扩张 registry。Session、Run、Goal、Schedule、Provider 等领域 identity 仍由各自 Domain owner 构造，只消费 shared-kernel policy。
- 后果：物理删除 `buildidentity`、`commitidentity`、`cursorresource`、`executoridentity`、`idempotencynamespace`、`modelidentity`、`productidentity`、`resourceidentity` 与 `runtimeinstanceidentity`，不保留 alias、forwarder 或 compatibility package。cross-ring import DAG 仍指向一个无外部环依赖的叶子，领域值没有合并成 magic string；新增 identity 只有在证明跨环消费者后才能进入该 package，否则必须留在语义 owner。

## ADR-RT-127：Run 边界标题生成属于 `runsegment`，不是独立 Adapter context

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime Adapter 批次完成；允许 Runtime internal Go API breaking change，公共 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
- 背景：标题生成只在 `runsegment.Finalizer` 的 terminal/parked Run 边界发生；`TitleGenerator` 消费接口、fallback 结算和 maintenance lifecycle 都已由 `runsegment` 拥有。旧 `adapter/sessiontitle` 只有这一位生产消费者，没有独立状态、生命周期、外部协议或替换轴，只把同一个 use case 的接口与唯一实现拆成两个 package。相反，`adapter/model/auxiliary` 同时服务 title、compaction、Memory 和 Skill maintenance，并拥有 Scope model SDK 的 request envelope，仍是有真实多消费者和外部翻译的边界。
...
- 后果：物理删除 `adapter/sessiontitle`，不保留 alias、forwarder、factory shim 或 Bootstrap wrapper。Run-boundary maintenance 的接口、实现、fallback 和生命周期在同一 package 可发现；`auxiliary` 继续隔离 middleware-free auxiliary provider call，未把 Scope SDK 泄漏到 Application。
...
## ADR-RT-130：Session 应用上下文按生命周期收口，不按读写 operation 平铺

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime Session Application 批次完成；允许 Runtime internal Go API breaking change，公共 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
...
## ADR-RT-136：Skill proposal library translation 属于 Workspace adapter context

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime Workspace Adapter 批次完成；允许 Runtime internal Go API breaking change，公共 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
...
## ADR-RT-139：测试构造机制不按 Domain 实体拆 package

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime Test Support 批次完成；允许 Runtime internal test API breaking change，生产 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
- 背景：`internal/testsupport` 下的 `conversationfixture`、`identityfixture`、`itemfixture`、`runfixture` 与 `sessionfixture` 都只为 Runtime 测试提供合法对象构造、稳定 identity 或内存 fake，没有独立领域 vocabulary、lifecycle、外部 translation 或 replaceable boundary。每个目录只有一个生产文件，测试必须同时 import 多个 sibling package，并依靠 `runfixture` alias 避开通用 `MustRestore`/`Input` 命名冲突；这是“一个实体一个 package”对测试机制的机械复制。
- 决策：建立唯一 `internal/testsupport` package，以 `conversation.go`、`identity.go`、`item.go`、`run.go` 与 `session.go` 等职责文件共同拥有 Runtime 测试支持。跨职责的公开符号使用语义全名：`ConversationStore`、`ItemInput`、`RunMetricsInput`、`MustRestoreItem`、`MustRestoreRun` 与 `MustRestoreSession`；测试 invalid state 时仍直接调用对应 Domain restore 并断言错误，testsupport 不成为第二生产模型。
- 后果：物理删除五个 fixture 子package、全部旧 import、alias 和空目录，不保留 forwarding 或 compatibility API。测试消费者只依赖一个机制 package，Domain 与 Application 的生产依赖图不引入 testsupport，现有行为测试继续验证同一产品合同。

## ADR-RT-140：Admission 与 recovery election 属于同一 Ownership Application context

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime Ownership Application 批次完成；允许 Runtime internal Go API breaking change，公共 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
- 背景：`application/sessionadmission` 定义 Session/working-tree 的 process-local gate、`Lease` 与 `Ownership`，`application/ownershiprecovery` 又为 recovery sweep 定义同形 `Lease` 与另一个 `Ownership`。同一个 `adapter/runtimeownership.Manager` 必须同时 import 并实现两套类型；Bootstrap 也把 admission 与 ordered Run→Goal recovery 当作两个平级 operation package 装配。它们的共同事实都是“哪个 Runtime process 暂时拥有某项提交权”，物理分包复制了 vocabulary，而不是表达不同 bounded context。
- 决策：建立单一 `application/ownership`，以 `admission.go`、`recovery.go` 与 `lease.go` 共同拥有 process-local admission、cross-process acquisition ports 和 ordered recovery use case。`Lease` 成为唯一释放合同；`AdmissionBackend` 与 `RecoveryBackend` 保持不同的 Try/wait 语义，构造入口改为 `NewGate` 与 `NewRecovery`，不建立总 service。`adapter/runtimeownership` 继续独占 data-directory lock layout、physical working-tree identity 与 advisory-lock translation。
- 后果：物理删除 `application/sessionadmission`、`application/ownershiprecovery`、全部旧 import、重复 Lease/Ownership 类型和空目录，不保留 alias、forwarder 或 compatibility package。Session single-writer、working-tree shared/exclusive、startup/survivor recovery election、Run→Goal reconciliation顺序与跨进程行为不变。

## ADR-RT-141：Environment Provider registry overlay 属于 Model adapter context

- 状态：已接受并实施，当前 Runtime/CLI 治本重构 Goal 的 Runtime Model Adapter 第二批完成；允许 Runtime internal Go API breaking change，公共 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
- 背景：`adapter/providerregistry` 只有 Bootstrap 一个生产消费者，职责是把 environment credential snapshot 叠加到 `application/models.ProviderRegistry`，并守住 stored-over-environment precedence、environment-only Provider、identity一致性与 secret 不落库。`adapter/model` 已拥有同一 Provider/Model integration boundary 的 credential/custom endpoint提取、catalog、probe、chat与embedding construction；保留平级 registry package 迫使 composition root 把同一外部上下文按处理阶段拆开认识。
- 决策：把 environment overlay 收回 `adapter/model/environment_registry.go`。`WithEnvironmentKeys` 继续返回 consumer-owned `models.ProviderRegistry` port，inner registry始终只收到 durable patch；effective read才叠加 immutable environment key。错误词汇统一为 `model`，identity mismatch使用语义 sentinel `ErrRegistryIdentityMismatch`。
- 后果：物理删除 `adapter/providerregistry`、全部旧 import、错误前缀和空目录，不保留 alias、forwarder 或 compatibility package。credential precedence、environment-only Provider、stored endpoint、clear/update行为和 secret authority不变；Bootstrap只组合一个 Provider/Model adapter context。

## ADR-RT-142：依赖环内按已证明的上下文建立一层物理层级

- 状态：已接受并实施，取代 ADR-RT-007 中“每一环默认一层深”的物理目录结论；ADR-RT-007 关于禁止 `service/repository/manager/impl` 套娃与 package 必须证明独立职责的约束继续有效。允许 Runtime/CLI internal Go import path breaking change，公共 Go surface、Runtime Protocol、Artifact、SQLite 与 Desktop 不变。
- 背景：前一轮删除无 owner 微包后，Runtime 的 Domain/Application/Adapter/Infra 仍分别把 22/16/21/17 个真实 package 直接平铺在依赖环下；CLI 的 28 个 package 也与 `internal` 一级目录一一对应。每个叶包可以独立成立，但物理结构没有表达已经存在的 Run、Session、Automation、Workspace、Integration、Toolset、Git、Process 等 context map，消费者只能在长目录清单中重新推断层级。单纯继续合并会制造 god package，单纯添加有 Go 文件的父包又会制造 facade。
- 决策：依赖环继续作为第一层；同一环内有多个经过 consumer、invariant、lifecycle 与 import DAG 证明的 package 时，增加一个无 Go 文件的 context namespace。Domain 以 Run、Session、Automation、Workspace 与 Integration 归组；Application 以 Agent、Automation、Integration 与 Workspace 归组；Adapter 以 Agent execution、Run、Toolset、Workspace、Model 与 Integration 归组；Infra 以 Filesystem、Git、Process、Integration 与 Storage 归组。`run`、`session`、`workspace`、`agentexec`、`model`、`toolset`、`git` 等 context root 自身拥有真实 package 时可以同时包含子 package。跨上下文机制和值对象保持环内直达；禁止 single-child namespace、facade parent 与装饰性深层目录。CLI 对应建立 `domain/application/adapter/delivery` 四个物理环，`arch`、`exactint` 与 test support 只作为已证明的跨环机制保留。
- 后果：全部 internal import、架构测试、路径型事实测试和文档一次性迁移，旧路径与空目录物理删除，不保留 alias、forwarding package 或 compatibility import。Runtime 的 production package 数没有因目录美化增加，CLI 也没有新增 facade；package topology gate 阻止新的未经审查平铺包和超过 `ring/context/package` 的嵌套。`executionctx` 因被 Agent execution 与 Toolset 两个 peer 共同消费而保持 Adapter 直达，`runtimeownership`、`scheduleidentity` 与 `advisorylock` 也不为单子节点制造父目录。

## ADR-RT-143：Package path 与 qualifier 只陈述一次 owner

- 状态：已接受并实施，补充 ADR-RT-142 的物理层级命名约束；允许 Runtime/CLI internal Go API、import path 与生成的 Schema/TypeScript 类型名 breaking change。公共 Go root/protocol/localruntime surface、Runtime Protocol method/field/JSON value、Artifact、SQLite 与 Desktop source 不变。
- 背景：层级迁移后仍有两类口吃。CLI 把同一个 Runtime binding 拆成 `adapter/runtimeadapter` 与 `adapter/runtimeprofile`，Profile 没有 Connection negotiation 之外的 producer、lifecycle 或替换边界；Runtime 又在 `adapter/model/utilitymodel` 中重复父级 `model`。调用点还出现 `runs.RunEvent`、`runs.RunProgress`、`sessions.SessionUsageReport` 与 `http.HTTPContract`，package qualifier 已经声明 owner，导出名却再次声明同一 owner。继续保留这些名字会让目录和 API 在每次读取时支付重复概念成本。
- 决策：CLI 以单一 `adapter/runtimebinding` 同时拥有 `Connection` lifecycle、DTO translation 与 immutable negotiated `Profile`，删除独立 profile package。Runtime 保留多消费者 auxiliary model SDK/resource-envelope 边界，但叶名改为 `adapter/model/auxiliary`。Application/HTTP internal API 改用 `runs.ProjectionEvent`、`runs.Progress`、`runs.ProgressCommit`、`runs.Projection`、`runs.TerminationCommitter`、`sessions.Items`、`sessions.UsageReport`、`sessions.Replacement` 与 `http.ContractSpec`/`http.Transport`。exact ubiquitous aggregate（如 `run.Run`、`session.Session`、`goal.Goal`）不为机械消重改成含糊同义词。
- 后果：旧 CLI/Runtime import path、旧导出名、package comment、错误前缀、测试与架构例外一次性删除，不保留 alias、forwarder 或 compatibility package。`HTTPTransportKind` 同步在生成 Contract 中 breaking 改为 `TransportKind`；JSON 字段仍是 `transport`，值仍是 `"http"`，因此不引入 Go/Schema 双词汇。CLI internal package 数减少一个；Runtime package 数不变。命名与路径 fitness tests 分别阻止 qualifier 前缀回流和叶名重复外层 ring/context，同时保留领域语言优先于机械 lint 的明确例外。

## ADR-RT-144：A2A lifecycle 由已发布的 ToolSet 唯一拥有

- 状态：已接受并实施；允许 Runtime internal Go API 与 import path breaking change，公共 Go surface、Protocol、Artifact、SQLite、Desktop 与 CLI 不变。
- 背景：`infra/integration/a2a` 只有 `adapter/toolset` 一个生产消费者，却把 `A2AAgentConfig` 逐字段复制为第二份 `ClientConfig`，再翻译为 Scope `Endpoint`；它还用 `Connections` 重复实现 Scope `ToolSet` 已有的 tool snapshot、partial-open cleanup、nil-safe close 与 close-once lifecycle。这不是独立 Infra mechanism，而是同一 tool assembly 边界中的转发 wrapper 和重复 authority。
- 决策：`adapter/toolset` 直接把唯一 `A2AAgentConfig` 投影为 Scope endpoint，保留属于 Runtime startup 的 tracing，并直接持有已发布的 Scope `ToolSet`。Scope 库唯一拥有连接 acquisition、失败回收、tool snapshot 与幂等 shutdown；Flame 不复制其实现或上游生命周期测试。低层 A2A/MCP transport SDK 仍被架构门禁隔离在各自 adapter/infra 边界。
- 后果：物理删除 `infra/integration/a2a`、重复配置类型、`Connections` wrapper、转发函数与重复 close-once 测试，不保留 alias、shim 或 compatibility package。Toolset 的失败清理与正常 teardown 都按资源 acquisition 的逆序关闭，A2A startup fail-closed 行为不变。
