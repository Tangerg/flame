...
- **JSON-RPC envelope `id` 必须是 string**（硬约束）。虽然 JSON-RPC 2.0 本身也允许 number id，本协议**只接受字符串 id**
  ...
  Flame 的 streamable-HTTP binding 对无法解码的 request 返回 transport problem，不再生成带 `null` id 的 JSON-RPC error envelope。
...
| `400`  | HTTP body 读不出，或 JSON / envelope shape（含 duplicate/unknown member、request/response 混合、空 method、非字符串显式 id、client response）无法解码成 Flame request/notification。 |
...
> 解码成功，method / params 的一切问题都是 `200` + JSON-RPC error。Flame 的 string-only id 是 envelope decode 约束，
...
`urn:flame:transport:*` 判别键，`requestId` 与响应头 `Request-Id` 相同。客户端不得把 transport problem
误当作 JSON-RPC `error`。例如 JSON 无法解码为 JSON-RPC message 时返回 `400` +
`urn:flame:transport:invalid_request`，而已经解码成功后的 method / params 错误仍返回 `200` JSON-RPC error。
