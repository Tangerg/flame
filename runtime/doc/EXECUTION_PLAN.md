...
Q3/Q4 本轮结构 Goal 已闭合：Runtime 与 CLI 的高置信转发层、重复表示和微包候选均已迁回真实 owner，剩余单消费者 package 都拥有可证明的外部边界或生命周期。后续只有新的真实 consumer、可复现缺陷或失败反例才能重开结构批次，不再按文件数、目录数或静态候选继续改动。Q5 只处理真实 TUI flow 暴露的信息层级或交互问题，不按参考实现或像素差异机械重写。
