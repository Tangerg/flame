...
| 批次 | 目的 | 状态 | 验收 | 提交 |
| --- | --- | --- | --- | --- |
| Q0 | 建立根与模块规则，收敛文档 owner，删除 CLI 点时台账 | 已完成 | 链接检查；Runtime 文档/SQLite 事实门禁；CLI 架构门禁；CLI 随附配置示例由自身严格 loader 验证，Runtime 示例不再复制 provider catalog；CLI 入口文档明确分离 consumer preferences 与 Runtime 执行配置 | `f9cab5a`、`94e45e7`、`6c9ebc3`、`1600127` |
...
| Q6 | 继续收敛 Runtime/CLI 平铺式 package 与退役路径 | 已完成 | 已删除前批遗留的 20 个空目录；CLI Goal/feedback/usage/Agent Memory 收回 Agent context，authoring/Knowledge/Skills/Hooks/direct diagnostics 收回 Workspace context，portable document/transfer 收回 Session context，transport reconnect budget 收回唯一 retry mechanism；Runtime Conversation 编排收回 `application/agent/runs`，Plan/query/usage/feedback 收回 `application/agent/sessions`，direct diagnostics 收回 `application/workspace`，Skill library与path translation收回 `adapter/workspace`，catalog/client/Provider input/environment registry收回`adapter/model`，Session/working-tree admission与recovery election收回`application/ownership`；测试构造与内存fake收回唯一`internal/testsupport`机制package；第二次审计把 CLI 物理收敛为 Domain/Application/Adapter/Delivery 四环，并把 Runtime 各环内叶包按 Run、Session、Automation、Workspace、Integration、Toolset、Git、Process、Storage 等已证明 context 建立一层 namespace；最终命名审计把 CLI Runtime binding/Profile 收回唯一 `adapter/runtimebinding`，把 Runtime 辅助模型边界改为 `adapter/model/auxiliary`，删除 runs/sessions/http internal API 的重复 qualifier 前缀，并让Toolset直接消费已发布的Scope A2A lifecycle而不再保留转发Infra package；跨上下文机制保持直达，父目录不声明 facade package，旧路径、重复 helper/算法、过时别名、import、空目录与架构例外均删除；Runtime 90、CLI 27个internal Go package，空目录与空文件为零，拓扑与命名门禁阻止重新平铺、装饰性深嵌套或 package 口吃 | `f115040`、`5f2dc86`、`7d6b7eb`、`3627e85`、`722bf7d`、`7e308c3`、`597b065`、`87d583e`、`6ed8e77`、`6408f0d`、`9c4007c`、`24464ae`、`265ca3f`、`8a9953f`、`6e677fe`、`8545465` |

## 当前下一步

Q6 经用户要求的层级与命名复审后完成。Runtime/CLI package graph 先删除不能证明独立性的叶包，再让保留的 package 以真实 context map 形成浅层物理层级，并让 import path/qualifier 各自只陈述一次 owner；没有通过含糊改名破坏 `run.Run` 等领域语言，也没有通过父 facade、single-child namespace 或 compatibility import 伪造层次。当前下一步只剩独立的 Q5 TUI 体验工作，它必须由真实交互反例驱动，不与已闭合的结构批次混改。
