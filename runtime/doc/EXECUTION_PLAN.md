...
| Q4 | 收敛 Runtime owner、抽象与 corner cases | 进行中 | endpoint provider 环境凭据、URL 校验与单消费者假边界已根治；Google chat 与 embedding 现在共用同一 custom endpoint truth，真实 HTTP 请求已验证不再静默回落官方地址；Runtime YAML 使用闭合输入词汇，未知或拼错字段不再静默回落默认值；继续审计 recovery/lifecycle | `c0cd320`、`18413c7`、`2a3c7de`、`2bdf819`、本批提交 |
