...
| Q6 | 继续收敛 Runtime/CLI 平铺式 package 与退役路径 | 进行中 | 已删除前批遗留的 20 个空目录；CLI Goal/feedback/usage/Agent Memory 收回 Agent context，authoring/Knowledge/Skills/Hooks/direct diagnostics 收回 Workspace context；Runtime Conversation 编排收回 `application/runs`，Plan/query/usage/feedback 收回 `application/sessions`，direct diagnostics 收回 `application/workspace`；旧路径、重复 helper、import、空目录与架构例外均删除 | `f115040`、`5f2dc86`、`7d6b7eb`、`3627e85`、`722bf7d`，CLI Agent Memory 本批 |

## 当前下一步

Q6 已重新打开结构收敛：旧结论把“单个 package 能讲出理由”误当成整体 package 图已经合理，没有继续验证同一 bounded context 的共同变化、调用路径和命名成本。CLI Agent 与 Workspace context、Runtime Run/Conversation、Session 与 Workspace Application owner 已先闭合；下一批继续审计 CLI 剩余 projection 与 Runtime Adapter 中只按处理阶段或单一 operation 存在的 package。每批同时删除退役路径、空目录、旧守卫和文档引用。Q5 只处理真实 TUI flow 暴露的信息层级或交互问题，不与结构批次混改。
