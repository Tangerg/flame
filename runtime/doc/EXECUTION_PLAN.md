...
| Q6 | 继续收敛 Runtime/CLI 平铺式 package 与退役路径 | 进行中 | 已删除前批遗留的 20 个空目录；CLI Goal/feedback/usage 收回 Agent context；Runtime Conversation 编排收回 `application/runs`，Plan/query/usage/feedback 收回 `application/sessions`；旧路径、重复 helper、import 与架构例外均删除 | `f115040`、`5f2dc86`，Session 批次待提交 |
