...
>
> 适用范围：`desktop`、`desktop/frontend`；只有真实产品反例证明权威事实缺失时，才进入 `runtime`。`app/cli` 明确不在范围内。
>
...
- Codex 作为主参考，优先复用其页面心智模型和交互反馈；视觉数值必须落入 Flame 现有 token、组件和可访问性体系；
...
像素级复刻不是复制源码。每个采纳点必须说明它在 Flame 中由哪个组件、token 或 presentation owner 承担。
...
- 参考结果必须先翻译为 Flame 的 Domain、Application、Adapter、Infra、Delivery 或 Desktop owner，再决定是否采纳。
...
5. 参考采纳点和拒绝理由可追溯，最终实现符合 Flame 的领域边界和单 Desktop 产品约束；
