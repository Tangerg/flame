...
> **来源**：对 **Grok Build**（`grok`，xAI/SpaceXAI 的终端 AI coding agent，Rust，~70 个 `xai-*` crate 的 Cargo workspace；桌面克隆 `~/Desktop/grok-build`，其 `crates/codegen/xai-grok-pager/docs/user-guide/` 24 篇是可信的能力索引）的源码级对比分析。Grok Build 与 flame 是**高度同构的两个生产 coding agent 运行时**（TUI/headless/ACP 三形态 vs flame 的 desktop/headless）；本文档只收录"它工程化更细、而 flame 值得搬"的点子，过 scope 哲学筛子后落地。
>
...
1. **"Computer-Hub" 不是 computer-use**。`xai-computer-hub-*` 是一套**多租户云端工具路由 fabric**（JSON-RPC/WebSocket + OIDC + 连接池 + replay + 遥测 donation），命名假朋友。grok 全仓**没有** GUI/浏览器自动化。flame 想要 computer-use，这里一点设计都借不到——必须另寻来源（Anthropic 式 computer-use 工具面挂在既有工具系统之后）。
2. **压缩算法 flame 不落后、反而领先**。grok 的 `xai-grok-compaction` 每次压缩必走 **full-replace LLM 摘要**；flame A3 的"先确定性裁剪、必要时才 LLM"更省。A1 tool-pair 安全双方同构。**不要被 grok 带偏去把压缩重构成 full-replace**。

## 0.1 筛选准则

沿用 scope 四道筛子：① **取思想、不取形态**（flame 用 Go + `iter.Seq2` + `context.Context`，不引 Rust/tokio/`nono`/gitoxide，不抄 TUI）；② **不为多租户云**（单进程 in-house 后端、单本地用户、单 SQLite；分布式/水平扩展/多端 leader/云分发一律不吸，但单用户**安全 confinement 是要的**）；③ **不引双机制债**（flame 刻意保持 skills/hooks/recipes/MCP/rules 分立，已删 queueStore 换 BeforeRound steer——重引须过高门槛）；④ **薄核优先**（运行时编排落 `runtime`，稳定 SPI 才下沉）。另守反向不变量：无 retry-layer/Transient 分类；用户填 provider key（不做 LLM-provider OAuth）；runtime 协议无 stdio transport。
...
- **目标**：给 flame 真实的、内核强制的执行边界，退役掉现在脆弱的"分类危险命令再自我否决"拐杖（B5/C3）。
...
- **计划**：
  1. 定 `SandboxProfile`（allow/deny 路径集 + network flag）+ 内置 profile；`.flame/sandbox.*` 解析 + 项目只能加规则。
...
- **风险/边界**：**架构分歧（必须尊重）**——grok 是一次性 CLI、启动时对**自身进程**一次性、不可逆 apply；flame 是**长驻多 session、跨 workspace** 后端，不能锁死 runtime 进程 → 必须**逐子进程 confine**。取"声明式 profile → OS confinement + fail-closed + 项目只能加"的思想，**不搬 `nono`**。bwrap re-exec 与 Seatbelt last-match 怪癖是"看起来沙箱实则没有" bug 的高发区，预算要留足。
- **优先级**：**P1（头号）**。
...
- **目标**：让 flame 跨会话记住项目约定/决策/调试经验，agent 自动召回，写路径受生命周期治理 + HITL 一致（对齐 B4）。
- **落点**：新 `memory` 子模块（SPI 形，feature-gated 默认关），**复用 flame 现成的 @codebase embedding/cosine**；`FLAME.md` 直接当 curated 层；dream/consolidation 由 session lifecycle（session-end + `/memory consolidate`）驱动。
- **计划**：
  1. 加 session-log 层（每会话一份 markdown）。
  2. dream consolidation：gate + lock + `NO_REPLY` 自限 + 只删已读；蒸馏进 `FLAME.md`。
...
- **落点**：runtime 压缩重建路径末尾——用运行时快照（running shells / todos / running subagents）生成独立 reminder item 注入；工具名从当前 toolset 解析。
...
- **落点**：新 runtime 子系统（per-session hunk tracker）；`workspace.hunks.*` RPC + 桌面"Review Changes"面板。归属键复用 flame 现有 `runId/segmentId`（≈ grok 的 `prompt_index`）。
...
- **落点**：runtime watcher（Go fsnotify + debounce + 小 HEAD/index/refs 状态机）。
- **风险/边界**：**只作"workspace 变更"的唯一真源替换现有显式 re-index，不叠第二条路**（否则双机制债）。价值门控于 G5——单独作"增量 re-index"nicety 时优先级低（flame 刻意选显式）。
- **优先级**：**P2（G5 采纳则升 P1）**。
...
- **目标**：把 flame **parked 的 `ToolReturn.Media`** 落地，且干净区分 model-view / UI-render / raw-data 三个消费者。
...
- **目标**：把 flame 的一次性 subagent 委派升级为**可组合的流水线阶段**（explore→review→fix 无需硬编码自动串联）。
- **落点**：给 flame 的 subagent/agent 定义加可选 `inputs`/`outputs`（name/type/required/description）；task 工具描述里 surface 出来供编排 agent introspect。
...
- **落点**：从 flame 现有 safety class 派生 `ToolScope`；在 subagent coordinator 强制"Write ⇒ leader session only"。
...
- **落点**：runtime 压缩触发与摘要路径。
- **验收**：短摘要触发重试；超窗前主动压；触发点留 headroom。
- **风险/边界**：**切勿连 grok 的 `Deterministic/Transient` 重试分类一起抄**——那正是 flame 反向不变量禁的 retry-layer；只要"质量门/降级输入/提前压"这几个决策，不要那台分类机。input-ladder（超窗历史降级喂给 summarizer）仅在确认 flame 存在"历史大到压不动"死局时再补（P3）。
- **优先级**：**P2**。
...
- **落点**：runtime 压缩重建——确保项目指令/AGENTS.md verbatim 重注入（带 resume 幂等 tag）+ 前次摘要里的用户原话骨架在再压缩前剥离单独携带。
...
- **来源/为什么**：`xai-interjection-core` + host `interjection.rs`。interjection 在 turn 安全点 drain、framing 成独立 synthetic user message 注入、**永不取消 turn**；关键：若在 idle 时或最后一次 drain 之后到达（错过 turn），转成一个 queued prompt turn（`flush_stranded_interjections`）而**不静默丢失**。framing **不加 defer 指令**、把权衡交给模型（与 flame 哲学一致）。
- **目标**：flame BeforeRound steer 若在"turn 已无后续 round"时到达，应降级为下一 prompt turn 而非丢弃；并考虑 interject/send-now 的显式二分。
- **落点**：核对并加固 flame BeforeRound steer 的 stranded 兜底。
...
- **落点**：flame MCP HTTP client。
- **风险/边界**：廉价健壮性，与 flame 现有 MCP gating 平级、无冲突。
- **优先级**：**P2**（直接做）。
...
- **优先级**：**P2**（若 flame 导出遥测到任何非本地 sink 则升 P1）。
...
- **风险/边界**：廉价、自包含、与 PTY 无关（独立于 flame 的 no-PTY 决策）。
- **优先级**：**P2**。
...
- **目标**：对 flame 现有 config/approval loader 做一次安全核查。
...
- **来源/为什么**：`xai-grok-hooks`（14 个 seam）。对照 flame 现有 seam，补 **PreCompact / PermissionDenied / SubagentStart·Stop**（若 flame 跑 subagent/compaction 而缺）；确认**单一 folder-trust 权威**统一门控 hooks+MCP+LSP。
- **目标**：hook seam 覆盖不漏关键生命周期点。
- **落点**：现有 hooks 模块（无新机制）。
- **风险/边界**：flame hook 设计已等价，**别重构**；HTTP hook = skip（单用户 subprocess 足够）；plugin-bundle 统一 = 不吸（见刻意不吸）。
- **优先级**：**P3**。
...
- **落点/计划**：**现在只借设计**——给 flame 自有 runtime→desktop 事件流一套 typed `session/update` 分类；把 file read 建模成能返回 **未保存 buffer** 内容的 host capability。**线协议本身 defer**——只有编辑器触达成为产品目标时，才加 additive `flame agent stdio`（Go ACP SDK `coder/acp-go-sdk`），**绝不重写桌面协议**。
- **风险/边界**：**不是**双机制债（方向=外部编辑器驱动我的 agent，异于 flame 桌面协议与 a2a，反向不变量"runtime 协议无 stdio"也不违反——ACP-over-stdio 是对编辑器说的另一套协议）。但 grok 自己证明"上 ACP 不消除自有协议、只是 rebase 成 `x.ai/*` 扩展"——win 是产品/分发决策、非工程免费午餐。
...
- **风险/边界**：**关键 filter #3**——它与被删的 queueStore（mid-run **转向**）正交不重叠，过得了高门槛；但 grok 实现重度绑定**多端 leader 模式**（broadcast + owner + last_editor + version-LWW），那是 filter #2 要砍的复杂度，**只留单用户本质**。**务必先证伪"flame 当前 run 进行中提交 prompt 已有等价 type-ahead"再动**，避免重造被删的机制。
- **优先级**：**P3，且先证伪**。
...
- **落点**：flame MCP client（device/authorization-code + 凭证库）。
...
| 项 | 来源 | 不吸理由 |
|---|---|---|
| **computer-hub fabric** | `xai-computer-hub-*` | 多租户云工具网（OIDC/连接池/replay/donation），**非** computer-use；违 filter #2/#4。flame 的"远程工具"=MCP 已覆盖。若真需 builtin-vs-MCP 重名规则，用 local-shadows-remote 一行策略 |
| **plugin marketplace** | `xai-grok-plugin-marketplace` | 本质多作者云分发、骑 Anthropic plugin 生态；且逼你把 hooks/skills/recipes/MCP 揉成 "bundle"=双机制债。用户要 skill 包可自行 `git clone` 进 `.flame/skills/` |
| **circuit breaker + Retryable/Terminal 分类** | `xai-circuit-breaker` | 捆绑的分类器正是 flame 反向不变量禁的 Transient/NonTransient retry-layer；breaker 本体需并发/规模（grok 只用于云 blob 上传队列），单用户无 consumer。真出现"死 MCP 端点被紧循环猛敲"再考虑窄 per-endpoint breaker（future bug fix，非现建） |
| **PTY** | `ptyctl`/`ptyctl-cli` | 重申 flame 已权衡后选 background commands；screen-grid 仿真 + HTTP 控制面重子系统、收益 niche（KISS/YAGNI） |
| **system-power** | `xai-system-power` | 动机是保护 OAuth refresh token 跨 suspend——flame 无 OAuth、run 本就 durable 可跨重启恢复 |
| **结构化 codebase-graph 作 agent 上下文** | `xai-codebase-graph` | name-based（非类型解析）比 flame LSP 更不准；**grok 自己都只给编辑器人工导航、agent 走单独 LSP**——正是 flame 已有（LSP + 语义嵌入）。缺全仓符号表就给 LSP 加 `workspaceSymbol` op（治本在正确层），不另造图。仅纯前端 serverless 导航时才 defer 级"部分吸"（LOW） |
| **fast-worktree / gix-status** | `xai-fast-worktree`/`xai-gix-status` | YAGNI（flame 无并行 worktree 隔离需求）/ gitoxide+panic=abort 特有 mitigation |
...
| **agent-lifecycle registry / token-estimation / sampling 结构** | `xai-agent-lifecycle` 等 | flame 已覆盖且更强（durable resume + 真 tokenizer + per-run model 配对） |
...
| **full-replace 压缩策略** | `xai-grok-compaction` | flame A3"确定性优先、必要才 LLM"更省，别倒退成"每次必 LLM" |
