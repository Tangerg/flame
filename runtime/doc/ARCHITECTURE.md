...
> 状态：已接受的重构目标设计
>
> 适用范围：`runtime` 及其为完成服务端重构必须调整的直接后端依赖

本文定义 Flame Runtime 重构完成后的稳定架构、统一语言、所有权和依赖方向。它不记录逐批进度，不枚举完整协议字段，也不复制 Agent Framework 的内部设计。

## 1. 定位

Flame Runtime 是产品应用后端，不是第二个 Agent Framework。
...
| `Knowledge` | 用户可编辑的 home/projectRoot/cwd `FLAME.md` 级联 | Conversation、Agent memory index |
...
LSP 的 document synchronization 是一次有界内存传输，不是通用文件读取旁路。唯一 `ensureOpen` 入口最多接纳 8 MiB：先用 stat 快速拒绝已超限文件，再以 cancellation-aware `limit+1` reader 防读取期间增长；只有完整通过包络的 bytes 才能进入 digest、字符串转换、`didOpen/didChange` 和 open-document state。Workspace file API 的编辑器窗口包络与 LSP 固定同步包络属于不同消费者，不得互相推断或让语言服务器绕过资源上限。
...
Model file mutation 的 post-processing 也属于 Runtime adapter 资源边界。read-before-mutation stamp 对完整文件做 64 KiB chunk 的 cancellation-aware streaming SHA-256，不把文件复制进独立 `[]byte`；无法读取当前 regular file 时 fail closed。成功删除必须撤销 stamp，成功创建/修改才记录新内容，外部以相同 bytes 重建的路径不能继承已删除资源的授权。Auto-format 只接纳最多 8 MiB 的完整文件：Go/JSON 在进程内形成完整结果，Prettier 只通过 stdin/stdout 变换 bytes，不取得 `--write` 权；stdout 最多 8 MiB、stderr 只保留 64 KiB 且继续 drain，caller cancellation 与 `WaitDelay` 共同闭合进程等待。只有 formatter 成功且完整输出通过包络后才原子替换文件。

Runtime 的 model/direct `read` 使用独立 consumer adapter，不把通用 `tools/fs.LocalExecutor` 的“零表示无限”带入产品。单文件最多 8 MiB、单行最多 1 MiB，默认一次结果最多保留 1 MiB 且只在完整行边界截断；`EndLine` 指向最后一条真实返回的完整行，调用方可从下一行继续。Reader 以 64 KiB buffer 流式扫描并在整个文件范围验证 UTF-8/NUL、BOM/CRLF、总行数、读取期间增长和 caller cancellation，最多只扫描已准入的 8 MiB。`Truncated` 同时表示前缀/后缀或 result budget 被省略，不再把从中间读到 EOF 伪装成完整文件。read-before-mutation tracker 以相同 8 MiB 包络在 Tool call 前后计算全文件 streaming digest；只有两次 identity 相同才提交 stamp，读取后被替换的文件不能取得未见内容授权。

公共 `workspace.files.read` 是 Desktop/editor-facing 的另一种 bounded consumer，不再委托通用 `tools/fs` 先 `os.ReadFile` 再截断。`application/workspace` 唯一规定默认 1 MiB、最高 8 MiB result，最多 64 MiB complete source scan 与 8 MiB single line；`maxBytes=0` 选择默认值，过大的 caller budget 被收敛到最高值。`tools/textread` 是唯一 scanner owner，以 64 KiB buffer 共享 context、UTF-8/NUL、BOM/CRLF、trailing empty line、growth 与 total-line 验证，但由消费端选择输出策略：模型读只返回完整行，编辑器读允许最后一行在有效 UTF-8 边界发布 prefix。Application 对 port result 再验证 output bytes、UTF-8/NUL、line window/content correspondence 与 truncation honesty，使替代 adapter 不能绕过合同。`Truncated`、实际 window 与超出文件的 range 必须诚实；binary/invalid UTF-8 映射为 `unsupported_mime`。无 truncation 字段的 `workspace.files.head` 默认 200、最高 400 行，结果预算不足时整体失败。Desktop 打开带行号引用时读取目标前后各 200 行并使用响应 `startLine` 保持真实 gutter identity，不因默认前缀预算丢失深处目标。

公共 `workspace.files.search` 是另一个 Application-owned read model，不再借用 model-facing `tools/fs` executor 或外部 `rg/grep` 的 stdout shape。`query` 是最多 64 KiB、在 port 调用前编译成功的 RE2-compatible regular expression；零/最高 retained match 数为 100/1000。Adapter 复用 Workspace file listing 的 ignore-aware、20,000-candidate 完整目录，在 64 KiB buffer 上逐文件扫描：单个 searchable UTF-8 regular file 最多 8 MiB、单行最多 1 MiB、一次请求实际扫描最多 512 MiB，只保留最多 8 MiB 的完整匹配行前缀。Binary、invalid UTF-8、超大文件与病态长行不属于 searchable text corpus；目录或 aggregate source 越界则明确失败。`Total` 在整个 admitted corpus 上继续精确计数，不再从截断后的第二次 count 推断；Application 复验 match count/material、root-relative path、行号、排序、文本与 regex correspondence，替代 port 不能发布不安全或不诚实前缀。Desktop 已有 `total > matches.length` overflow presentation 直接消费该语义，不建立第二套搜索状态。

Model/direct `glob` 与 `grep` 现在也消费同一有限 Workspace catalog/text corpus，但它们仍是 Runtime Tool adapter，而不是把公共 Workspace use case 反向暴露给 Agent。两者在任何目录访问前限制 4 KiB glob、64 KiB Go/RE2 regex、4 KiB path 与 100/1000 retained rows，并把 path 解析为 canonical root 内的 workspace-relative identity；两种最终 encoded Tool result 都不超过 1 MiB，grep 的前置 corpus scan 继续服从 8 MiB retained rows 与 8/1/512 MiB file/line/scan 包络。两种响应都给出 exact `total` 与由 `total > retained` 推导的 `truncated`，不再让 `max_results` 只约束子进程退出后的 slice。`grep` schema 只保留 `pattern/path/max_results`：case flag 使用 RE2 inline flag，文件集合先用 glob，命中上下文再用 read；`file_glob/file_type/ignore_case/multiline/before_context_lines/after_context_lines/output_mode` 这些外部进程特性或重叠组合已删除。`glob` 同样只保留 `pattern/path/max_results`，完整 segment `**` 相对所选 path 匹配。`tools/fs.LocalExecutor`、`rg/grep/find` 与 Tool-result offload 均不再处于该模型搜索的资源准入链上。

Lifecycle Hook 的 `hooks.json` 同时形成 `hooks.list` 非分页管理目录与每次 Run 的可信执行策略，因此也是有限 complete cascade。Hook Domain 唯一规定每份配置最多 256 KiB、每文件最多 128 条、global + project 完整级联最多 256 条；matcher 最多 256 bytes、command/inject 最多 8 KiB、command timeout 最多 5 分钟，全部文本必须是有效 UTF-8。filesystem loader 在 JSON decode、Hook materialization、管理投影与 Run binding 前执行 stat fast rejection 和 cancellation-aware `limit+1`，读取期间增长或任一越界使完整级联失败，不发布截断前缀，也不让 HTTP body、shell timeout 或前端页面承担隐式容量。

Lifecycle Hook command 是外部进程边界，不是普通 in-process callback。每个 stdout/stderr 各以 64 KiB bounded-drain writer 完整排空但只保留有限前缀；stdout 只能是空输出或单一 UTF-8 JSON decision object，unknown field、unknown verdict、多值、畸形或 overflow 都形成可观察的 broken-hook failure，不静默改写为 allow。非 2 退出和输出合同失败保持既有 non-blocking policy，明确 exit code 2 仍 fail closed 为 deny 且 reason 只来自有界 stderr/decision。Unix command 在独立 process group 中执行，timeout/cancellation 与返回后 cleanup 都终止整组后代；`WaitDelay` 只负责给异常遗留 pipe 一个有限 join 上限，不充当 lifecycle owner。

模型 Shell 的 foreground/background 是一个 owner 内的展示状态，不是进程脱管许可。每次 command 从 Start 前就由 `Shells` ledger 拥有；Unix leader 独占 process group，timeout、`stop_shell`、foreground cancellation、Host `KillAll` 与 leader 自然退出后的 final cleanup 都终止整组并 join `cmd.Wait`。自然退出的 leader 若被后代持有 pipe 触发 `WaitDelay`，仍保留真实 leader exit code；cleanup diagnostic 是 terminal Outcome 的一部分，并由 foreground result、`read_shell_output` 或 Host shutdown 中至少一个仍有 owner 的边界观察。foreground command 只有在 Done 关闭后才能从 ledger 删除。

Isolated Workspace command 的 stdout/stderr 也是不可信进程流。Sandbox runner 对每条 stream 完整 drain，但最多保留 256 KiB 并附加明确 truncation marker；有界 writer 必须私有组合 storage，不能通过匿名嵌入暴露 `io.ReaderFrom` 等绕过 `Write` 的快路径。该限制在 `os/exec` 建立 pipe 之前已由具体 writer shape 保证，不能依赖 Tool result、timeout 或命令结束后的 slice。

Isolated Workspace 的初始 working tree 也是一次资源传输，不是 snapshot/archive 产品。Runtime 只在同一进程内把 source 复制到新建 scratch directory，因此不建立 tar `[]byte` 中间真相源；source/destination 都以 `os.Root` confinement 打开，单次 walk 直接流式写入目标。完整 source 最多 100,000 entries、单个 regular file 128 MiB、aggregate content 512 MiB，文件使用固定 64 KiB buffer 并在 opened identity、size、growth 与 caller cancellation 上 fail closed；relative symlink 必须保持在 workspace 内，directory mode 在 children 完成后由深到浅恢复。scratch destination 若物理位于 source 内必须在 walk 前拒绝，不能让 materialization 递归消费自己的输出。

Lifecycle Hook command input 也不是把现成 Runtime 字符串直接 `json.Marshal` 后交给 shell。Hook Domain 唯一规定 prompt 与 lossless canonical arguments 各最多 256 KiB、Tool/Subagent result 最多 128 KiB、reason/description/error 最多 8 KiB；Application 只为实际命中的 command hook 懒生成一次 ownership-isolated projection，prompt/result 超界时发布有效 UTF-8 前缀与 explicit truncation marker，arguments 作为 effect policy input 绝不截断成另一项动作。Shell 在 JSON allocation 前先做 512 KiB raw-material preflight，编码后再以 512 KiB exact stdin envelope 拒绝 escaping/shape expansion，失败发生在 timer 与 process spawn 前并进入既有 observable broken-hook path；纯 declarative inject 不承担 command projection 或 process cost。
