...
Lifecycle Hook 的 `hooks.json` 同时形成 `hooks.list` 非分页管理目录与每次 Run 的可信执行策略，因此也是有限 complete cascade。Hook Domain 唯一规定每份配置最多 256 KiB、每文件最多 128 条、global + project 完整级联最多 256 条；matcher 最多 256 bytes、command/inject 最多 8 KiB、command timeout 最多 5 分钟，全部文本必须是有效 UTF-8。filesystem loader 在 JSON decode、Hook materialization、管理投影与 Run binding 前执行 stat fast rejection 和 cancellation-aware `limit+1`；decode 只接受 document 与 hook 的闭合字段词汇，unknown field 不能被忽略成空策略或更宽 matcher。读取期间增长或任一 unknown/越界输入使完整级联失败，不发布截断前缀，也不让 HTTP body、shell timeout 或前端页面承担隐式容量。
...
内部 `delivery` 根 package 拥有一个内聚的 Runtime 交付边界：`Endpoint`、operation catalog、`Handler` 和 protocol projection。这些部分围绕同一份方法身份、能力门禁和 Application 翻译共同变化，优先以多个职责命名文件表达，不再拆成一个文件一个 package。

- `Endpoint` 是唯一 binding-neutral 入口和 Delivery lifecycle owner，统一执行严格 validation、capability、idempotency、problem projection 与 stream replay attachment；
- `Handler` 只负责 Application use case 与 protocol value 的双向 projection，不是独立入口，不对 Bootstrap 暴露第二套关闭权；
...
└── internal/
    ...
    ├── delivery/              # Endpoint, catalog, Handler, presenters
    │   ├── dispatch/        # JSON-RPC mechanism
    │   └── transport/       # HTTP/SSE and envelope mechanism
    ├── bootstrap/
    ├── config/
    └── arch/
```
