...
Directory replacement during execution remains an integration gap in Scope v0.32.0: native edits retain the original directory authority, while Runtime's read stamps, protected-path checks, and formatting resolve workspace paths. These policies can therefore observe a different tree after the root is renamed and replaced. Closing this gap requires a released Scope capability that lets host policies share the executor's directory authority; reopening the executor or adding path-only identity checks does not establish that shared authority.
...
Recipe catalog observation uses the same bounded Markdown collection and global/project directories as discovery. External additions, content edits, deletions, and source-alias changes publish `recipes.changed`; consumers re-read `recipes.list` to apply project precedence. A missing alias target is an observable source state, so a cached catalog cannot conceal the query error until the target returns. Observers belong to the Runtime subscription and close with it.
