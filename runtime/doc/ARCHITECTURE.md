...
Scope tools v0.34.0 exposes patch execution and acknowledgements but no public pre-execution target query. Runtime therefore still reads prospective paths through `go-gitdiff` for approval, locking, and read-before-write policy. This duplicates interpretation of patch endpoints and can drift from the executor. Replace that interpretation when a released Scope contract exposes its validated prospective targets; removing it before that would remove the product guards.
