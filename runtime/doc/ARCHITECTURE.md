...
Delegates adapt only Runtime's task input to Scope Interaction. Their executions, checkpoints, and outputs remain Scope-owned. Root and delegated completion both project `interaction.Output` from the committed Process result. The parent receives that complete typed output, preserving reasoning, refusal, media, metadata, and direct Tool results; there is no Runtime reply envelope or in-memory reply cache.
