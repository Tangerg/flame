...
>
> 适用范围：只评价 `app2/runtime`，并为原版 `runtime` 的后续治本重构提供证据和候选方向
...
`runtime` 内按真实消费者和失败反例逐个纵切改造，不再进行第二次 big-bang rewrite。
...
这说明当前 Flame Protocol 的窄腰已经成熟，正确做法是保留它并只修复有反例的缺陷；它也说明 app2 并非真正从零
重建全部语义，较小体积不能直接归因于更好的架构。

## 3. app2 真正做得更好的地方

### 3.1 没有把 Codex 的 wire 搬进 Flame

这是 app2 最重要、也最应该保留的判断。app2 明确让当前 Flame dotted operations、typed registry、
...
| app2 经验 | 决策 | 迁回原版的形态 |
|---|---|---|
| 保留 Flame Protocol 窄腰 | 保持 | 不改 wire；继续由 canonical registry 和 generator 驱动 |
...
下一轮应在原版 `runtime` 内进行证据化、可回滚、逐纵切的治本重构：保留原版已经赢得的正确性，只删除无法证明
