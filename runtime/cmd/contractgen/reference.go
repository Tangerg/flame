...
// reference renders the human-readable projection of the Contract Registry.
//
...
// source. This file restates what the code does, and the drift gate proves it
// still does.
func reference(m manifest) string {
	var b strings.Builder
	writeReferenceHeader(&b, m.ProtocolVersion, len(m.Methods))
	writeMethodReference(&b, m.Methods)
	writeHTTPReference(&b, m.HTTPEndpoints)
	writeStreamingReference(&b, m.StreamingMethods)
	writeRunEventReference(&b, m.RunEventPolicy)
	writeUnionReference(&b, m.Unions)
	writeValueConstraintReference(&b, m.ValueConstraints)
	writeCapabilityReference(&b, m.CapabilityPolicy)
	writeInvariantReference(&b, m.SystemInvariants)
	return b.String()
}

func writeReferenceHeader(b *strings.Builder, protocolVersion string, methodCount int) {
	b.WriteString("# Flame Runtime — generated API reference\n\n")
	...
	b.WriteString("> method the Runtime does not serve. The adjacent JSON artifacts are the\n")
	b.WriteString("> machine-readable contract; this file is its mechanical human-readable index.\n\n")
	fmt.Fprintf(b, "Protocol `%s` · %d methods\n\n", protocolVersion, methodCount)
}

func writeMethodReference(b *strings.Builder, methods []methodEntry) {
...
func writeHTTPReference(b *strings.Builder, endpoints []httpEndpointEntry) {
	b.WriteString("\n## HTTP endpoints\n\n")
...
func writeStreamingReference(b *strings.Builder, methods []string) {
	b.WriteString("\n## Streaming methods\n\n")
	b.WriteString("A client reads this set instead of hardcoding names: these calls answer with\n")
	b.WriteString("their own event stream.\n\n")
	for _, name := range methods {
		fmt.Fprintf(b, "- `%s`\n", name)
	}
}

func writeRunEventReference(b *strings.Builder, events []eventEntry) {
	...
	b.WriteString("state is still correct.\n\n")
...
func writeUnionReference(b *strings.Builder, unions []unionEntry) {
	b.WriteString("\n## Discriminated unions\n\n")
	b.WriteString("Discriminated by `type` with no exceptions. A field belongs to at\n")
...
func writeValueConstraintReference(b *strings.Builder, entries []valueConstraintEntry) {
...
func writeCapabilityReference(b *strings.Builder, entries []capabilityEntry) {
	b.WriteString("\n## Capability gating\n\n")
...
func writeInvariantReference(b *strings.Builder, invariants []invariantEntry) {
	b.WriteString("\n## System invariants\n\n")
