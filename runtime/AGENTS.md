...
- Domain packages own aggregates, value objects, invariants, and pure transitions. Application packages own cohesive use-case families and cross-aggregate ordering. Neither depends on protocol, storage, provider SDKs, or delivery types.
...
- Provider/model selection is exact and durable. Provider SDK types, credentials, endpoints, and provider-specific errors stay behind provider adapters.
...
- Prefer real Runtime end-to-end scenarios for Goal, Plan, steer, HITL, compaction, provider failure, restart, and recovery. Test concurrency only where Runtime owns concurrent lifecycle.
