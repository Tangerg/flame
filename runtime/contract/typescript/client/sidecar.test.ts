...
describe("SidecarClient", () => {
  it("info() returns public bootstrap metadata", async () => {
    const fetchStub = makeFetch(200, {
      ...
      server: { name: "flame-core", version: "0.8.1", instanceId: "runtime_1" },
      transport: "http",
      endpoints: {
        rpc: "/v2/rpc",
        info: "/v2/info",
    ...
    expect(info.server.name).toBe("flame-core");
  ...
  it("preserves problem details on sidecar failures", async () => {
    const client = createSidecarClient({
      baseUrl: "http://x",
      fetch: makeFetch(500, {
        type: "urn:flame:transport:internal_error",
    ...
    await expect(client.info()).rejects.toMatchObject({
      status: 500,
      requestId: "req_123",
      problemType: "urn:flame:transport:internal_error",
  ...
  it("rejects sidecar JSON that violates the endpoint contract", async () => {
    const info = createSidecarClient({
      ...
      fetch: makeFetch(200, {
        protocolVersion: PROTOCOL_VERSION,
        server: { name: "flame-core", version: "0.8.1" },
        transport: "ipc",
        endpoints: {},
      }),
    });
