...
import { createMethods } from "@flame/runtime-contract/client/methods";
import {
...
describe("methods factory", () => {
  it("retains the exact mutation input and key across automatic and explicit replay", async () => {
    ...
    const call = vi.fn(async (_method: string, params: unknown, options?: RpcCallOptions) => {
      ...
      if (attempts.length < 3) throw new RpcTransportError("connection lost");
      return { id: "ses_1" };
    });
    const methods = createMethods({ call } as unknown as RpcClient);
    const params = { title: "original", workspace: { path: "/original" } };
    const mutation = methods.sessions.create(params);
    params.title = "edited before dispatch";
    params.workspace.path = "/edited";
    await expect(mutation).rejects.toBeInstanceOf(RpcTransportError);
    params.title = "edited before retry";
    await expect(mutation.retry()).resolves.toEqual({ id: "ses_1" });
    expect(attempts).toHaveLength(3);
    expect(attempts.every((attempt) => attempt.key === mutation.idempotencyKey)).toBe(true);
    expect(attempts.map((attempt) => attempt.params)).toEqual(
      Array.from({ length: 3 }, () => ({
        title: "original",
  ...
  it("honors the server backoff while the original mutation is in progress", async () => {
    ...
    const attempt = methods.sessions.create({
      title: "same",
      workspace: { path: "/repo" },
    });
  ...
  it("keeps concurrent runtime subscriptions isolated by their response stream", async () => {
    ...
    const sessionsPromise = methods.runtimeEvents.subscribe({
    ...
    const schedulesPromise = methods.runtimeEvents.subscribe({
