...
import { createPushPullChannel } from "@flame/runtime-contract/client/channel";
...
describe("createPushPullChannel", () => {
  it("tryPush reports bounded saturation without retaining the rejected value", async () => {
    ...
    await expect(ch.iterator().next()).resolves.toEqual({
      value: 1,
      done: false,
    });
  ...
  it("keeps the first terminal state", async () => {
    ...
    expect(await closed.iterator().next()).toEqual({
      value: undefined,
      done: true,
    });
  });
