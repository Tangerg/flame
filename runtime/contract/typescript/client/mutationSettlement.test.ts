...
} from "@flame/runtime-contract/client/mutationSettlement";
...
describe("unary mutation settlement", () => {
  ...
  it("replays a timed-out attempt with the same logical mutation", async () => {
    ...
    const result = createMutationSettler().settle("test:timeout-replay", open, {
      timeoutMs: 10,
    });
  ...
  it("settles after the finite retry budget even when the transport ignores abort", async () => {
    ...
    const result = createMutationSettler().settle("test:ignored-abort", open, {
      timeoutMs: 10,
    });
