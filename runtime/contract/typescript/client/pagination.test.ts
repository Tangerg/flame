...
import {
  ...
  PaginationError,
  PaginationViolation,
} from "@flame/runtime-contract/client/pagination";
...
describe("auto-paging promise", () => {
  ...
  it("rejects an oversized initial cursor before transport I/O", async () => {
    ...
    await expect(call).rejects.toMatchObject({
      violation: PaginationViolation.cursorTooLarge,
    });
  ...
  it("bounds retained cursor material, page rows, and array collection independently", async () => {
    ...
    await expect(
      ...
      violation: PaginationViolation.cursorRetentionExceeded,
    });
    ...
    await expect(
      ...
      violation: PaginationViolation.pageRowsExceeded,
    });
    ...
    await expect(
      ...
      violation: PaginationViolation.collectionCapacityExceeded,
    });
  });
});
