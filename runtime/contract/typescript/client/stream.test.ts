...
import {
  ...
  streamRunEvents,
  streamRuntimeEvents,
} from "@flame/runtime-contract/client/stream";
import type {
...
describe("streamRunEvents — response ownership (bound)", () => {
  ...
  it("an owning EOF without root completion reports observation loss", async () => {
    ...
    emitDown();
    await expect(consume).rejects.toBeInstanceOf(RpcConnectionError);
  ...
  it("remembers a stream-end that arrives before the call result is bound", async () => {
    ...
    await expect(next).rejects.toBeInstanceOf(RpcConnectionError);
    expect(activeCount()).toBe(0);
  });

  it.each([
    { segment: "seg_root", run: "run_root", expected: { done: true, value: undefined } },
    { segment: "seg_child", run: "run_child", expected: expect.any(RpcConnectionError) },
  ])(
    "resolves a buffered $segment finish against the acknowledged root before interpreting EOF",
    async ({ segment, run, expected }) => {
      ...
      await expect(iterator.next()).resolves.toMatchObject({
        ...
        value: { segmentId: segment },
      });
      const end = await iterator.next().catch((error: unknown) => error);
...
describe("streamRuntimeEvents — response-stream ownership", () => {
  ...
  it("retains a Runtime EOF failure while immediately releasing listeners", async () => {
    const { client, emitDown, activeCount } = fakeClient();
    const stream = streamRuntimeEvents(client);
    stream.bindRequest("rpc_runtime");
    ...
    expect(activeCount()).toBe(0);
    expect(stream.requestSignal.aborted).toBe(true);
    await expect(stream.events[Symbol.asyncIterator]().next()).rejects.toBeInstanceOf(
      RpcConnectionError,
    );
  });
  ...
  it("isolates concurrent subscriptions on the same RpcClient", async () => {
    ...
    await expect(sessionsIterator.next()).rejects.toBeInstanceOf(RpcConnectionError);
    expect(activeCount()).toBe(2);
    emitDown("rpc_schedules", "runtime.subscribe");
    await expect(schedulesIterator.next()).rejects.toBeInstanceOf(RpcConnectionError);
    expect(activeCount()).toBe(0);
  });
