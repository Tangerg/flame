...
import { createRpcClient } from "@flame/runtime-contract/client/client";
import {
  RpcError,
  RpcProtocolError,
  RpcTransportError,
...
describe("RpcClient", () => {
  ...
  it("rejects a response carried by another request without settling that request", async () => {
    ...
    const sessionsPromise = client.call("sessions.get", {
      sessionId: "ses_01",
    });
  ...
  it("dispatches only validated published notifications", async () => {
    ...
    const next = vi.fn();
    const error = vi.fn();
    const unsubscribe = client.subscribe("notifications.runtime.event", {
      next,
      error,
    });
