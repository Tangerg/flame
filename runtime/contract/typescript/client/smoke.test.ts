...
} from "@flame/runtime-contract/client/transports/memory";
import {
  injectRunEvent,
  injectRunFinished,
  respondSuccess,
...
import { createRpcClient, type RpcClient } from "@flame/runtime-contract/client/client";
import { asItemId, asRunId, asSessionId } from "@flame/runtime-contract/client/ids";
...
describe("smoke: v2 end-to-end happy path", () => {
  ...
  it("discover → create → start → interrupt → resume → completed", async () => {
    ...
    respondSuccess(transport, resumeReq.id, {
      runId: "run_1",
      segmentId: "seg_2",
    });
