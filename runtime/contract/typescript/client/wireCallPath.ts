...
export function createWireCallPath(client: RpcClient, options: MethodsOptions): WireCallPath {
  const refuse = <M extends WireMethodName>(
    ...
    throw new RpcError({
      ...
      data: {
        ...
        requiredCapabilities: missing.map((name) => ({
          type: "feature",
          name,
        })),
      },
  ...
  const openMutation = <M extends WireMethodName, Result>(
    ...
    const preparedParams = structuredClone(params);
    ...
    try {
      reservation =
        requestedKey !== undefined
          ? undefined
          : options.mutationJournal?.reserve(method, preparedParams, preferredJournalKey);
    } catch (error) {
    ...
    const mutation = createMutationPromise(
      (idempotencyKey, attempt) => {
        ...
        return execute(idempotencyKey, { ...attempt, idempotencyNamespace }, preparedParams);
      },
