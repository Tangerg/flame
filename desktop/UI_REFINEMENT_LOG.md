...
## Round 207 —— 被宽度藏起来的界面，和一条自带证明的例外

上一轮的缝是"只有交互之后才存在"。同一条缝还有第二个形状：**只有某个宽度才存在**。
审计跑在默认视口（1120），而 narrative 的 turn rail **要到 1440px 才挂载** ——
所以一整个 `nav` 导航区从来没有被审计过。

把 12 条路线 × 2 个宽度全跑一遍 axe，只多出一处：

```
target-size  34x9  "You 1/3 · 8:00 AM"   （×3）
```

### 这一条改不动，也不该改

tick 是 `height: 9px`，而且是**故意的** —— 组件自己的注释写着
*"A 9px box for a 1px tick: the height is the rail's rhythm, not the mark's."*
它是一张缩略地图，一行一个 turn。

**在 9px 的节距下，没有任何排布能满足 2.5.8 的尺寸条款或间距条款** ——
24px 的目标在 9px 间距上必然互相重叠。所以这个 rail 只能靠**例外**成立，
无论怎么改（除非把节距拉到 24px，那样 50 个 turn 就是 1200px，地图就不成其为地图了）。

把 rail 改成一个复合控件（roving tabindex）**并不能解决** —— 2.5.8 管的是**指针目标尺寸**，
不是键盘。这一点我先想错了，验证之前就纠正了。

改 rail 的尺寸是结构性设计改动，不该由我单方面做。
**扩覆盖 + 记录例外不是**，所以这轮做后者。

### 例外自带证明

2.5.8 允许"同一页面上有另一个满足条款的控件能达成同一功能"。
所以例外不写成一句断言，而是**写成一条测试**：

```
每一个 rail 指向的 turn，都有它自己的可聚焦内容
  ticks=3  ≤  带自身控件的 [data-turn-id] 数量
```

用的是产品自己的 `data-turn-id` 标记，不是我猜的容器 ——
第一版我猜 `closest("article, section, li, div")`，量出来是 **0**，当场被这条测试自己打回。

例外的机制也做了约束：`{ rule, within, because }` —— **规则 + 容器 + 理由**，
不是整条规则关掉。只有落在 `nav[aria-label]` 里的 `target-size` 被豁免。

**验证过它是有承重的**：把例外拿掉 → 宽视口审计立刻红在 `target-size` 上。

### 顺带

途中发现**没有任何一个滚动容器在 Tab 顺序里**（`SCROLLERS reachable by Tab: 0`）。
这不构成 WCAG 失败（Chromium 只在滚动容器没有可聚焦子元素时才把它加进 tab 顺序，
而 transcript 有），但它否掉了我原本想用的那条等价论证 ——
所以等价性最后建立在"每个 turn 有自己的控件"上，而不是"可以聚焦滚动区再用方向键"。
**先想的理由不成立，换一个能证明的。**
...
| | 结果 |
| --- | --- |
| 被宽度藏起来的审计盲区 | 1 个 `nav` 导航区 → **0** |
| 新增审计 | **23 条**（每个 agent 状态一条宽视口审计）|
| 例外 | 0 → **1 条，带容器、带理由、带证明它成立的测试** |
| 视觉套件 | **709 全通过**（10.9m，零失败；+24）|
| typecheck / lint / prettier / knip / 7 个守卫 | 全绿 |
| 生产代码改动 | **零** |

### 留给你的一个判断

turn rail 现在靠 2.5.8 的"等价功能"例外成立，而这条例外的强度取决于
"tab 到某个 turn 的按钮"算不算"达成了跳到该 turn 这个功能"。
我的判断是算（那三个 tick 只是 `scrollToTurn` 的快捷方式），但这是个可以有另一种读法的地方。
另一条路是把节距拉到 24px —— 代价是 turn 多了地图就装不下。**要改随时说。**

## Round 208 —— 审计跑在一个只挂载了三分之一的页面上

本来是查同一条缝的第三个形状：**只有某个密度/字号才存在的界面**。
`density` 和 `font-size` 都是 URL 参数，审计只跑默认值。
5 条路线 × 5 个变体跑完 axe：**0 违规**。密度阶梯和字号阶梯本身是干净的。

但为了证明"探针真的在看"，我顺手打印了每个变体下的控件数 —— 于是掉出了真正的东西：

```
(default)         row=34px  controls=73
&density=compact  row=29px  controls=42
&font-size=11     row=39px  controls=68
```

**同一条路线，控件数在 42 和 73 之间跳。** 密度不该改变控件的数量。

### `data-visual-ready` 不等于"挂载完了"

```
fixture=workspace&state=settings   atReady=13  settled=73   path=13,68,68,73,73,…
```

审计开始时页面上有 **13 个控件**，最终 **73 个**。

真正跑审计的 `openFixture` 比我的探针多等两件事（标题可见 + `main section` 里没有
`aria-busy`），所以我先量了它：仍然不够。逐个面板量：
...
**根因**：`aria-busy` 消失说明这个面板的**代码块**加载完了，不是这个面板**渲染完了** ——
它自己的数据在那之后才到。**那个信号回答的不是被问的那个问题。**

### 修的过程里错了两次，两次都是量出来的

| 我做的 | 量出来 | 改成 |
| --- | --- | --- |
| 用 `main section :is(button,…)` 做 settle 的作用域 | 两个面板下这个数是 **0**，另一个是 3 —— 循环在一个永远不变的数上"settle"，还白等 3 秒 | 数**整页** |
| 等"两次读数相同" | 面板是**分批**到的，间隔大于一次采样：`13,68,68,73` —— 两次相同正好落在 68 那个平台上 | 等**连续四次**相同 |

改完十二个面板**全部 100%**。

### settle 放在哪里，也是一个判断

不放进 `openFixture` —— golden 也走它，全加一遍要给 11 分钟的套件加约 5.6 分钟；
而且 golden 如果和自己的内容赛跑早就 flaky 了，它们不是。
**放进 `expectNoWcagViolations` 本身**：审计的结论取决于它看到了多少，
所以这件事属于审计，不属于打开页面。这样连交互态审计（弹层刚打开）也一起覆盖了。

顺带量了非 settings 的 54 条路线：**12 条**在 ready 时仍在挂载（全是 dock 状态，
差 1–6 个控件，92–98%）—— 比 settings 轻，但同样真实，现在也一并覆盖。

### 结果：产品是干净的，不干净的是审计的自信

全部 **233 条 WCAG 审计在完整页面上重跑，零新增违规**。
这不是白做 —— 之前那句"通过"覆盖的是三分之一的 plugins 面板，现在它覆盖的是整个。

### 验收

| | 结果 |
| --- | --- |
| 密度 / 字号变体的违规 | **0**（这条线干净，已记录）|
| settings 面板的审计覆盖 | 30% / 34% / 59% / 90% → **全部 100%** |
| 挂载未完就审计的路线 | 12 / 54 → **0** |
| 新增违规 | **0** —— 产品本来就是干净的 |
| 视觉套件 | **709 全通过**（12.5m，零失败；settle 的代价 +1.6m）|
| typecheck / lint / prettier / knip / 5 个守卫 | 全绿 |
| 生产代码改动 | **零** |
...
## Round 210 —— 一个 token 坐在两块不同的底色上

上一轮的形状是「偏好只推动了一半的推导」。把 13 个外观偏好逐个过一遍，
`accent` 是最危险的一个：**它是用户自由选的颜色，而坐在它上面的墨是主题写死的 `#ffffff`。**

先把 `accent` 做成 fixture 参数，再量：
...
| `#ffe066`（淡黄）| **1.52** | **1.39** |
| `#a8e6a3`（淡绿）| **1.98** | **1.65** |
| `#00b3ff`（青）| 5.89 ok | **3.32** |

`--color-cta-text: var(--color-text-on-accent)`，所以每一个主按钮的文字都跟着一起失守。

### 第一版修错了，而且是量出来的

我先让墨跟着 `--color-accent` 走 —— **深色默认反而变差了**：`#ffffff` → `#000000`，比值从 5.65 掉到 3.72。

查下去才看见真正的结构：

```
--color-accent  = #3574f0        ← 主题的品牌色，白字 4.28
--color-cta     = #115beb        ← flame-dark 写的是 var(--color-accent-border)，另一个色阶
--color-cta-text = var(--color-text-on-accent)   ← 但墨只有一个
```

**一个 token 坐在两块不同的底色上，它不可能对两边都正确。**
而且两边要求的门槛还不同 —— checkbox 的勾和 switch 的拨杆是**图形**（1.4.11，3:1），
主按钮的标签是**文字**（1.4.3，4.5:1）。`#3574f0` 配白是 4.28：**作为图形合格，作为文字不合格**。
用一个门槛判两者，正好会把出厂就正确的那个判错 —— 这就是我第一版做的事。

### 治本：每一种墨跟着它自己的底色走，用它自己的门槛

```
--color-text-on-accent = inkOnFill(主题声明, 解析后的 --color-accent, 3)
--color-cta-text       = inkOnFill(主题声明, 解析后的 --color-cta,    4.5)
```

底色是**从浏览器解析后读回来的**，不是在这里重算每个主题的表达式 ——
所以一个把 CTA 定义成别的东西的调色板，被同样这两行覆盖。
主题自己的选择只要读得过就保留，所以**默认逐字节不变**（实测两个主题的墨都仍是 `#ffffff`，
比值 5.75 / 4.28 + 5.65，和改动前一致，727 项视觉套件零 golden 移动）。

### 顺带：自己写对比度，而不是 `extend()` colord

`colord` 的 `contrast()` 在 a11y 插件里，而 `extend()` **会改掉整个 app 共享的那个实例** ——
这正是仓库禁止的环境全局。核心只有 `brightness()`，那是知觉亮度，不是 WCAG 的相对亮度，
两者能把一对颜色判到线的两边。所以 `kit/legibility.ts` 自己写了这段线性化，
并且用**判据自己定义的两个锚点**（黑白 = 21、同色 = 1）钉住它。

### 新覆盖

- `accent` 成为 fixture 参数（和 `contrast` / `density` / `font-size` 并列）。
- **10 条新断言**：5 个 accent × 明暗，**两对分别按 3 和 4.5 判**。
  不走 axe —— 那个墨只有在主按钮恰好可用时才上屏，而它在 fixture 里缺席**不是证据**。
- `legibility` 的 9 条单测，含一条专门钉住「同一个底色，作为标签翻、作为标记不翻」。

**验证过会失败**：把 `inkOnFill` 改回「原样返回主题声明」→ 精确报出 `a mark on the accent`。

### 验收

| | 结果 |
| --- | --- |
| 用户可达的 accent 里失守的墨 | 淡色全线 1.39–3.32 → **0**（10 条全绿）|
| 一个墨 token 服务的底色数 | 2 → **各 1** |
| 默认外观 | **逐字节不变**，0 张 golden 移动 |
| 单测 | **134 全通过**（theme + `src/ui`，+9）|
| 视觉套件 | **727 全通过**（13.2m，零失败；+10）|
| typecheck / lint / prettier / knip / 9 个守卫 | 全绿 |

### 一句话

第一版我让墨跟着 accent 走，深色默认立刻变差 —— **因为按钮根本不坐在 accent 上**。
「跟着底色走」这句话是对的，但前提是先搞清楚**到底有几块底色**。

## Round 211 —— 百分比买到的是外观，不是对比度

`customTheme` 是 13 个偏好里最自由的一个：用户直接给 `bg` 和 `fg`，整条调色板由它们推导。
而 fixture 的 `theme` 只认 light / dark —— **自定义主题从来没有被任何测试打开过**。

把它做成 fixture 参数（`custom-bg` / `custom-fg`），量出来：

| 调色板 | soft | muted | faint |
| --- | --- | --- | --- |
| **内置浅色**（手写）| 10.27 | 6.60 | **5.75** |
| **内置深色**（手写）| 9.95 | 7.41 | **5.76** |
| 自定义 白/黑 | 20.3 | 6.83 | **3.11** |
| 自定义 = **产品自己的深色配色** | 10.09 | **4.19** | **2.43** |
| 自定义 暖浅色 | 9.37 | **3.47** | **2.12** |

**把产品自己的两个颜色喂进它自己的推导，出来的墨不合格。**
这是能拿到的最干净的证据：问题不在颜色，在推导。

### 根因

```
textMuted: mix(fg, p(45, 75), bg)     ← 离背景固定百分之几
textFaint: mix(fg, p(28, 52), bg)
```

**一个百分比买到的是一种外观，不是一个比值。** 手写主题是人挑的，挑的时候看着对比度；
推导是按比例走的，没有任何一步问过"这样读得清吗"。
而 `fgFaint` 有 **79 处**用途 —— section 标题、空状态、步骤行，全是真内容。

### 治本：每一档取"设计想要的比例"和"读得清的最小比例"里更大的那个

```
textFaint: mix(fg, legible(p(28, 52)), bg)
```

`legible()` 在 TS 里搜出最小的百分比，使得**混出来的颜色**对平面达到 4.5:1。
已经合格的档位原样返回 —— 所以读得清的调色板外观**一点不变**。

改完：产品自己的深色配色 muted 4.19 → **4.81**、faint 2.43 → **4.81**；白/黑 faint 3.11 → **4.85**。

### 为此要能预测浏览器会混出什么

推导发出的是 `color-mix(in oklab, …)` 字符串（故意的：拖动取色器时由浏览器实时解析）。
要按比值选百分比，TS 就得**预测**那个混合结果 —— 于是 `kit/legibility.ts` 里补了
sRGB ↔ Oklab 的往返。

**但拿自己的数学验自己的数学等于没验。** 所以新增一条视觉测试，
把同一个 `color-mix` 画进 1×1 canvas 再读像素 —— **整条转换由浏览器完成**，
35 组（5 对颜色 × 7 个百分比）逐一比对，容差 1 个通道。
（读 `getComputedStyle` 不行：它给回 `oklab(...)`，还得由我这边转换，那就绕回圆圈里了。
canvas 能解析 `color-mix` 并给出 sRGB 像素，是页面里唯一一处能做到的。）

**验证过会失败**：把混合权重扰动成 `(pct/100)^1.05` → 逐条报出 painted / predicted 的差。

### 这一轮我错了三次，每次都是量出来的

| 我做的 | 量出来 | 改成 |
| --- | --- | --- |
| 用 `getPropertyValue` 读墨的颜色 | 三档比值**完全相同**（21/21/21）—— 读到的是 `color-mix(...)` **文本**，canvas 解析失败静默变黑 | 通过真实元素解析 |
| 换成元素解析后仍然相同 | 计算值是 `oklab(...)`，**canvas 的 fillStyle 不认**，又静默变黑 | 画进 canvas 读像素 |
| 搜索时对 `plane` 比、发出时对 `bg` 混 | 守卫报 4.09 —— **我认证的比值和渲染出来的不是同一个** | `base` 与 `against` 分开传 |

第三条尤其值得记：**"在哪儿混"和"在哪儿读"是两件事**，混淆它们会让一个函数
认证一个它并没有产生的比值。

### 新覆盖

- `custom-bg` / `custom-fg` 成为 fixture 参数。
- **4 条**自定义调色板的墨梯断言（三档 × 对自己的表面）。
- **1 条** oklab 预测对齐浏览器的断言（35 组）。
- **5 条**新单测：两端、单调性、抬升、不抬升、以及"连 fg 本身都不合格时停在 100"
  —— 最后这条第一版选的 `#909090` 其实是 **5.10:1**（合格），93% 就够了，
  测试自己把我的假设打回来了。
...
| | 结果 |
| --- | --- |
| 自定义调色板里不合格的墨 | 2.12–4.19 → **全部 ≥ 4.5** |
| 内置主题 | **未触碰**（它们不走这条推导）|
| 单测 | **89 全通过**（theme，+5）|
| 视觉套件 | **732 全通过**（14.2m，零失败；+5）|
| typecheck / lint / prettier / knip / 10 个守卫 | 全绿 |

### 一句话

三次测量错误全都长一个样：**我读到的不是我以为的那个东西，而它没有报错，只是安静地给了黑色。**

## Round 212 —— 字体这条线是干净的，而我的守卫差点不是

偏好清单里最后一个会改变**尺寸**而不是颜色的自由输入：`uiFont`。
选择器列的是机器上装的字族，而产品里每个控件都从 `--control-height-*` 拿一个**固定高度** ——
换一个度量完全不同的字族会怎样，没有任何东西问过。

做成 fixture 参数（`ui-font`）后量，188 个控件 × 4 条路线：

| 字体 | 内容溢出控件的数量 |
| --- | --- |
| 内置 Geist | 1 |
| Helvetica / Verdana / Georgia / Times New Roman | 1 / 1 / 1 / 1 |
| Menlo / Optima / Futura / Avenir Next | 1 / 1 / 1 / 1 |
| **Zapfino** | **31** |

**九个真实可选的字族 —— 无衬线、人文、几何、衬线、等宽 —— 结果与内置字体一模一样。**
这条线是干净的，固定高度是字体健壮的。

那个恒定的 1 是滑块的 `input[type=range]`：原生 shadow 部件超出它自己的 14×14，
`overflow: hidden` 兜着，不画到外面 —— 和 Round 196 排除它的理由、措辞都一样。

Zapfino 会溢出（最多 11px，`overflow: visible`，也就是真的画到控件外面），
但它是脚本展示体，**任何固定高度都撑不住它**。所以它不进守卫，
而"为什么不进"写在守卫里，不是留成名单上的一个空缺。

### 我的守卫差点是瞎的

守卫必须先证明**字体真的生效了** —— 否则一个没装的字族会回落到 fallback，
于是"九个字体"其实测了九遍内置字体，还报绿。

第一版我用 `document.fonts.check(\`16px "X"\`)`。**验证的时候塞进一个
`NoSuchFamilyInstalled` —— 它通过了。** 这个 API 对不存在的字族返回 `true`。

改成量出来的办法：拿同一串字符分别按 `"X", monospace` 和 `monospace` 渲染，
**宽度相同就说明 X 没生效**。这个没法被糊弄。再塞一次假字族 → 精确报出
`NoSuchFamilyInstalled is not installed, so this route proves nothing`。
...
| --- | --- |
| 找到的缺陷 | **0** —— 这条线是干净的 |
| 覆盖的外观轴 | **+uiFont**（9 个字族 × 4 条路线 × 188 个控件）|
| 瞎掉的断言 | 1（`document.fonts.check`）→ **0** |
| 两个断言 | **都验证过会失败**（假字族 / Zapfino）|
| 视觉套件 | **733 全通过**（13.5m，零失败；+1）|
| typecheck / lint / prettier / knip / 8 个守卫 | 全绿 |
| 生产代码改动 | **零** |
...
## Round 218 —— 焦点丢失的第三种成因，和我自己制造的十二个假阳性

### 这一轮最重要的事：我的审计先错了一次

写了个通用扫描：对每个控件 focus → 按 Enter → 看焦点是否掉到 `body`。
第一版在 dock 上报出 **13 个控件丢焦点**。

**12 个是我自己的错。** dock tab 的 × 在未 hover 时是 `visibility: hidden`，
而 `visibility: hidden` 的元素**根本不能被聚焦**，`focus()` 是静默的空操作 ——
于是焦点仍停在页面加载后的 `body`，按 Enter 什么也没发生，
读数"BODY"其实是**"焦点本来就在那儿"**被打扮成了一个发现。

修正：**每一轮都先断言 focus 真的落到了控件上**，落不上的单独归类、不算缺陷。
改完之后：54 个按钮里 **1 个**真的丢焦点。

（× 不可聚焦也不是缺陷：`tablist` 里放一个可聚焦的兄弟节点是 axe 的
`aria-required-children` critical 违规，所以 ARIA 实践是**在聚焦的 tab 上按
Delete/Backspace**。代码注释早就写了这个决定 —— 我实测验证了它：
Delete 确实关闭标签页（11→10）并把焦点交给下一个标签页。）

### 扩大到三条路线后，找到 3 处真缺陷（同一族的第三种成因）

前两轮的成因是"控件禁用自己"。这一轮是"**控件把自己拿掉**"：

| 控件 | 为什么焦点没了 |
| --- | --- |
| `Regenerate response` ×3 | 点击让消息重新 materialize → `messageActionsVisibility` 返回 `"absent"` → **整条操作栏被卸载**（节点消失、之后渲染一个新的） |
| `Deny`（审批卡） | 作答后卡片被移除 |
| `Send` | 清空草稿 → 送出按钮变 `disabled` → 被挤走 |

### 治本

**消息列一次性声明规则**（`MessageBlock`）：焦点丢给"空"时，交还给消息列本身
（`tabIndex={-1}`，只接受程序化聚焦）。和 dock 关闭标签页时同一个套路 ——
**在持有焦点的东西还存在时就决定焦点去哪**。

**`submitComposer` 的三条受理路径收敛成一条**：`clear()` 就是"这次提交被受理了"
的时刻（这个文件自己的 invariant），所以焦点在那一刻回到编辑器。

### 我在这一轮里犯的第二、第三个错（都被自己的审计抓出来）

1. **靠 blur 事件做救援 —— 而移除被聚焦的元素不会触发 blur。**
   我把第一版能用的实现"简化"成 `onBlurCapture`，结果**在唯一需要它的时刻根本不会运行**，
   审计照样报出同样的孤儿。改成监听**焦点到达**（这个一定会触发），
   再在每次渲染后检查"我们曾持有焦点、而现在焦点在 body"。
2. **把焦点返回写进了一条这个路径不走的函数。**
   `focusComposer()` 放在 `accept()` 里，而 `accept()` **只有 submit MODE 会走** ——
   普通消息走文件末尾的默认路径、斜杠命令走中间那条。测量结果一动不动，
   我先以为是 `focusComposer` 的 target 是空的，插桩后才看到
   **`setComposerFocusTarget TEXTAREA` 有、`focusComposer` 一次都没被调用。**

### 守卫

`visual/activationFocus.visual.spec.ts`（新）—— 三条路线、每个能聚焦的控件
activate 一次，断言焦点不落在 `body`。**前置条件就是这个审计的全部价值**，
所以它写在最显眼的地方。32.8s。

| 破坏验证 | 结果 |
| --- | --- |
| 去掉消息列的救援 | 报出 4 处（Regenerate ×3 + Deny），红 |
| 去掉 composer 的焦点返回 | 报出 Send，红 |

### 验收

| | 结果 |
| --- | --- |
| 治本的缺陷 | **3**（操作栏卸载 / 审批卡移除 / 送出后禁用） |
| 我制造并自查掉的假阳性 | **12** |
| 收敛 | `submitComposer` 三条受理路径 → 一条 |
| 新审计 | 1（两项破坏验证都红过） |
| lint / prettier / knip / 全部守卫 | 全绿 |
| 单测 | **2444 通过**；失败 5 个全是 `src/rpc` 契约（非本轮） |
| 视觉套件 | **737 全通过**（15.0m；+1）。第一次跑有 1 个 `workspace` 用例失败，**单独跑、整文件跑（114 全通过）、整套重跑都绿**，判定为竞争性 flake 而非回归 |
...
## Round 222 —— 三处注释互相印证，而它们说的数字是错的

上一轮发现"我的笔记和 `closure` 的注释对 `fillStyle` 说法相反"，是靠碰巧读到才发现的。
这一轮把各处对**同一个机制**的断言抽出来对齐 —— 因为两处说反话时必有一处是错的，
而错的那处会误导下一次修改。

### 找到的矛盾：StyleX 的特异性到底带几个 `:not(#\#)`

| 出处 | 断言 |
| --- | --- |
| `globals.css:828` | "carries **three** `:not(#\#)`" → (3,n,0) |
| `visual/focusRing.visual.spec.ts:109` | "carries **three**" → (3,n,0) |
| `scripts/check-interactive-chrome.mjs:78` | "carries **three**" → (3,n,0) |
| `visual/cascade.visual.spec.ts:19`（我 Round 214 写的） | "**(2,n,0) 或 (3,n,0)**" |

三处互相印证，一处不同。**实测下来四处全错**：
...
从 1 到 8 都有 —— 因为**个数就是 StyleX 编码自身优先级的方式**（伪类、伪元素越深越多），
不是一个常数。

### 真正承重的那句话，四处都没写

`#\#` 是**一个 ID 选择器**，所以每一个 `:not(#\#)` 都是 **ID 级别的一个单位**；
而 globals.css 里的规则一个 ID 都没有 —— 那条全局 focus 规则是 (0,4,3)（我逐项算过：
`html` + `:not(input)` + `:not(textarea)` = 3 个元素；`:not([data-pointer])` +
`:focus-visible` + `:not([data-chrome-focus])` + `:not([contenteditable])` = 4）。

**所以一个就已经够了。** 那个"三"从来不是结论成立的原因，只是恰好是最常见的个数。
四处注释已全部改成这句准确的话 + 实测分布，并说明个数为什么会变。

（顺带确认：`cascade` 审计用 `.includes(":not(#\\#)")` 识别 StyleX 规则，
对 1–8 个都成立，**审计本身不受这个错误影响**。）

### 对齐后没有矛盾的两处（记下来，免得下轮重查）

- 「无层规则胜过任何 layer」—— 5 个文件 7 处提及，**说法一致**（也符合 CSS 层叠规范）。
- 「WebKit 到 Safari 26.5 没有 `corner-shape`」—— `globals.css:1522` 与 `DESIGN.md:414`
  **完全一致**。

### 顺带修掉一处会误读的旧快照

`check-bundle-size.mjs` 那张体积构成表以 "Measured" 开头，却**带着一行
`@layer utilities (Tailwind)`** —— 它是迁移期间量的，Tailwind 还在。现在读起来像当前事实。
已标明它是有日期的快照，并给出当前数字（884 条原子规则 / 2884 处 padding，
对当时的 821 / 2630），同时说明**没变的是那个答案的形状**，也就是体积上限为什么定在那儿。

### 验收

| | 结果 |
| --- | --- |
| 找到并改正的错误断言 | **1 个事实 × 4 处**（三处互相印证仍然全错） |
| 补上的承重解释 | 1（`#\#` 是 ID 选择器，所以一个就够） |
| 确认一致、无需改的机制 | 2 |
| 标明为历史快照 | 1 |
| 生产行为改动 | **零** |
| lint / prettier / bundle / 全部守卫 | 全绿 |
| `cascade` + `focusRing` 两条被改注释的规格 | 3 用例全通过 |
| 视觉套件 | **737 全通过**（14.9m，零失败） |
...
## Round 225 —— 按一次 Escape 什么也不发生，因为一个 tooltip 站在对话框上面

### 证据

Base UI 自己实现了 Escape 关闭 + 焦点归还，所以唯一丢掉这个契约的方式是**产品往
dismiss 栈上放了别的东西**。它放了。

| 时刻 | dialogs | tooltips | 焦点 |
| --- | --- | --- | --- |
| 聚焦触发器 | 0 | 1 | Expand table |
| 打开对话框 | 1 | **1** | **Close table preview** |
| **Escape #1** | **1** | 0 ← 关掉的是 *tooltip* | Close table preview |
| Escape #2 | 0 | 1 | Expand table |

**用户按一次 Escape，看起来什么都没发生。**

### 根因（我第一次诊断错了）

第一版我以为是**触发器**的 tooltip 没随对话框打开而关闭，于是给 `Tooltip` 加了个
`suppressed`、由 `IconButton` 从自己的 `aria-expanded` 推导。改完**毫无变化** ——
再去看那个开着的 tooltip 的文字，写的是 **"Close table preview"**：
它属于**对话框内部那个被自动聚焦的按钮**，跟触发器无关。那套 `suppressed` 机制
因此是纯多余的抽象，已整个回退。

真根因：**Base UI 的 `initialFocus` 默认聚焦"第一个可 tab 的元素"**，而 lightbox 里
第一个可 tab 的元素总是一个带 tip 的图标按钮。于是打开 lightbox 就把焦点放到一个
**用户从没导航到的控件**上、它的 tooltip 随之打开、然后这个 tooltip 压在对话框上面
吃掉第一次 Escape。图片画廊聚焦的是 **"Download image"**（不是关闭按钮），所以
"去掉关闭按钮的 tip"这种局部修法根本盖不住。

### 治本：聚焦面板本身

`LightboxDialog` 一处改动，覆盖三个消费方（表格预览 / 图片画廊 / Mermaid 放大）：
`initialFocus={popupRef}` + `tabIndex={-1}`。这也是惯例做法 —— 对话框自己会播报标题，
第一次 Tab 就到控件。画廊的方向键绑在 `window` 上，不受焦点位置影响。

| 触发器 | 修改前 | 修改后 |
| --- | --- | --- |
| Expand table | 焦点→Close 按钮，tip 开，**Escape ×2** | 焦点→面板，无 tip，**Escape ×1**，焦点归还 |
| Inline architecture / Inline detail | 焦点→Download image，tip 开，**Escape ×2** | 同上 |
| Enlarge diagram | 本来就一次（无带 tip 的控件抢到焦点） | 一次 |
| Switch model | 一次（焦点进搜索框） | 一次 |

### 守卫

`visual/dialogDismiss.visual.spec.ts`（新）—— 四条路线、每个能聚焦的
`[aria-haspopup]` 用键盘打开一次，断言**一次 Escape 关闭**且**焦点回到触发器**。
**破坏验证：回退修复 → 精确报出 4 处**（表格 ×2 + 两张图）。

**每个触发器之间重新加载页面，这不是细节** —— 第一版在一次加载里连着扫，报出 9 处失败，
**其中 8 处是上一轮留下的浮层没关**，让之后每一次"没关掉"都为真。
一个会改变自己所测页面的扫描，必须每次从头开始。

### 一个查清了不是回归的项

修完发现画廊 ArrowRight 不切图。**回退我的改动后重测：一样不切** —— 不是我引入的。
原因是每张内联图各自是一个**只有一项**的画廊（上/下一张的按钮也只在多于一项时才渲染），
所以没有可去的地方。记下来，免得下次当成缺陷。

### 验收

| | 结果 |
| --- | --- |
| 治本的缺陷 | **1**（4 个实例：3 个 lightbox 消费方） |
| 回退的多余抽象 | 1（`Tooltip.suppressed`，诊断错的产物） |
| 新守卫 | 1（破坏验证精确报出 4 处） |
| 自查掉的假阳性 | **8**（浮层状态跨迭代泄漏） |
| 查清不是回归的项 | 1（画廊方向键） |
| lint / prettier / knip / 全部守卫 | 全绿 |
| 单测 | 2449 通过；失败 5 个全是 `src/rpc` 契约（非本轮） |
| 视觉套件 | **737 通过 / 1 失败**（14.0m；+1）。失败的是 `workspace` 那条**第二次**在整套里失败的用例（Round 218 也是它）；隔离跑 **5/5 通过**，不是本轮引入。第二次出现就不再当 flake —— 处理见下 |

### 一句话

契约是"按 Escape 关闭"，实现它的是库；**丢掉它的是产品把焦点放到了一个
用户从没去过的按钮上** —— 那个按钮的提示框于是成了栈顶，而它替对话框接了这一下。

### 关于那条第二次失败的用例（不再当 flake）

它驱动的正是 Round 218 量到的那个**悬停才显形**的 × （未 hover 时 `visibility: hidden`），
先 `hover()` 再 `click()`。隔离 5/5 通过、整套里两次失败 —— 这是上下文相关，
不是本身不稳。

**而我两次都没拿到失败原因**：整套跑的命令一直是 `npm run visual:test 2>&1 | tail -20`，
`tail` 把 Playwright 打印的错误块整个丢掉了，输出文件里只剩最后 22 行。
**是我的管道把证据扔了。** 下一轮改成完整落盘再看尾部，并在能复现时读 error-context。

## Round 226 —— 一个 738 全绿的套件里，藏着一个没人看得见的未处理错误

### 起点：我自己的管道把证据扔了

上一轮那条第二次失败的 `workspace` 用例，我两次都没拿到失败原因 ——
因为整套跑的命令一直是 `npm run visual:test 2>&1 | tail -20`，
`tail` 把 Playwright 的错误块整个丢掉。改成完整落盘之后，那条用例这次通过了
（738 全绿），但完整日志里露出了另一样东西：

```
[WebServer] [vite] (client) [Unhandled error] Error: ResizeObserver loop completed with undelivered notifications.
```

### 缺陷一（守卫的洞）：这类错误套件根本看不见

实测：在页面里挂 `window` 的 error 监听器**能看到**它；
而 `page.on("console")` 和 `page.on("pageerror")` **一个都没触发**。

所以 22 个规格依赖的 `quietConsole` 对**整整一类未处理错误是瞎的** ——
一个真实的未处理错误，安安静静躺在 738 个绿灯里。

**治本**：`visual/test.ts` 增加第三条通道 —— 用 `exposeFunction` + `addInitScript`
在页面里监听 `error` 与 `unhandledrejection`，经 binding 回传。
用 binding 而不是往 `window` 上攒，是因为每次导航都会换一个新的 `window`，
而一条规格会导航很多次。

**破坏验证**：回退产品修复后，`workspace` 那条窄行用例**当场变红**并报出
`window.error: ResizeObserver loop completed…`。

### 缺陷二（产品）：resize 级联超出了一帧的投递预算

先量清范围：**6 条路线加载时 0 次**；`dock-light` 与 `dock-review` 在 resize 压测后
**各 1 次**；settings / agent / shell 都是 0。

逐个观察器二分：把 `ChatPanel` 的 observer 掐掉 → 错误消失；
把 `context-dock` 的掐掉 → 依然报错。

但插桩之后发现 **`ChatPanel` 的回调并没有失控** —— 一次 End 按键只跑 3 次、
宽度恒定 600、`dockOpen` 正常 true→false 收敛。所以这不是某个回调打转，
而是**整个文档十个观察器的级联在一帧内超出了 Chrome 的投递预算**，
掐掉任何一个参与者都会把它压回限额以下。

**治本**：把 resize 驱动的写操作推迟一帧（rAF 合并），级联就不再全落在同一帧。
改完两条路线 6 轮压测**各 0 次**。

### 我在这一轮里改错了两次

1. **第一个假设是错的**：我以为是 `collapseWorkspaceDock()` 在 store 往返落地前被反复
   调用，于是加了"每次进入 too-narrow 只塌一次"的 ref 守卫。**错误一次没少**，
   已整个回退 —— 那是个没有依据的抽象。
2. **第一版修复动了 golden**：把**全部**投递推迟一帧，让 dock 比例晚一帧落地，
   `answer-opening` 的工具汇总行**文字完全一样**、只是亚像素抗锯齿偏移，
   差了 **270 px**（两个主题各一条 golden）。
   而我早先量到的事实正好给出答案：**加载时从来不报错，只有 resize 才报** ——
   所以第一次投递（`observe()` 立即触发、且此时行宽可能仍为 0）**保持同步**，
   只推迟后续的。改完 golden 恢复通过，错误依然是 0。

### 缺陷三（顺带量出来的）：一条 golden 的容差低于它自己的失败模式

整套跑完只剩一条红：Mermaid 放大视图的 golden，差 **476 px**，而预算是 `maxDiffPixels: 400`。

单独重跑 5 次：**4 次 0 px、1 次 476 px** —— 不是渐变噪声，是**两种离散状态**
（两次出现都恰好是 476）。看差异图：红的是**面板最底边那一行 1px**，不是图本身。
Mermaid 用自己那套不完全可重复的字形排版决定 SVG 尺寸，面板高度因此落在不同的小数上，
底边抗锯齿就变了。

**先验证不是我上一轮改的**：把 `initialFocus` 去掉再跑 5 次，**依然 1/5 出现 476**。
所以这是既有问题 —— 一条**每约 5 次跑就红一次**的 golden。

容差按实测模式抬到 700（离散模式必须跨过去，不能压在下面），并把测量写进注释。
改完 6 次 × 两个主题**全部稳定通过**。
...
| | 结果 |
| --- | --- |
| 治本的缺陷 | **3**（守卫对一类错误全盲；resize 级联超预算；golden 容差低于自身失败模式） |
| 回退的错误假设 | 1（collapse 去重 ref） |
| 避免的 golden 变更 | 2 条（靠"加载不报错、只有 resize 报"这个已测事实） |
| 自己的取证失误 | 1（`tail` 丢掉错误块，已改为完整落盘） |
| 视觉套件 | **738 全通过**（14.8m，零失败，`window.error` 计数 **0**） |
...
## Round 229 —— 滚动锚定站得住；而"跟随尾部"这条契约现有 fixture 测不了

AGENTS.md 把 long context / long execution / restart / recovery 列为测试优先级。
Round 224 只用克隆 DOM 摸过 304 个 turn 的滚动，没量过"读者的位置能不能保住"。

| 量的性质 | 结果 |
| --- | --- |
| 长会话加载后停在哪 | **底部**（最新内容），不是顶部 |
| 窗口从 1472 收到 1120 | 内容高度**一点没变** —— 阅读栏宽度是固定的 measure，两个宽度都宽于它，所以**根本不发生重排** |
| 内容高度真的变化时（读者调大字号档） | 内容长了 **58px**，读者的锚点只移动 **22px**，`scrollTop` 保持不变 —— 浏览器的 scroll anchoring 吸收了大部分 |

22px 的位移发生在用户主动改全局字号时，属于合理代价，不算缺陷。

### 记一个 harness 的覆盖缺口（不是产品缺陷）

真正要紧的那条契约是：**只有读者已经在底部时才跟随新内容**，否则保持原位并给出
"回到底部"的入口（`JumpToBottomButton` 就是为此存在的）。

想量它却量不了：`answer-opening` / `running` / `waves` 都是**静态快照** ——
可滚动范围只有约 55px、等 2.5 秒后内容高度分毫未变。**没有任何 fixture 会随时间
产出新内容**，所以"跟随/不跟随"这条分叉在当前 harness 下无法触发。

做一个流式 fixture 是 harness 层的实质改动；先如实记下这个缺口，
而不是拿静态快照假装量过了。
...
| --- | --- |
| 找到的缺陷 | **0** |
| 量出来的事实 | 3（加载停底部；阅读栏在 1120–1472 间不重排；字号变化下 58px→22px） |
| 记下的 harness 缺口 | 1（无流式 fixture，跟随尾部无法测） |
| 生产代码改动 | **零** |
...
## Round 231 —— 审计自己的审计：谁在用启发式挑被测对象

Round 230 的教训是"用启发式定位被测对象，失败时是静默的"。本会话已经出现三次
（218 的 12 个假阳性、225 的 8 个、230 的量错元素）。这轮把全部视觉规格过一遍。

### 结果：**提交进套件的审计基本是干净的**，三次事故都发生在**我的一次性探针**里

| 模式 | 情况 |
| --- | --- |
| `sort(...)` 按尺寸挑"最大的那个" | **committed 规格里一个都没有**（只存在于我丢弃的探针中） |
| `querySelectorAll("*")` 全量扫描（5 处） | **按设计如此** —— 它们是"遍历一切"，与"挑一个"相反，且都带 floor 断言 |
| `.first()` / `.nth()`（9 个文件） | 绝大多数被**属性或容器限定**（`[data-settled-answer]`、`[data-user-message-bubble]`、`thinking.locator(...)`…），属于按名定位 |
| 两处 `.first()` 的历史陷阱 | **已有注释记录**：弹层是「dialog 里套 listbox」会匹配两个；dock 视图全部保持挂载，`.first()` 曾选中隐藏的那个 |
| `getByRole("heading").first()`（2 处） | 注释明确说明它**故意是弱断言**，真正的就绪判据是下一行的 `aria-busy` 计数 |

这个区分要紧：**不是套件在骗人，是我写探针时草率。** 已修好的三条审计现在都用
具名目标或断言了前置条件。

### 唯一一处仍是位置性的，是我自己写的

`fontSmoothing.visual.spec.ts` 用 `REGION = "aside"` 加 `.first()`。
实测当前 shell fixture 里 `aside` 只有一个（产品自己的 `agent-drawer`），
所以 Round 214 的测量是对的 —— **但它对是靠当下 DOM 的运气，不是靠构造**。

而它的失败方式恰恰是静默的：这条审计**把同一个区域的两张截图互相比较**，
所以哪天 shell fixture 长出自己的脚手架 `aside`（agent fixture 早就有一个），
它会**把 harness 拍两遍然后照样通过**。

改成具名 `aside.agent-drawer`，并在每次导航后断言**它恰好匹配 1 个元素**。

| 破坏验证 | 结果 |
| --- | --- |
| `REGION = "aside, main"`（匹配 2 个） | 红：Expected 1, Received 2 |
| `REGION = "aside.does-not-exist"`（匹配 0 个） | 红：Expected 1, Received 0 |

（顺带记一个我自己的失误：第一版把这条断言放在了**任何导航之前**，页面还是空白，
于是它报 0 —— 断言的位置和断言本身一样要紧。）
...
| 确认为"按设计如此"的模式 | 3 类 |
| 生产代码改动 | **零** |
| 视觉套件 | **738 全通过**（14.7m，零失败，window.error 计数 0） |

### 一句话

去审计自己的审计，最有价值的发现是**边界在哪**：
三次事故全在一次性探针里，套件本身只有一处位置性定位 —— 而那一处正是我写的，
且它的失败方式是**把测试脚手架拍两遍然后报绿**。

## Round 232 —— 我的审计有四分之一到一半在测量测试脚手架

Round 231 顺带发现：agent fixture 有自己的脚手架侧栏，shell fixture 没有。
这轮量了这个偏差有多大。
...
| 路线 | 脚手架占按钮 | 脚手架占元素 |
| --- | --- | --- |
| `agent/narrative` | **23 / 51（45%）** | 146 / 527（28%） |
| `workspace/dock-light` | **28 / 52（54%）** | 198 / 773（26%） |
| `shell/populated` | 0 / 20 | 0 / 216 |

也就是说：在 agent 与 workspace 路线上，**我那些"遍历整页"的审计，将近一半激活的
按钮是 harness 自己的状态切换器**，不是产品。

后果有两个方向，都不好：
- **覆盖率虚高** —— "激活了 60 个控件"里有一大半是脚手架。
- **误报** —— 脚手架里的缺陷会被报成产品缺陷（Round 218 的 12 个假阳性正是这一类的近亲）。

### 治本：给脚手架起个名字，让扫描能把它认出来

两个 fixture 的外层加 `data-fixture-chrome=""`，四条我写的审计据此排除：
`activationFocus`、`dialogDismiss`、`cascade`、`radiusLadder`。

**然后把被虚高的数字改成真数字**：

| 审计 | 之前（含脚手架） | 现在（仅产品） |
| --- | --- | --- |
| `activationFocus` 激活的控件 | 60+ | **41**（floor 60 → 30） |
| `radiusLadder` 比较的圆角 | 1660 | **1252** |

`activationFocus` 的 floor **当场就红了** —— 因为那个 60 有三分之一是脚手架在帮忙凑够的。
这正是这轮的价值：**一个 floor 被测试脚手架满足着，和没有 floor 差不多。**

### 记一下边界

`shell` fixture 没有脚手架侧栏（它的 `aside` 就是产品的 drawer），所以那条路线一直是干净的 ——
这也解释了 Round 231 为什么在 shell 上只找到一个 `aside`。

`closure` 等更早的审计多数已经把范围限定在产品区域（`main section`、`.msg-content` 等），
本轮没有改动它们。
...
| --- | --- |
| 找到的产品缺陷 | **0** |
| 修正的测量失真 | **2 个计数 + 1 个 floor** |
| 加的标记 | 1（`data-fixture-chrome`，两个 fixture） |
| 更新的审计 | 4 |
| 生产代码改动 | **零**（只动 `visual/`） |
| 视觉套件 | **738 全通过**（14.0m，零失败，window.error 计数 0） |

### 一句话

我一直在报告"扫了 1660 个圆角、激活了 60 个控件" ——
**其中四分之一到一半，量的是我自己搭的测试脚手架。**

## Round 233 —— 先否掉自己上一轮提的方案，再去查守卫会不会"读不到东西却报绿"

### 一、我上一轮说要做的守卫，做了也白做

上一轮结尾我打算加一条 `check:*`，禁止视觉规格用位置性定位。动手前先问一句：
**它能抓住已经发生的那三次事故吗？**

不能。三次（218 的 12 个假阳性、225 的 8 个、230 的量错元素）**全部发生在我未提交的
一次性探针里**，而守卫只看提交进仓库的规格。加了它既抓不到过去、也很难不误伤 ——
Round 231 已经量过，committed 规格里的 `.first()` 绝大多数是属性或容器限定的。

**所以不做。** 指南写着"不得为了继续迭代而制造工作"，这正是那种情况。

### 二、换成同一条线上真正可查的：守卫自己有没有 floor

`check-authored-classes` 曾经在 Tailwind 离开的同一个 commit 里死掉、却继续报绿。
那么现在这 21 个守卫，**哪些能在读不到任何东西的情况下报成功**？

逐个跑、看成功行说了什么：

| 情况 | 守卫 |
| --- | --- |
| 成功行带计数（读了多少文件/模块/键） | **19 个**，例如 `1267 modules, 4386 edges`、`1259 files read`、`1107 keys` |
| 成功行不带计数 | `check-layers`、`check-bootstrap` |

再逐个查这两个：
- `check-layers` **内部有 floor**（`MIN_MODULES=600` / `MIN_EDGES=1000`，不满足 exit 2），
  只是成功行没把数字印出来。
- `check-bootstrap` 不遍历文件，它读 5 个固定文件、用正则抽 7 个值。

**结论：会因为"读不到东西"而静默通过的守卫，一个都没有。** 这条线比我预期的干净。

### 三、两处对不上仓库自己标准的小口子（已补）

1. **`check-bootstrap` 的 7 个正则抽取，6 个显式断言"抽到了"，第 7 个没有。**
   `appEndpoint` 缺了那句。它**不会**静默通过（`shellEndpoint === undefined` 为假 → 仍然红），
   但报出来的是"drift：app defaults to undefined"，读着像两边不一致，
   而真相是**这条检查已经找不到那个常量了**。
   补上后破坏验证：把 `DEFAULT_RUNTIME_ENDPOINT` 改名 →
   `the app no longer declares DEFAULT_RUNTIME_ENDPOINT — this check reads nothing` ✓

2. **`check-layers` 的成功行不带证据**，而它算好的 `moduleCount` / `graphEdgeCount` 就在手边。
   改成 `OK — no layer-boundary violations across 1267 modules, 4386 edges.` ——
   floor 拦得住"读了零个"，印出来才拦得住"读得越来越少"。

### 验收

| | 结果 |
| --- | --- |
| 否掉的自提方案 | **1**（会白做的守卫，理由已量过） |
| 审过的守卫 | **21 个全部** |
| 能静默通过的守卫 | **0** |
| 补上的口子 | 2（破坏验证过 1 处） |
| 生产代码改动 | **零**（只动 `scripts/`） |
| lint / prettier / 19 个守卫 / 单测 | 全绿（2449 通过） |
...
## Round 237 —— 三个"不该抄"，一个撤回的自己人改动，和一个补上的三年缺口

### 一、dimagent 的工具调用渲染，逐项比完

`index-BGibJl7P.js` 里有 `TimelineToolCallItem` / `ToolCallInlineSummary` / `ToolCodeBlock` /
`CommandApprovalPanel`。它用 Tailwind，类名可读。

**折叠态**：图标 `h-3.5`(14px)、标签 `text-sm font-medium`、详情 `truncate text-muted-foreground/70`，
`group-hover/tool-call` 让整行提亮。
**展开态**：输出面板 `max-h-[240px] overflow-auto rounded-md border bg-card`，
输出文字 `font-mono text-xs leading-5`，**面板内有 `sticky top-0 z-10 bg-card` 的标题**。

三条对照结论，**都是不该抄**：

| dimagent | Flame | 判断 |
| --- | --- | --- |
| 整行 hover 提亮 | `--row-ink: fgMuted → fg` | **已经一样**，注释还写着"标签和详情读同一个通道" |
| `max-h-[240px]` 内滚 + sticky 标题 | 按行数折叠（9 / 展开 1000）+ 切口渐隐 | **不抄**：Flame 没有内层滚动，所以不需要 sticky 标题，也避免了嵌套滚动抢滚轮；注释里有实测（1000 行 120ms、10000 行 700ms、50000 行 9 秒） |
| 图标 14px | 12px | 见下 |

### 二、图标那条我改了，量完又撤了

Flame 的图标阶梯所有者把规则写死在偏移里：`xs: -2, sm: 0, md: 2, lg: 6`（相对 `--fs-ui-md` 14px）。
`sm` 的偏移是 **0** —— 按构造就是"与文字等大"、即紧邻标签那一档。
工具行的身份字形用 `xs`，比标签低一档；而 `step-row` / `context-dock` / `composer-chip` /
`catalog-picker` 全用 `sm`。实测确认：`agent-activity-mark` 是 **12px 字形配 13px 标签**。

改成 `sm` 后跑 golden：**51 处差异**。看图对比（前后两张 1:1）—— **几乎无法分辨**，
布局零位移（盒子固定 16px）。于是重新评估，**撤回**：

1. **两个参考彼此就不一致**：dimagent 14px、Codex 16px。既然它们自己都不同，
   就**没有一个"像素"可以复刻** —— 这一条是决定性的。
2. **Flame 有一条相互竞争、且已写明的规则**：`--glyph-step: 0.8`
   （"字形比它旁边的词退后多少"），说明"字形从属于文字"是刻意的语言，12px 与它自洽。
3. 视觉收益近零，代价是 51 张 golden —— 那是拿我的口味换搅动。

`step-row` 的 14px 不构成反证：它是**状态标记**（勾/环），必须读得出状态，不是装饰字形。

**留给用户决定**：若希望工具行的身份字形与标签等大（两个参考都 ≥ 标签），
改动是一个 token、零位移、51 张 golden。

### 三、补上一个记了三轮却一直没补的缺口：跟随尾部

前几轮反复记："没有 fixture 会随时间流式产出内容，所以'只在贴底时跟随尾部'这条契约测不了。"

**它不需要流式 fixture。** `use-stick-to-bottom` 响应的是**内容盒变高**，
所以测试可以直接制造这件事 —— 和"逼出计划步骤换行"是同一招。
往内容元素追加一个块，不是在测 fixture，是在测**上线的滚动器在它监视的东西长高时做什么**。

实测，产品是对的：

| | 内容长高 400px 后 |
| --- | --- |
| 贴底时 | `scrollTop` 1774 → 2173，距底始终 ~0 —— **跟随了** |
| 向上滚 600px 后 | `scrollTop` 1052 → **1052 纹丝不动**，距底 600 → 1000 —— **没有跳** |

**缺的是测试，不是产品。** 新守卫 `visual/followTail.visual.spec.ts` 覆盖三半 ——
跟随 / 不跳 / 回得去 —— 因为前两半从错的一端看是一样的：
一个永远跳到底的滚动器能通过"它跟随了"，一个从不滚动的能通过"它没跳"。

**过程中还量到一件事**：跳转是**弹簧动画**，从 600px 外读数为
200ms→184px、400ms→47px、800ms→3px，收敛在 1px。我第一版用固定 700ms 等待，
抓在半途读到 12px 就报了红。改成 poll，因为契约说的是它**落在哪**，不是**多快到**。

破坏验证：给 `ControlsRelay` 加一个 120ms 的强制 `scrollToBottom`（这是真实回归的形状 ——
有人"为了修流式"这么干），守卫报
`the reader has to actually be away from the bottom: expected > 100, received 42` ✓
报的是"读者没能停在自己放的位置"，不是一个像素数。

### 验收

| | 结果 |
| --- | --- |
| 逐项比完的 dimagent 工具渲染维度 | 3（hover 提亮 / 输出面板 / 图标尺寸） |
| 结论为"不该抄"的 | **2**，各有理由 |
| 改了又撤回的 | **1**（图标尺寸，51 张 golden 的代价换近零收益） |
| 补上的测试缺口 | **1**（记了三轮的跟随尾部） |
| 新守卫 | 1（三半 + 破坏验证，连跑 3 次稳定） |
| 产品代码改动 | **零** |

### 一句话

这一轮**一行产品代码都没改**，而它可能是最该这样的一轮：
三条对照里两条的答案是"不该抄"，第三条我抄了、量完发现
**两个参考彼此都不一致 —— 那就没有像素可复刻**，于是撤回。
真正的产出是把一个记了三轮的"测不了"变成了"测过了"——
而它从来不需要流式 fixture，只需要**测试自己去制造那个条件**。

## Round 238 —— 审批卡也一致；然后在流式揭示上找到一个真的覆盖缺口

### 一、dimagent 的审批面板，安全相关的那一条也对上了

`CommandApprovalPanel` / `ApprovalInputPanel` 的命令体是
`whitespace-pre-wrap break-words px-3 pb-2 font-mono text-xs leading-5`。

**`whitespace-pre-wrap break-words` 是安全相关的**：审批框里的命令若横向滚动，
就可能出现"看见 `rm -rf node_modules`、实际批准的后面还有一截"。

Flame：`ApprovalCard` 用 `<Well as="code" ink="strong" cap="lg">`，
而 `Well` 的默认 `wrap` 正是 `{ whiteSpace: "pre-wrap", overflowWrap: "break-word" }` ——
**本来就换行**，隐患不存在。高度上限 320px vs dimagent 240px，同一个想法。

对照到此，工具调用渲染这一块的结论是：**结构、hover、换行、输出折叠策略全部对齐或各有更好理由。**

### 二、转回直接量 Flame，找到一个真缺口

按区域数测试覆盖，`streamReveal.test.ts` 只有 68 行、只测 `pickRate`。
文件头写着：hook 层"不测：它是跟着性能特性走的移动靶"。

**这个理由只覆盖了速率。** 40/80/160 确实可以变，但下面这些在任何速率下都必须成立：

- 返回的永远是原文的**前缀**（否则 markdown 解析器会看到一份没人写过的文档）
- 永远不停在**孤立的高代理项**上（那一帧会渲染成 ▯）
- `instant` / reduced-motion 立即给出全文
- `smooth` 停在**词边界**，`typewriter` 不
- 停止后仍会排空到底
- 原文被替换时**从新文本重新开始**，不从旧位置继续

补了 7 条行为测试（2449 → 2456）。三条关键的都做了破坏验证：

| 破坏 | 报出 |
| --- | --- |
| 拿掉 `isHighSurrogate` 修正 | 3 个帧停在孤立高代理项（`'ship �'`） |
| smooth 强制走逐字分支 | 40+ 个帧切在词中间 |
| 拿掉变短钳制 | 见下 |

### 三、我自己写的测试里，有一条是空过的

第一版的"原文变短"测试：长 → 短，断言结果等于短文本。**拿掉钳制后 15 条仍然全过。**

因为 `slice` 本身就会截断 —— 一个指向文本末尾之外的揭示位置，
交出来的仍然**正好**是那个短字符串，看上去毫无问题。

**那个钳制守的不是"变短"，是"再变长"。** 位置停在旧的大值时，
下一段文本会**开局就带着一大截从未被逐字揭示的内容**，
读者看到的是一整段凭空出现、然后才继续一个字一个字地长。

改成 长 → 短 → 再变长 后，破坏验证给出
`the replacement opened with text that was never revealed: expected 45 to be less than 40` ✓

### 验收

| | 结果 |
| --- | --- |
| 对照确认一致的 | 1（审批命令换行，安全相关） |
| 找到的测试缺口 | **1**（`useStreamReveal` 的全部行为） |
| 新增行为测试 | 7（2449 → 2456） |
| 破坏验证过的断言 | 3 / 3 |
| 自己写完发现空过、重写的测试 | **1** |
| 产品代码改动 | **零** |
...
一个"不测 hook"的理由，写的时候只想到了速率 ——
而**代理对切一半、词切一半、替换后跳出没揭示过的正文**，都跟速率无关。
更值得记的是我自己那条空过的测试：它断言"变短之后结果正确"，
而 `slice` 让这句话在守卫被删掉后依然为真 ——
**钳制守的从来不是变短那一步，是再变长那一步。**

## Round 239 —— 打字机插入符落在句子中间，而它从来没被测过

### 一、dimagent 的 DiffView：一条推测性的性能改动，不做

`DiffView-CXGaFR8E.css` 只有 401 字节，但有一条：

```css
.context-panel-diff-file-item { content-visibility: auto; contain-intrinsic-size: 34px; }
```

Flame 有同样的机制，但只用在 transcript turn 上，而且**有实测记录**
（304 turns、真实高度 28–528px、滚动条 70143 → 53866px 收缩 23%），
还带一条重要例外：**尾部 turn 不能跳过**，否则冷恢复时 HITL 控件只剩占位符、也从无障碍树里掉出去。

把它推广到 diff 文件列表是**推测性的性能改动**。仓库规则写着"不要猜性能在哪，先测"，
而 fixture 里只有 2 个 diff 文件，测不出来。**不做，记录。**

（顺带印证一条：dimagent 的 diff 代码块是 `overflow-x: auto` ——
diff 该横向滚动而不是换行，正是 Flame `Well wrap="pre"` 注释写的
"有意义的列：JSON、diff、表格 —— 滚动而不是重排"。）

### 二、`rehypeStreamCaret.ts` 没有测试，而它有个真缺陷

同目录 7 个文件有测试，它没有。29 行，契约看着简单，读代码时觉得可疑：

它从子节点**末尾往回扫第一个 element**，然后下降进去。
但流式文本最常见的形状是**段落以纯文本结尾** —— 这时"最后一个 element"在文本之前。

在真实渲染的 DOM 里量（渲染 `MarkdownMessage`，typewriter + streaming，推进 300 帧）：

| markdown | 插入符的父元素 | 它右边还有的文字 |
| --- | --- | --- |
| `Hello world here` | `<p>` | `""` ✓ |
| `Hello **world** and more text` | **`<strong>`** | **`" and more text"`** |
| `See [docs](…) for more text` | **`<a>`** | **`" for more text"`** |
| ``Run `npm test` before pushing now`` | **`<code>`** | **`" before pushing now"`** |
| `Hello and **world**` | `<strong>` | `""` ✓ |

**任何含粗体 / 链接 / 行内代码、后面还有文字的流式段落，插入符都显示在句子中间，
右边还跟着一整个从句。**

只影响 typewriter 模式（smooth 走 `rehypeFadeIn`），但那是用户可选的模式。

### 三、修法：只在"最后一个有意义的子节点是元素"时才下降

不能简单地"只看最后一个子节点" —— `remark-rehype` 会在块之间插 `\n` 文本节点，
那样会把插入符推到段落之外。所以向回扫时**跳过纯空白文本节点**，
找到最后一个有意义的子节点：是元素且非 ATOMIC 才下降，否则就在当前层追加。

修复后八个用例（含代码块结尾、列表、标题）**右边全部为空**；
`Hello and **world**` 仍然落在 `<strong>` 里 —— 那是对的，粗体本来就是最后一样东西。

### 四、我的守卫第一版是空过的，破坏验证抓住了

第一版量的是 `caret.parentElement` 到末尾。**旧实现下它也通过。**

因为旧实现里插入符的父元素是它下降进去的那个 `<strong>`，
而 `" and more text"` 是 `<strong>` 的**兄弟**、在 `<p>` 里 ——
"父元素内没有后续文字"这句话对错误实现同样成立。

我的第一个探针用的是 `closest("p")`，第二个改成了 `parentElement`，
两者结论不同而我当时没注意。改成量到**最近的块级祖先**后，
旧实现精确报出三个缺陷用例及各自被甩在右边的文字 ✓

### 验收

| | 结果 |
| --- | --- |
| 明确不做并写明理由的 | 1（diff 列表的 `content-visibility`，属推测性性能） |
| 真缺陷 | **1**（打字机插入符落在句中），在真实 DOM 里量出 |
| 新测试 | 1 个文件（2456 → 2457） |
| 自己写完发现空过、重写的守卫 | **1**（本会话第二次） |
| typecheck / lint / prettier | 全绿，余 `src/rpc` 越界既有阻塞 |

### 一句话

一个 29 行、没人测过的 rehype 插件，把打字机插入符放在了**句子中间** ——
因为它找的是"最后一个元素"，而流式段落最常见的结尾是**文字**。
...
## Round 243 —— 判定"这一格是数字"的那个正则，只认整数

### 一、先做覆盖面的元审计，结论是大多已经够宽

上一轮的教训（审计被 `querySelector("main")` 锁在 transcript 里）值得推广，
所以先扫了全部 27 个视觉规格各自覆盖哪些 fixture。
**大多数已经跑满 agent / shell / workspace 三个**；只跑一个的那几条各有理由
（`followTail` 只有 transcript 有、`overflowFade` 的跑马灯在 work index、
`fontSmoothing` 和 `oklabPrediction` 是文档级设置）。**没有第二个"只查了一半"。**

### 二、换成注入一种还没试过的条件：数字变化

CLAUDE.md 的检查项里有"数字 `tabular-nums`"，dimagent 的 `TimelineToolCallItem`
也显式用了它。会跳动的数字若不是等宽的，每跳一次整行都会重排。

Flame 现状比我预期的好：有共享档 `vocab.figures`，而且 `globals.css:804` 有一条全局规则
让**等宽字体自动获得等宽数字**（"decided here so no call site has to remember"）。

扫 7 条路由上所有"以数字为主"的文本节点，比例字体且非 tabular 的只有 7 处，
其中 3 处是 `sr-only` 的测试探针（`dock-open` / `persisted-dock-ratio`），不是产品。
**真正的是 markdown 表格的数字单元格。**

### 三、`markdown.css` 早就准备好了，是分类器没认出来

`.md-table-cell-numeric` 存在，给 `tabular-nums` + `min-width: calc(3ch + 24px)` + `nowrap`。
而决定哪些格子拿到它的是 `markdownComponents.tsx:229`：

```ts
/^\d+$/.test(visibleText(children).trim()) && "md-table-cell-numeric"
```

**`/^\d+$/` 是无符号整数，别的都不是。** 而 agent 真正会写的表格是跑分或 diff 汇总：

| 写法 | 旧判据 |
| --- | --- |
| `12` | ✓ |
| `91.2%` `4.2s` `1,234` `-3` `+7` `0.5` `128MB` | **全部 ✗** |

它们拿不到等宽数字、拿不到最小宽度、也拿不到 `nowrap`，于是一列数字对不齐、还会换行。
...
尾部允许最多三个字母的单位（`ms` / `GB` / `px`）—— **刻意略宽**：
认错了只是给一个短格子加了最小宽度；认漏了是一整列对不齐。
两端都锚定，所以一句话永远够不到它。

测试同时钉住**两边**：7 种数量都要被认出，而 `n/a` / `2024-01-15` / `v1.2.3` /
`3 of 7` / `see 12 rows` 都不能被认成数字 —— 因为 `nowrap` 加在散文上，
是一列再也让不出宽度的格子。

破坏验证：换回 `/^\d+$/` → 精确报出那 7 种写法，只有 `12` 幸存。

### 验收

| | 结果 |
| --- | --- |
| 元审计：覆盖面只查一半的规格 | **0**（上一轮那个是唯一一个） |
| 注入的新条件 | 1（数字用比例字体） |
| 扫出的非 tabular 数字 | 7 处，其中 3 处是测试探针 |
| 真缺陷 | **1**（数字格分类器只认整数） |
| 新测试 | 1 条，正反两边各 7/5 个用例 |
| 破坏验证 | 精确命中 7 种写法 |

### 一句话

样式早就写好了，`min-width` 和 `nowrap` 都在那儿等着 ——
...
## Round 245 —— 判定"这段没有词间空格"的正则，只认汉字

### 一、先把上一轮那条脉络收尾：入口封完了

Round 240 / 244 修的是同一个形状（`nested-interactive`）的两个入口。
这轮问第三遍："还有别的吗？" 把十种 markdown 形状一次试掉：

脚注在链接里、文件路径在脚注里、图片/链接/路径在表格单元格里、`<kbd>` 在链接里、
标题里的徽章链接、列表里的徽章链接、引用块里的链接、带路径的自动链接 ——
**全部 `nested=0`。** 两次修复把入口封完了。

（唯一可注意的：`[text[^1]](url)` 会被解析器把脚注锚点提到链接外、留下一个空 `<sup>`。
那是 markdown 管线不允许嵌套锚点的标准行为，不是 Flame 组件的决定。）

### 二、换用 Round 243 的另一个可推广形状：判据过窄
...
它驱动的是 `markdown.css` 里一条收紧**连续 CJK 段落间距**的规则（4px）。
理由成立：没有词间空格的文字是一整块均匀的墨，
同样的段落间距在两块之间读起来就比在两段参差的西文之间大。

但 **Han 是汉字。** 日文由汉字 + 平假名 + 片假名组成。实测：

| 段落 | 标记 |
| --- | --- |
| `実行は完了しました。` | `true` |
| `それではつづきをおねがいします。`（全平假名） | **无** |
| `コンパイルエラーガアリマス。`（全片假名） | **无** |
| 中文 | `true` |
| 韩文谚文 / 英文 | 无 |

而 CSS 规则是 `p[han] + p[han]` —— **两段都得匹配才生效**。
所以日文回复里夹一个全假名段落，它**上下两个间距都会退回西文的宽间距**，
而其余段落是紧的。**不是"少了个优化"，是同一条消息里节奏不一致。**

### 三、韩文刻意不加，理由写在判据上

谚文**有词间空格**（띄어쓰기），节奏本来就是参差的 ——
它不属于这条规则要处理的情况，而且韩文回复是**整体一致地不标记**，不会自相矛盾。

所以判据收敛到真正的属性上，名字也跟着走（仓库规则：一套词汇，不留旧名别名）：
`HAN_TEXT` → `UNSPACED_SCRIPT`，`data-markdown-han-text` → `data-markdown-unspaced`
（组件、CSS 两条规则、测试三处同步改名）。

### 四、破坏验证

把判据还原成 `\p{Script=Han}` → 三段日文只标中 1 段。
测试同时钉住反面：韩文 + 英文必须**零标记**。

### 验收

| | 结果 |
| --- | --- |
| 封口验证的形状 | 10 种，全部 `nested=0` |
| 真缺陷 | **1**（日文全假名段落打断间距节奏） |
| 刻意不纳入并写明理由的 | 1（韩文，有词间空格） |
| 改名同步的文件 | 3（组件 / CSS / 测试） |
| 破坏验证 | 3 段只剩 1 段 |

### 一句话

`\p{Script=Han}` 看着像"CJK"，**它只是汉字** ——
而日文可以一个汉字都不写。更要命的是那条 CSS 配的是**相邻两段**：
...
### 附：一条低频 flaky 的基线（本轮不修）

全量视觉跑到第 11 次时，`layoutShift › workspace states hold still` 报红一次：

```
fixture=workspace&state=dock-file  shifted 0.0007
   <span …> y 58 -> 58
```

位移 **0.0007**，而 y 值没变 —— 是**水平**方向的亚像素残留，
出现在 `dock-file`（带滚动条的语法高亮代码视图）。

单独复跑 **8/8 全部通过**；9 次观测里 1 次失败（约 11%）。

**不修，也不加阈值。** 那条守卫的注释明确写着 "Nothing here filters"，
...
## Round 248 —— 一张手工表和 runtime 精确同步，而没有任何东西在守它

### 一、两个判据查完是干净的

- **`INLINE_IMAGE`**（决定哪些图片能显示）只允许 `data:image/…`。
  那是**刻意的安全边界**（桌面侧拒绝抓取远程图片，已有"不抓取跟踪像素"的测试），
  不是判据事故。格式列表 avif/gif/jpeg/png/svg+xml/webp 也够用。
- **`toolFamilyId`** 背后的 `TOOL_FAMILIES` 是手工维护的 30 个内置工具。
  逐个比对 runtime 的权威常量（`runtime/internal/domain/run/tool/tool.go`）：
  **30 = 30，双向零差异。** 表本身没问题。

### 二、问题不在表的内容，在于没有任何东西守着它

`toolRendering.test.ts` 检查的是**内部一致性**：每个工具要么有 preview、
要么由常驻界面（plan bar / goal bar / Schedules）接管，且 preview 互不重复。

但它把 `TOOL_ICON_BY_NAME` **当作真值来源**来遍历。
所以 runtime 哪天加一个内置工具，前端这张表不更新的话：

| 查表 | 结果 |
| --- | --- |
| `toolFamilyId()` | `undefined` → **审批卡显示裸 wire 名**，而不是能力族 |
| `TOOL_ICON_BY_NAME` | 无条目 → 兜底图标 |
| `toolVerbId()` | `undefined` → 目录里读作通用动词 |

**而这个文件周围的所有测试都会保持全绿** —— 因为它们遍历的正是这张表本身。
`ApprovalCard` 的注释里记录过同一形状的事故：
"the card asked permission to run `web_fetch` while the row beneath it said 'Fetched'"。

### 三、从契约层守不了，我查过了

本想写一条对着 `runtime/contract/` 校验的守卫（`check:api-consumers` 已有先例）。
逐个扫 `manifest.json`：

**30 个内置工具名里，只有 6 个出现在契约的任何地方** ——
`lsp` 在 `features[13]`，另外 5 个（glob / grep / apply_patch / shell / web_search）
出现在 `toolResultPresentations`，而那是"**有结构化结果**"的工具，不是词汇表。

**Runtime Protocol 不发布内置工具词汇表。** 要让这个同步可守，
要么协议发布它，要么守卫去读 runtime 内部 —— 前者是协议改动（指导禁止我主动动），
后者会把桌面耦合到 runtime 的内部结构，越过契约这条边界。

### 四、所以这一轮做的是把这件事写在会被读到的地方

那个文件原来的注释解释了"为什么在 lib"、"为什么不用颜色区分"、
"这是词汇表不是实时清单" —— **唯独没说真值在哪、以及没人在守**。

补上的注释点名三件事：名字属于 `runtime/internal/domain/run/tool/tool.go`；
今天精确一致是**比对过的**；以及漏掉一个工具时**三个查表同时失效而所有测试保持绿**。

### 留给用户的一个选择

要让它可守，只有两条路，都在用户手里：

1. **协议发布内置工具词汇表**（manifest 里加一份名字清单）——
   之后可以写 `check:*` 守卫，和 `check:api-consumers` 同一条路子。
2. **接受它是手工同步**，靠这条注释提醒。
...
| | 结果 |
| --- | --- |
| 查完确认干净的判据 | 2（`INLINE_IMAGE` 安全边界 / `TOOL_FAMILIES` 内容） |
| 验证过的同步 | 30 = 30，双向零差异 |
| 找到的守卫缺口 | **1**，且**从契约层不可守**（30 个里契约只提 6 个） |
| 本轮改动 | 注释 1 处（点名真值来源与失效方式） |
| 提给用户的选择 | 1（协议是否发布工具词汇表） |

### 一句话

这一次找到的不是一处错，是**一处没人看着的正确** ——
一张手工表今天和 runtime 一个字不差，
...
## Round 250 —— 先用眼睛看了一遍 UI，然后补上守卫自己写明看不到的那块

### 一、真的去看了产品长什么样

前面十几轮都在比数值、查代码。这轮打开 golden 逐屏看：

**Work Index（1440×900）**：会话行 34px 等距；右侧元数据（"6 days ago" / "Jul 24"）
和项目计数（"6" / "1"）右对齐到同一边；带状态点的行比不带的**提前截断给点让位**；
区域之间不画线 —— 与 `DESKTOP_UI_POLISH` 的规则一致。

**Diff review（workspace dock）**：diff 行号右对齐、`-`/`+` 独立成列；
长路径用**前置省略号**保住文件名（`…/shell/kernel/panel/DockResizer.tsx`）；
换行的续行既无行号也无标记，因而可辨。

**没找到可指名的排版缺陷。** 记一处设计判断（不是缺陷）：
`+25 -9` 在同一屏出现两次 —— 顶部 chrome 栏（会话级）和 Diff 视图头（视图级），
同样的绿/红权重。两者立场不同，属设计选择。

### 二、`check-locales` 自己写明了它看不到什么

规则 5 的原文：

> Dynamic keys (a table of them, `` t(`x.${y}`) ``) can't be checked here and aren't.

而规则 9 的实现把**整个前缀**加进豁免集。产品里这样的动态键有四族：

| 前缀 | 值从哪来 | 表能产生 | 兜底 | 目录里 |
| --- | --- | --- | --- | --- |
| `tools.family.` | `TOOL_FAMILIES[].id` | 10 | `UNPLACED_FAMILY` | 11 ✓ |
| `tool.doing.` | 工具名 → camelCase | 30 | `GENERIC_VERB_ID` | 31 ✓ |
| `tool.done.` | 同上 | 30 | 同上 | 31 ✓ |
| `rpcError.` | `MAPPED_TYPES` | 28 | — | 28 ✓ |

**101 个键，双向精确，兜底有据。** 又一处"今天正确、无人守着"——
但这三族的值**是可枚举的**，所以豁免可以换成核对。

### 三、Rule 16：把整段豁免换成逐个枚举

两个方向都查：

- **表能产生、目录没有** → 读者看到自己的键，如 `tool.doing.someNewThing`
- **目录有、没人产生** → 死文案，而它要被翻进**全部 8 种语言**

兜底键（`other` / `generic`）**从命名它的常量里读**，不在守卫里再拼一遍 ——
否则改个常量名就会把它的键悄悄变成"死文案"。

破坏验证两个方向都精确命中：
给表加一个工具 → `tool.doing.newTool` / `tool.done.newTool`；
加一条 `rpcError.ghost_symbol` → 精确点名（并连带触发"其他语言缺这个键"，规则正确叠加）。

### 四、顺带记一个不动的小问题

这个脚本的规则编号**已经冲突**：代码里有三处 `Rule 12`（省略号、spec 携带键、以及我最初写的），
两处 `Rule 9`。我把自己的改成了 **16**（头部最大号是 15），**没有去重排既有编号** ——
那是纯改名的搅动，会让每条规则的 blame 失真，而收益只是可读性。记录。

### 验收

| | 结果 |
| --- | --- |
| 逐屏看过的 golden | 2（Work Index / Diff review） |
| 看出的排版缺陷 | **0**（记了 1 处设计判断） |
| 补上的守卫盲区 | **1**（脚本自己写明的动态键豁免） |
| 新规则覆盖的键 | 101（四族 + 两个兜底） |
| 破坏验证 | 双向各命中 |
| 不做并记录的 | 1（既有规则编号冲突） |
...
一个守卫在自己的头部写着"**这一类我查不了**"，
而其中三族的值其实就摆在源码的表里 ——
可枚举的东西不该被整段豁免。
补完之后它守的是两个方向：**读者看到自己的键**，
和**八种语言各翻一遍一句没人能读到的话**。

## Round 251 —— composer 工具条的"让出标签"机制，从来没被测过

### 一、两次无效探针，都是自己的错

想验证工具条在放不下时是否**整体让出标签**（而不是把三个 chip 都截成残桩）。

**第一次**：从 1472 一路收窄到 560 —— `labelled` 始终为 true、3 个标签全在、零溢出。
原因不是机制没生效，是 composer 在阅读栏里有最大宽度，**收窄视口根本触发不到**。

**第二次**：注入一个 41 字符的长模型名再触发 resize。结果仍然全绿 ——
而这次是**探针本身无效**：`setLabelled()` 会触发重渲染，
**React 用 props 里的原文覆盖掉了我改的文本**，我读到的是还原后的状态。

（这是本会话第三次自己的测量出错，三次都靠"结果太干净了"这个直觉查出来。）

### 二、第三次：观察器盯的是 footer 自己的盒子，那就给它一个宽度

```
footer 宽度       带标签的自然宽      结果
768（实际）       768                保留 3 个标签，零溢出
强制 340          460                标签全部让出，溢出 0   ← 设计意图
强制 260          460                仍然放得下
强制 200 / 150    460                溢出 38 / 88
```

**机制是对的。** 200px 以下才会真正溢出，而 composer 不可能窄到那里
（最窄视口 560 时 footer 仍是 768px）。

### 三、所以问题不是机制错，是它从来没被测过

`useToolbarLabels` 的注释解释了为什么这件事 CSS 答不了：
flexbox 同时压缩每个 chip，而每个标签盒按自己的文字定宽，
于是整行变成 `Balanc…` / `GP…` / `Mediu…` 三个残桩 ——
"each was tried against a screenshot and each still ellipsed"。

它依赖一条 `!important`（全文件唯一一处），注释写着它的作用是：
**"losing here does not misplace a pixel, it makes the row measure as fitting and the labels never go."**

而**没有任何 fixture 能触发它**。回归会静默变回那三个残桩。

### 四、守卫，以及那条 floor 恰好就是抓手

`visual/toolbarLabels.visual.spec.ts`：宽的时候有标签且不截断；
强制到 340px 时**标签为 0、整行不溢出、没有任何 chip 被省略号截断**。

我本来加了一条 floor 防止"挤压没挤到"——
破坏验证时发现**它正是抓住真实回归的那条**：
拿掉 `!important` 后，测到的"带标签自然宽"回来是 **340**（就是被挤压后的宽度本身），
因为 chip 在被测量时跟着缩了 —— 一字不差地就是注释那句话。

消息因此改写成同时指向两种可能：

```
the labelled row measured 340px inside 340px — either the squeeze is too wide,
or the chips are shrinking while being measured
```
...
| | 结果 |
| --- | --- |
| 无效探针 | **2**（收窄视口触发不到 / React 覆盖了注入文本） |
| 验证为正确的机制 | 1（让出标签，四档宽度实测） |
| 新守卫 | 1（fixture 造不出的条件，自己制造） |
| 破坏验证 | 命中，且命中的是 floor 而不是主断言 |

### 一句话

三次探针才问对问题：**观察器盯的是 footer 自己的盒子，那就给它一个宽度**。
而最值得记的是破坏验证的落点 ——
...
## Round 254 —— 差点加了一条永远不会红的守卫

### 一、四种行组件的横向对照

产品里有四种行：`navigation-row`（工作索引）、`option-row`（浮层）、
`step-row`（计划）、`viewStyles.treeRow`（文件树）。比它们对同一批问题的处理：

| | 对齐 | 换行 |
| --- | --- | --- |
| `navigation-row` | 外层 `flex-start` + 内层 `center` | **无 `overflowWrap`** |
| `option-row` | `center` | 无（注释写明"每个第二子元素都截断"） |
| `step-row` | `flex-start`（Round 235 修） | `break-word` |
| `treeRow` | `center` | 无 |

`navigation-row` 少 `overflowWrap` 看着像缺口 —— 但它的标签走截断，**无碍**。

### 二、真正的发现：第三个区域没人审计

`unbreakableContent` 有两个根：agent fixture 的 `main`、workspace 的 `.agent-context-dock`。
**工作索引（drawer）在两者之外** —— 而那里落着 agent 生成的**会话标题**。

同一个"审计范围"形状。于是我写了第三条测试，根设在 `.agent-drawer`，**跑过，绿的**。

### 三、然后问了那个该问的问题：它可能变红吗

不能。实测：

| 条件 | drawer 的宽度 |
| --- | --- |
| 正常 | `clientWidth = scrollWidth = 275` |
| blob 塞进全部 15 个文本节点 | `275 / 275` |
| **再加全局 `overflow-wrap: normal !important`** | **`275 / 275`** |

元素级也一样：10 个目标、8 个 `overflow-x: visible`，禁止断行后溢出仍是 **0**。

**原因**：drawer 宽度固定（侧栏宽度），里面每个标题由**祖先**裁切，
而不是由持有文字的元素自己断行 —— 这条审计赖以成立的前提在那里根本不适用。

### 四、所以删掉它，把"试过了"留下

一条不可能变红的守卫**比没有更糟**：它会报告一个它从未检查过的区域的覆盖率。
Round 233 专门查过"有没有守卫能读不到东西还报绿"，答案是零 —— 我差点新增一个。

删掉测试，在规格里留下**为什么不需要**：下一个人看到这个缺口时，
会先读到"试过了，量过了，前提不适用"，而不是重写一遍。
...
| --- | --- |
| 横向对照的行组件 | 4 |
| 看似缺口、实测无碍的 | 1（`navigation-row` 的 `overflowWrap`） |
| 写了又删掉的守卫 | **1**（drawer 审计，结构上不可能失败） |
| 留下的记录 | 1（在规格里，含三组实测数字） |
| 产品代码改动 | **零** |

### 一句话

这一轮**没有改一行产品代码**，产出是一条**没有被加进去的测试** ——
...
## Round 257 —— 同一个缺陷的第三、第四次：谁在行里让路

延续上一轮的横扫，换条件：**8 组（en/de/ja/fr/ko × 16px/18px × comfortable/compact）× 36 状态**。
上一轮只跑了浅色、默认字号、英文。

两条发现，都是同一个根因的新实例 —— **一行里唯一能收缩的，恰好是唯一不该消失的**。

| | 位置 | 条件 | 结果 |
| --- | --- | --- | --- |
| ① | 共享活动行的 `trailing` | 德语 335px 行 | `Subagent` → **1.1px** |
| ② | timeline 运行行的 id | 德语 + 18px | `run_child` → **0px**（行还溢出 16px） |

①的 `trailing` 是 `flex-shrink: 0`，却装着**由 locale 决定长度**的文本
（"Eingabe erforderlich" + 步数 = 229px），标签是唯一能让路的。

### 两个"显然的配套"都测出了代价

| 做法 | 代价 |
| --- | --- |
| 给槽位加 `overflow: hidden` | **切掉了 `StatusDot` 的光晕** —— 它的脉冲画在自己盒子外面 |
| 给标签加 `min-width` 下限 | 撑宽了所有短于下限的标签，**移动了从未被挤压的行** |

第一个是放大像素才看出来的：旧 golden 的蓝点带一圈柔光，新的是个秃点。
第二个是靠 golden 抓到的 —— `tool-shells` 整片文本位移了两三个像素。

最后的形状同时避开三种缺陷：**槽位收缩但不裁剪**（光晕保住），
**点旁边的文字自己截断**（不喷出盒子）。
...
| 扫描 | 8 条件 × 36 状态 |
| 修掉 | 2（活动行标签、运行 id） |
| 试过又撤掉 | 2（槽位裁剪、标签下限），各有实测代价 |
| golden 变化 | **0** |
| 全套 | 745 passed |

（`followTail` 在满负载长跑里抖了一次 —— 它自己的 floor 报的"读者没离开底部"，
隔离重跑 3/3 通过。不去动阈值掩盖它。）

### 一句话

上一轮的教训是"截图只测被摆好的那一帧"，
这一轮是：**一个只在德语、只在最大字号下出现的缺陷，英文 golden 永远拍不到**。
而两次试图修它的"显然做法"，各自又制造了一个新缺陷 ——
两个都是靠实测挡下来的，不是靠想。
