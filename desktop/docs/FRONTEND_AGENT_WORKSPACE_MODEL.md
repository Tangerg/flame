...
> 本文记录 Flame desktop 现行的主 UI 心智模型。它回答的是：
...
> [`runtime/contract/API_REFERENCE.md`](../../runtime/contract/API_REFERENCE.md)。

## 1. North Star

**Flame 是一个 AI agent 工作台，不是普通 IDE，也不是功能菜单驱动的后台系统。**

主界面不是回答“Flame 有哪些功能”，而是回答：
...
Flame Runtime Protocol 的核心原语已经决定了 UI 的主轴：
...
Flame 已经有主题系统，后续 UI 调整必须把主题系统当作唯一视觉权威：
...
- popup / menu / select 走 Base UI primitive，视觉由 Flame theme 接管。
...
- Context Dock destinations 已进入 `flame.contextDock.destination` contribution registry，首批内置入口由 workspace 插件贡献，launcher 按 `workspace / run / session` scope 渲染 read model。
