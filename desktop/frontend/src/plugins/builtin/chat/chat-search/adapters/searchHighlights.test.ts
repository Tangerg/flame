...
afterEach(() => {
  document.getElementById("flame-chat-search-highlight-styles")?.remove();
});
...
describe("installChatSearchHighlightStyles", () => {
  it("owns one runtime stylesheet and removes the instance it created", () => {
    ...
    const style = document.getElementById("flame-chat-search-highlight-styles");
    expect(style?.textContent).toContain("::highlight(chat-search-active)");

    dispose();
    expect(document.getElementById("flame-chat-search-highlight-styles")).toBeNull();
  });
  ...
  it("does not remove a stylesheet owned by an existing adapter instance", () => {
    ...
    disposeGuest();
    expect(document.getElementById("flame-chat-search-highlight-styles")).not.toBeNull();
