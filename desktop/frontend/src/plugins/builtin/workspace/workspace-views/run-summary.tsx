    ...
    >
      <div {...stylex.props(vs.gutter, vs.statusPad)}>
        <Badge
          ...
          face="mono"
          className={stylex.props(vs.semibold).className}
        >
      ...
      <Section title="runSummary.section.changedFiles" count={view.changedFiles.count}>
        {view.changedFiles.items.map((f) => (
          <div key={f.path} {...stylex.props(vs.entry, vs.muted, typeStep.uiMd)}>
            <Icon name="filetext" size="xs" className={stylex.props(vs.caption).className} />
            <FilePath path={f.path} className={stylex.props(vs.grow, vs.ink).className} />
      ...
      <Section title="runSummary.section.readFiles" count={view.readFiles.count}>
        {view.readFiles.items.map((p) => (
          <div key={p} {...stylex.props(vs.entry, vs.muted, typeStep.uiMd)}>
            <Icon name="filetext" size="xs" className={stylex.props(vs.caption).className} />
            <span {...stylex.props(vs.truncate)}>{p}</span>
          </div>
        ))}
      </Section>
      ...
      <Section title="runSummary.section.approvals" count={view.approvals.count}>
        {view.approvals.items.map((a, i) => {
          ...
          return (
            <div key={`${a.command}:${i}`} {...stylex.props(vs.entry, vs.muted, typeStep.uiMd)}>
              <Icon name="shield" size="xs" className={stylex.props(vs.caption).className} />
              <span {...stylex.props(vs.truncate)}>
      ...
      <Section title="runSummary.section.errors" count={view.errors.count}>
        {view.errors.items.map((e, i) => (
          <div key={`${e}:${i}`} {...stylex.props(vs.entryPlain, vs.negative, typeStep.uiMd)}>
            <Icon name="bug" size="xs" />
            <span {...stylex.props(vs.wrapText)}>{e}</span>
          </div>
        ))}
      </Section>
    </WorkspaceViewLayout>
