...
describe("Go Runtime ↔ HTTP ↔ TypeScript SDK", () => {
  ...
  const runtimeEnvironment = () => ({
    ...process.env,
    HOME: runtimeHome,
    FLAME_HOME: runtimeData,
    FLAME_PROVIDER: "openai-compatible",
    FLAME_MODEL: "e2e-model",
    FLAME_APIKEY: "e2e-placeholder-key",
    FLAME_BASEURL: providerBaseUrl,
...
`,
  ...
  it("converges unary mutations across pre-admission and post-commit disconnects", async () => {
    ...
    try {
      for (const cutpoint of ["beforeCommit", "afterCommit"] as const) {
        ...
        const faultClient = createFlameClient(
      ...
      const faultClient = createFlameClient(createHttpTransport({ baseUrl, fetch: fault.fetch }), {
        requestMeta,
      });
      try {
  ...
  it("reconciles Schedule response and acknowledgement cutpoints through cold reads", async () => {
    ...
    const callAfterCommitLoss = async <T>(
      method: string,
      call: (faultClient: FlameClient) => Promise<T>,
    ): Promise<T> => {
  ...
  it("reconciles skill archive and restore through skills.changed", async () => {
    ...
    const projectSkillDirectory = join(root, ".flame", "skills", projectSkillName);
  ...
  it("consumes workspace files, recipes, agent docs and hook trust through bound APIs", async () => {
    ...
    await Promise.all([
      ...
      writeFile(
        join(projectRoot, ".flame", "hooks.json"),
        JSON.stringify({
    ...
    const externalHookChanges = [
      ...
      {
        path: join(projectRoot, ".flame", "hooks.json"),
        marker: "external project hook",
        scope: "project",
        event: "UserPromptSubmit",
      },
      {
        path: join(workspaceRoot, ".flame", "hooks.json"),
