...
- **结构细节**（目录树 / 各模块职责 / composition root）见 `frontend/ARCHITECTURE.md`。Go 侧只是 Wails 壳、不内嵌 runtime；前端经 HTTP 连外部 Flame Runtime。
...
- **协议**：自研 Flame Runtime Protocol v2（JSON-RPC 2.0，已弃用 AG-UI），权威定义见 `../runtime/doc/`。
